...
type preferences = {
  ...
  withdoc : bool;
  byteonly : bool;
  caml_flags : string option;
  camlbyte_flags : string option;
  camlopt_flags : string option;
  debug : bool;
  profile : bool;
  bin_annot : bool;
  annot : bool;
...
let args_options = Arg.align [
  ...
  "-camlflags", arg_string_option (fun p caml_flags -> { p with caml_flags }),
    "<flags> Specifies additional flags to be passed to the ocaml compiler [byte and opt]";
  "-camlbyteflags", arg_string_option (fun p camlbyte_flags -> { p with camlbyte_flags }),
    "<flags> Specifies additional flags to be passed to the ocaml compiler [only byte]";
  "-camloptflags", arg_string_option (fun p camlopt_flags -> { p with camlopt_flags }),
    "<flags> Specifies additional flags to be passed to the ocaml compiler [only opt]";
...
let opt_iter f o = match o with Some x -> f x | _ -> ()
let opt_dflt d o = match o with Some x -> x | _ -> d
...
let print_summary () =
  ...
  opt_iter (pr "  OCaml compiler extra flags  : %s\n") !prefs.caml_flags;
  opt_iter (pr "  ocamlc   extra flags        : %s\n") !prefs.camlbyte_flags;
  opt_iter (pr "  ocamlopt extra flags        : %s\n") !prefs.camlopt_flags;
  pr "  Camlp5 version              : %s\n" camlp5_version;
...
let write_configml f =
  ...
  pr_s "caml_extra_flags" (opt_dflt "" !prefs.caml_flags);
  pr_s "camlbyte_extra_flags" (opt_dflt "" !prefs.camlbyte_flags);
  pr_s "camlopt_extra_flags" (opt_dflt "" !prefs.camlopt_flags);
...
let write_makefile f =
  ...
  pr "USERFLAGS=%s\n" (opt_dflt "" !prefs.caml_flags);
  pr "USERBYTEFLAGS=%s\n" (opt_dflt "" !prefs.camlbyte_flags);
  pr "USEROPTFLAGS=%s\n" (opt_dflt "" !prefs.camlopt_flags);
  pr "# Flags for GCC\n";
  pr "CFLAGS=%s\n\n" cflags;
