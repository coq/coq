...
let coqc_init ((_,color_mode),_) injections ~opts =
  ...
  Compiler.Colors.init_color (if opts.Coqargs.config.Coqargs.print_emacs then `EMACS else color_mode);
  DebugHook.Intf.(set
...
let custom_coqc : ((Coqcargs.t * Compiler.Colors.color) * Stm.AsyncOpts.stm_opt, 'b) Coqtop.custom_toplevel
 = Coqtop.{
