...
and v_annotated_argpat = Tuple ("*", [|Array v_name; v_argpat|])
and v_case_ret = Tuple ("*", [|v_annotated_argpat; v_sort_annot|])
...
let v_imit_cmp = Sum ("imitation_cmp", 1, [|[|List v_level|]; [|List v_level|]|])
...
let v_rrinfo = v_tuple "rewrite_rule_info"
  [|v_map v_qvar v_bool;    (* qualities *)
    ...
    List (v_tuple "full_ucstr" [|v_quality; v_univ; v_bool; v_univ|]); (* full_ucstrs *)
    v_evar_map;             (* evar_map *)
