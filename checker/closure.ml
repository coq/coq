...
let stop() =
  msgerrnl (str "[Reds: beta=" ++ int !beta ++ str" delta=" ++ int !delta ++
...
let rec knh m stk =
  match m.term with
    ...
    | FCaseT(ci,p,t,br,env) -> knh t (ZcaseT(ci,p,br,env)::zupdate m stk)
    | FFix(((ri,n),(_,_,_)),_) ->
...
and knht e t stk =
  match t with
    | App(a,b) -> knht e a (append_stack (mk_clos_vect e b) stk)
    | Case(ci,p,t,br) -> knht e t (ZcaseT(ci, p, br, e)::stk)
