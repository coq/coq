...
let write_config_cmf caml_flags o =
  let pr s = fprintf o s in
  let pr_s = pr "let %s = %S\n" in
...
let write_configml camlenv coqenv caml_version_nums arch arch_is_win32 hasnatdynlink browser prefs o =
...
let main () =
  ...
  write_config_file ~file:"config/coq_config.ml"
    (write_configml camlenv coqenv caml_version_nums arch arch_is_win32 hasnatdynlink browser prefs);
  write_config_file ~file:"config/coq_mf_config.ml"
    (write_config_cmf caml_flags);
