...
module Latex = struct
  ...
  let sublexer c loc =
    if c = '*' && !last_was_in then begin
      ...
      let tag =
        try Some (Index.find !current_module loc) with Not_found -> None
      in
  ...
  let ident s loc =
    ...
    try
      ...
      | Some loc ->
          let tag = Index.find !current_module loc in
...
module Html = struct
  ...
  let start_module () =
    let ln = !lib_name in
    if not !short then begin
      add_toc_entry (Toc_library (!current_module, get_full_title ()));
      if ln = ""  then
  ...
  let ident s loc =
    try
      ...
      | None -> raise Not_found
      | Some loc ->
         reference (translate s) (Index.find !current_module loc)
    with Not_found ->
      if is_tactic s then
