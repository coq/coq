...
let functional_induction with_clean c princl pat =
  let res g =
    ...
    let princ, bindings, princ_type, g' =
      ...
      | None -> (
        ...
        | Const (c', u) ->
          let princ_option =
            let finfo =
              ...
              try find_Function_infos c' with Not_found ->
                user_err
                  ( str "Cannot find induction information on "
                  ++ Printer.pr_leconstr_env (Tacmach.pf_env g) sigma
                       (mkConst c') )
            in
          ...
          let princ, g' =
            ...
            try
              let g', princ =
                Tacmach.pf_eapply Evd.fresh_global g
                  (Globnames.ConstRef (Option.get princ_option))
              in
              (princ, g')
            with Option.IsNone -> (
        ...
        | _ ->
          raise
            (UserError
               (None, str "functional induction must be used with a function"))
        )
      | Some (princ, binding) ->
    ...
    let sigma = Tacmach.project g' in
    let princ_infos =
      Tactics.compute_elim_sig (Tacmach.project g') princ_type
    in
    ...
    in
    let old_idl =
      List.fold_right Id.Set.add (Tacmach.pf_ids_of_hyps g) Id.Set.empty
    in
             ...
             (fun id ->
               Tacticals.tclTRY
                 (Proofview.V82.of_tactic
                    (Equality.subst_gen (do_rewrite_dependent ()) [id])) )
             idl)
          (Proofview.V82.of_tactic
  ...
  let rec_sign, rec_impls =
    List.fold_left
      (fun (env, impls) (({CAst.v = recname}, _), bl, arityc, _) ->
        ...
        let evd, (_, (_, impls')) =
          Constrintern.interp_context_evars ~program_mode:false env evd bl
        in
        let impl =
        ...
        ( EConstr.push_named (LocalAssum (recname, arity)) env
  ...
  let l' =
    List.map
      (fun ((fixna, _, bll, ar, body_opt), lnot) ->
        match body_opt with
        | Some body -> (fixna, bll, ar, body)
        | None ->
          user_err ~hdr:"Function" (str "Body of Function must be given") )
      l
  in
    ...
    try
      let evd', lind =
        List.fold_right
          ...
          fix_names (evd', [])
      in
      Invfun.derive_correctness Functional_principles_types.make_scheme
        fix_names_as_constant lind
...
let warn_cannot_define_graph =
  CWarnings.create ~name:"funind-cannot-define-graph" ~category:"funind"
    (fun (names, error) ->
      strbrk "Cannot define graph(s) for " ++ h 1 names ++ error )
...
let generate_principle (evd : Evd.evar_map ref) pconstants on_error is_general
    do_built
    (fix_rec_l :
      (Vernacexpr.fixpoint_expr * Vernacexpr.decl_notation list) list) recdefs
    interactive_proof
    (continue_proof :
         int
  ...
  let names =
    List.map
      (function (({CAst.v = name}, _), _, _, _, _), _ -> name)
      fix_rec_l
  in
  let fun_bodies = List.map2 prepare_body fix_rec_l recdefs in
  let funs_args = List.map fst fun_bodies in
  let funs_types =
    List.map (function (_, _, _, types, _), _ -> types) fix_rec_l
  in
  try
      ...
      let _ =
        List.map_i
          (fun i x ->
            ...
            Functional_principles_types.generate_functional_principle evd
              interactive_proof princ_type None None (Array.of_list pconstants)
              (* funs_kn *)
              i
...
let register_struct is_rec
  ...
  | [((({CAst.v = fname}, pl), _, bl, ret_type, body), _)] when not is_rec ->
    ...
    in
    ComDefinition.do_definition ~program_mode:false fname
      (Decl_kinds.Global, false, Decl_kinds.Definition)
      pl bl None body (Some ret_type);
    let evd, rev_pconstants =
      List.fold_left
        (fun (evd, l) ((({CAst.v = fname}, _), _, _, _, _), _) ->
          let evd, c =
            Evd.fresh_global (Global.env ()) evd
              (Constrintern.locate_reference (Libnames.qualid_of_ident fname))
          in
          let cst, u = destConst evd c in
...
let register_wf ?(is_mes = false) fname rec_impls wf_rel_expr wf_arg
    using_lemmas args ret_type body pre_hook =
  ...
  let rec_arg_num =
    ...
    | Some wf_arg -> List.index Name.equal (Name wf_arg) names
  in
  let unbounded_eq =
    let f_app_args =
      CAst.make
    ...
    CAst.make
    @@ Constrexpr.CApp
         ( (None, Constrexpr_ops.mkRefC (qualid_of_string "Logic.eq"))
  ...
  let hook ((f_ref, _) as fconst) tcc_lemma_ref (functional_ref, _) (eq_ref, _)
    ...
    with e when CErrors.noncritical e -> (* No proof done *)
                                         ()
  in
  Recdef.recursive_definition is_mes fname rec_impls type_of_f wf_rel_expr
    rec_arg_num eq hook using_lemmas

let register_mes fname rec_impls wf_mes_expr wf_rel_expr_opt wf_arg
  ...
  in
  register_wf ~is_mes fname rec_impls wf_rel_from_mes (Some wf_arg)
    using_lemmas args ret_type body
...
let recompute_binder_list
    (fixpoint_exprl :
      (Vernacexpr.fixpoint_expr * Vernacexpr.decl_notation list) list) =
  let fixl, ntns =
    ComFixpoint.extract_fixpoint_components false fixpoint_exprl
  in
  let (_, _, typel), _, ctx, _ =
    ComFixpoint.interp_fixpoint ~cofix:false fixl ntns
  in
  let constr_expr_typel =
    with_full_print
      (List.map (fun c ->
           Constrextern.extern_constr false (Global.env ()) (Evd.from_ctx ctx)
             (EConstr.of_constr c) ))
      typel
  in
  let fixpoint_exprl_with_new_bl =
    List.map2
      (fun ( (lna, (rec_arg_opt, rec_order), bl, ret_typ, opt_body)
           , notation_list ) fix_typ ->
        let new_bl', new_ret_type = rebuild_bl [] bl fix_typ in
        ( ( (lna, (rec_arg_opt, rec_order), new_bl', new_ret_type, opt_body)
          , notation_list )
          : Vernacexpr.fixpoint_expr * Vernacexpr.decl_notation list ) )
  ...
  List.iter
    (fun (_, l) ->
      if not (List.is_empty l) then
        error "Function does not support notations for now" )
    fixpoint_exprl;
  let _is_struct =
    match fixpoint_exprl with
    ...
    ->
      ...
      if register_built then
        register_wf name rec_impls wf_rel
          (map_option (fun x -> x.CAst.v) wf_x)
          using_lemmas args types body pre_hook;
      false
    | [ ( ((_, (wf_x, Constrexpr.CMeasureRec (wf_mes, wf_rel_opt)), _, _, _), _)
        as fixpoint_expr ) ] ->
      ...
      in
      if register_built then
        register_mes name rec_impls wf_mes wf_rel_opt
          (map_option (fun x -> x.CAst.v) wf_x)
          using_lemmas args types body pre_hook;
      true
    | _ ->
      List.iter
        (function
          | (_na, (_, ord), _args, _body, _type), _not -> (
            match ord with
            | Constrexpr.CMeasureRec _ | Constrexpr.CWfRec _ ->
              error
                "Cannot use mutual definition with well-founded recursion or \
                 measure"
            | _ -> () ))
        fixpoint_exprl;
...
let rec add_args id new_args =
  CAst.map (function
      ...
      CProdN
        ( List.map
            (function
              ...
              | CLocalPattern _ ->
                user_err (Pp.str "pattern with quote not allowed here."))
            nal
        , add_args id new_args b1 )
    | CLambdaN (nal, b1) ->
      CLambdaN
    ...
    | CGeneralization _ ->
      anomaly ~label:"add_args " (Pp.str "CGeneralization.")
...
let make_graph (f_ref : GlobRef.t) =
  let c, c_body =
    match f_ref with
    | ConstRef c -> (
      try (c, Global.lookup_constant c) with Not_found ->
  ...
  | Some (body, _) ->
        ...
        let l =
          List.map
            (fun (id, (n, recexp), bl, t, b) ->
              ...
              ( ( ( (id, None)
                  , (Some CAst.(make rec_id), CStructRec)
                  , nal_tas @ bl
                  , t
                  , Some b' )
                , [] )
    ...
    do_generate_principle [(c, Univ.Instance.empty)] error_error false false
      expr_list;
    (* We register the infos *)
    List.iter
      (fun ((({CAst.v = id}, _), _, _, _, _), _) ->
