...
let rec alpha_rt excluded rt =
  ...
  let new_rt =
    ...
    | (GSort _ | GInt _ | GHole _) as rt -> rt
    | GCast (b, c) ->
...
let is_free_in id =
  let rec is_free_in x =
    DAst.with_loc_val
      (fun ?loc -> function GRef _ -> false
        | GVar id' -> Id.compare id' id == 0 | GEvar _ -> false
        | GPatVar _ -> false
  ...
  DAst.with_val
    (function
      ...
      | PatCstr (constr, patternl, _) ->
        ...
        mkGApp
          (mkGRef (Globnames.ConstructRef constr), implicit_args @ patl_as_term))
    pt
  ...
  | (l, r) :: eqs -> (
    ...
    | PatCstr (constructor1, cpl1, _), PatCstr (constructor2, cpl2, _) ->
      if not (eq_constructor constructor2 constructor1) then raise NotUnifiable
        ...
        let eqs' =
          try List.combine cpl1 cpl2 @ eqs with Invalid_argument _ ->
            anomaly (Pp.str "are_unifiable_aux.")
        in
  ...
  | (l, r) :: eqs -> (
    ...
    | PatCstr (constructor1, cpl1, _), PatCstr (constructor2, cpl2, _) ->
      if not (eq_constructor constructor2 constructor1) then raise NotUnifiable
        ...
        let eqs' =
          try List.combine cpl1 cpl2 @ eqs with Invalid_argument _ ->
            anomaly (Pp.str "eq_cases_pattern_aux.")
        in
