  ...
  funind_purify
    ...
    Array.iteri
	 ...
	 let lem_id = mk_correct_id f_id in
         ...
         let lemma = Lemmas.start_lemma ~ontop:None
	   lem_id
         ...
         let _ = Lemmas.save_lemma_proved ?proof:None ~lemma ~opaque:Proof_global.Transparent ~idopt:None in
    ...
    Array.iteri
	 ...
	 let lem_id = mk_complete_id f_id in
         let lemma = Lemmas.start_lemma ~ontop:None lem_id
           (Decl_kinds.Global,false,(Decl_kinds.Proof Decl_kinds.Theorem)) sigma
         (fst lemmas_types_infos.(i)) in
         let lemma = fst (Lemmas.by
	   (Proofview.V82.tactic (observe_tac ("prove completeness ("^(Id.to_string f_id)^")")
              (proving_tac i))) lemma) in
         let _lemma = Lemmas.save_lemma_proved ?proof:None ~lemma ~opaque:Proof_global.Transparent ~idopt:None in
	 let finfo = find_Function_infos (fst f_as_constant) in
	 let _,lem_cst_constr = Evd.fresh_global
				  (Global.env ()) !evd (Constrintern.locate_reference (Libnames.qualid_of_ident lem_id)) in
         let (lem_cst,_) = destConst !evd lem_cst_constr in
