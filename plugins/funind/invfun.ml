...
let do_observe_tac s tac g =
  let goal =
    try Printer.pr_goal g with e when CErrors.noncritical e -> assert false
  in
  try
    let v = tac g in
    msgnl (goal ++ fnl () ++ s ++ str " " ++ str "finished");
    v
  with reraise ->
...
let observe_tac s tac g =
  if do_observe () then do_observe_tac (str s) tac g else tac g
...
let generate_type evd g_to_f f graph i =
  ...
  let evd', graph =
    Evd.fresh_global (Global.env ()) !evd
      (Globnames.IndRef (fst (destInd !evd graph)))
  in
  evd := evd';
  ...
  (*i The [pre_context]  is the defined to be the context corresponding to
    ...
    i*)
  let pre_ctxt =
    LocalAssum (Name res_id, lift 1 res_type)
    :: LocalDef (Name fv_id, mkApp (f, args_as_rels), res_type)
    :: fun_ctxt
  in
  (*i and we can return the solution depending on which lemma type we are defining i*)
  if g_to_f then
    ( LocalAssum (Anonymous, graph_applied) :: pre_ctxt
    , lift 1 res_eq_f_of_args
    , graph )
  else
    ( LocalAssum (Anonymous, res_eq_f_of_args) :: pre_ctxt
...
let find_induction_principle evd f =
  ...
  | Some rect_lemma ->
    let evd', rect_lemma =
      Evd.fresh_global (Global.env ()) !evd (Globnames.ConstRef rect_lemma)
    in
    let evd', typ =
...
*)
let prove_fun_correct evd funs_constr graphs_constr schemes lemmas_types_infos
    i : Tacmach.tactic =
 fun g ->
  ...
  (* Since we cannot ensure that the functional principle is defined in the
    ...
    *)
  let principle_id =
    Namegen.next_ident_away_in_goal (Id.of_string "princ") (Id.Set.of_list ids)
  in
           ...
           (reduce
              (Genredexpr.Cbv
                 { Redops.all_flags with
                   Genredexpr.rDelta = false; Genredexpr.rConst = [] })
              Locusops.onConcl)
        ...
        | hres :: res :: decl :: ctxt ->
          let res =
            EConstr.it_mkLambda_or_LetIn
              ...
              ( LocalAssum (RelDecl.get_name decl, RelDecl.get_type decl)
              :: ctxt )
          in
          res )
      lemmas_types_infos
  ...
  tclMAP
    (function
      | LocalAssum (id, t)
        ...
        tclTHEN
          (Proofview.V82.of_tactic (Tactics.generalize [mkVar id]))
          (thin [id])
      | _ -> tclIDTAC)
    (pf_hyps g) g
...
let prove_fun_complete funcs graphs schemes lemmas_types_infos i :
 ...
 fun g ->
  ...
  let f = funcs.(i) in
  let graph_principle =
    Reductionops.nf_zeta (pf_env g) (project g) (EConstr.of_constr schemes.(i))
  in
    ...
    List.map
      (fun decl ->
        List.map
          (fun id -> id)
          (generate_fresh_id (Id.of_string "y") ids
             (nb_prod (project g) (RelDecl.get_type decl))) )
      branches
  in
  ...
  let rewrite_tac j ids : Tacmach.tactic =
    ...
    then
      let eq_lemma =
        try Option.get infos.equation_lemma with Option.IsNone ->
          anomaly (Pp.str "Cannot find equation lemma.")
      in
      tclTHENLIST
    ...
    ; observe_tac ""
        (tclTHEN_i
           (observe_tac "elim"
              (Proofview.V82.of_tactic
                 (elim false None
                    (mkVar hres, NoBindings)
                    (Some (mkVar graph_principle_id, NoBindings)))))
...
let derive_correctness make_scheme (funs : pconstant list)
    (graphs : inductive list) =
  assert (funs <> []);
  assert (graphs <> []);
          ...
          Array.of_list
            (List.map
               (fun entry ->
                 ...
                 , EConstr.of_constr
                     (Option.get entry.Entries.const_entry_type) ) )
               (make_scheme evd
                  (Array.map_to_list (fun const -> (const, Sorts.InType)) funs)))
      in
      let proving_tac =
        prove_fun_correct !evd funs_constr graphs_constr schemes
          lemmas_types_infos
      in
      Array.iteri
        (fun i f_as_constant ->
          ...
          ignore
            (Pfedit.by
               (Proofview.V82.tactic
                  (observe_tac
                     ("prove correctness (" ^ Id.to_string f_id ^ ")")
                     (proving_tac i))));
          Lemmas.save_proof
            Vernacexpr.(Proved (Proof_global.Transparent, None));
...
let revert_graph kn post_tac hid g =
  ...
  | App (i, args) when isInd sigma i ->
    ...
    if MutInd.equal kn kn' then
      ...
      let info =
        try find_Function_of_graph ind' with Not_found ->
  ...
  | App (eq, args) when EConstr.eq_constr sigma eq (make_eq ()) ->
    let pre_tac, f_args, res =
      ...
      | App (f, f_args), _ when EConstr.eq_constr sigma f fconst ->
        ( (fun hid ->
            Proofview.V82.of_tactic (intros_symmetry (Locusops.onHyp hid)) )
        , f_args
        , args.(2) )
  ...
  try
    ...
    Proofview.V82.of_tactic
      (Tactics.try_intros_until
         (fun hid ->
           Proofview.V82.tactic
             (functional_inversion kn hid (mkConst f) f_correct) )
         qhyp)
  with
           ...
           Proofview.V82.tactic (fun g ->
               ...
               | App (eq, args) when EConstr.eq_constr sigma eq (make_eq ()) -> (
                 ...
                 try
                   ...
                   let finfos =
                     find_Function_infos (fst (destConst sigma f1))
                   in
                   let f_correct =
                     mkConst (Option.get finfos.correctness_lemma)
                   and kn = fst finfos.graph_ind in
                   functional_inversion kn hid f1 f_correct g
                 with
                 | NoFunction | Option.IsNone | Not_found -> (
                   try
                     let f2, _ = decompose_app sigma args.(2) in
                     if not (isConst sigma f2) then raise NoFunction;
                     let finfos =
                       find_Function_infos (fst (destConst sigma f2))
                     in
                     let f_correct =
                       mkConst (Option.get finfos.correctness_lemma)
                     and kn = fst finfos.graph_ind in
                   ...
                   | Option.IsNone ->
                     if do_observe () then
                       error
                         "Cannot use equivalence with graph for any side of \
                          the equality"
                     else
                       user_err
                         ( str
                             "Cannot find inversion information for hypothesis "
                         ++ Ppconstr.pr_id hid )
                   | Not_found ->
                     if do_observe () then
                       ...
                       user_err
                         ...
                         ++ Ppconstr.pr_id hid ) ) )
               | _ ->
                 user_err (Ppconstr.pr_id hid ++ str " must be an equality ")
           ) )
         qhyp)
      g
