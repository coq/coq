...
let compute_new_princ_type_from_rel rel_to_fun sorts princ_type =
  ...
  let change_predicate_sort i decl =
    ...
    Context.Named.Declaration.LocalAssum
      ( Nameops.Name.get_id (Context.Rel.Declaration.get_name decl)
  ...
  let is_pte =
    ...
    fun t ->
      match Constr.kind t with Var id -> Id.Set.mem id set | _ -> false
  in
  let pre_princ =
    let open EConstr in
    it_mkProd_or_LetIn
       ...
       (List.map
          (function
            | Context.Named.Declaration.LocalAssum (id, b) ->
              LocalAssum (Name (Hashtbl.find tbl id), b)
            | Context.Named.Declaration.LocalDef (id, t, b) ->
              LocalDef (Name (Hashtbl.find tbl id), t, b))
          new_predicates))
    (List.map
...
let change_property_sort evd toSort princ princName =
  ...
  let change_sort_in_predicate decl =
    LocalAssum
      ( get_name decl
      , let args, ty =
          decompose_prod (EConstr.Unsafe.to_constr (get_type decl))
        in
        let s = destSort ty in
        Global.add_constraints
          (Univ.enforce_leq (univ_of_sort toSort) (univ_of_sort s)
...
let build_functional_principle (evd : Evd.evar_map ref) interactive_proof
    old_princ_type sorts funs i proof_tac hook =
  (* First we get the type of the old graph principle *)
  let mutr_nparams =
    (compute_elim_sig !evd (EConstr.of_constr old_princ_type)).nparams
  in
  (*   let time1 = System.get_time ()  in *)
  let new_principle_type =
    compute_new_princ_type_from_rel (Array.map mkConstU funs) sorts
      old_princ_type
  in
  ...
  Lemmas.start_proof new_princ_name
  ...
  let {id; entries; persistence} =
    fst
    @@ close_proof ~opaque:Transparent ~keep_body_ucst_separate:false (fun x ->
           x )
  in
  match entries with
  | [entry] ->
  ...
  try
    ...
    let base_new_princ_name, new_princ_name =
      ...
      | None ->
        ...
        ( id_of_f
        , Indrec.make_elimination_ident id_of_f (Sorts.family type_sort) )
    in
        ...
        let register_with_sort fam_sort =
          ...
          ignore
            (Declare.declare_constant name
               (DefinitionEntry ce, Decl_kinds.IsDefinition Decl_kinds.Scheme));
  ...
  with e when CErrors.noncritical e ->
    ( try
        ...
        if String.length s >= n then
          if String.equal (String.sub s 0 n) "___________princ_________" then
      ...
      Array.mapi
        (fun i na ->
          ...
          | Name id ->
            ...
            (const, i)
          | Anonymous -> anomaly (Pp.str "Anonymous fix.") )
        na
    | _ -> [|(const, 0)|]
  in
  function
        ...
        (fun params ->
          if
            not
              (List.equal
                 (fun (n1, c1) (n2, c2) ->
                   Name.equal n1 n2 && Constr.equal c1 c2 )
                 first_params params)
    ...
    let _check_bodies =
      try
        let extract_info is_first body =
          ...
          | _ ->
            if is_first && Int.equal (List.length l_bodies) 1 then
              raise Not_Rec
        ...
        let check body =
          ...
          if not (eq_infos first_infos (extract_info false body)) then
            user_err Pp.(str "Not a mutal recursive block")
        in
...
let make_scheme evd (fas : (pconstant * Sorts.family) list) :
    Safe_typing.private_constants definition_entry list =
  let env = Global.env () in
  let funs = List.map fst fas in
  let first_fun = List.hd funs in
  let funs_mp = KerName.modpath (Constant.canonical (fst first_fun)) in
  let first_fun_kn =
    try fst (find_Function_infos (fst first_fun)).graph_ind with Not_found ->
  ...
  let (_, (const, _)), _ =
    try
      build_functional_principle evd false first_type (Array.of_list sorts)
        this_block_funs 0
  ...
  if List.is_empty other_princ_types then [const]
  else
    ...
    let first_princ_body, first_princ_type =
      (const.const_entry_body, const.const_entry_type)
    in
    let ctxt, fix =
      decompose_lam_assum (fst (fst (Future.force first_princ_body)))
    in
    (* the principle has for forall ...., fix .*)
    let (idxs, _), ((_, ta, _) as decl) = destFix fix in
    let other_result =
      List.map
        (* we can now compute the other principles *)
          (fun scheme_type ->
          incr i;
          observe (Printer.pr_lconstr_env env sigma scheme_type);
          let type_concl = strip_prod_assum scheme_type in
          let applied_f =
            List.hd (List.rev (snd (decompose_app type_concl)))
          in
	   ...
	   *)
            let (_, (const, _)), _ =
              build_functional_principle evd false
                ...
                (prove_princ_for_struct evd false !i
...
let build_scheme fas =
  ...
  let pconstants =
    List.map
      (fun (_, f, sort) ->
        let f_as_constant =
          try Smartlocate.global_with_alias f with Not_found ->
        ...
        evd := sigma;
        let c, u =
          try EConstr.destConst !evd f with DestKO ->
            user_err
              Pp.(
                pr_econstr_env (Global.env ()) !evd f
                ++ spc ()
  ...
  List.iter2
    (fun (princ_id, _, _) def_entry ->
      ignore
        (Declare.declare_constant princ_id
...
let build_case_scheme fa =
  ...
  let first_fun_kn =
    try fst (find_Function_infos first_fun).graph_ind with Not_found ->
      raise No_graph_found
  in
