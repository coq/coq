...
let rec print_debug_queue e =
  if not (Stack.is_empty debug_queue) then
    ...
    let _ =
      ...
      | None ->
        Feedback.msg_debug
          (str " from " ++ lmsg ++ str " on goal" ++ fnl () ++ goal)
    in
...
let do_observe_tac s tac g =
  ...
  with reraise ->
    ...
    if not (Stack.is_empty debug_queue) then
      print_debug_queue
        (Some (fst (ExplainErr.process_vernac_interp_error reraise)));
    iraise reraise
...
let rec incompatible_constructor_terms sigma t1 t2 =
  ...
  and c2, arg2 = decompose_app sigma t2 in
...
let change_eq env sigma hyp_id (context : rel_context) x t end_of_type =
  ...
  let rec compute_substitution sub t1 t2 =
    ...
    else if isAppConstruct sigma t1 && isAppConstruct sigma t2 then (
...
let rewrite_until_var arg_num eq_ids : tactic =
  ...
  *)
  let test_var g =
    ...
    not
      (isConstruct sigma args.(arg_num) || isAppConstruct sigma args.(arg_num))
  in
...
let clean_hyp_with_heq ptes_infos eq_hyps hyp_id env sigma =
  ...
  let rec scan_type context type_of_hyp : tactic =
    if isLetIn sigma type_of_hyp then
      ...
      else if is_incompatible_eq sigma t_x then raise TOREMOVE
        (* t_x := C1 ... =  C2 ... *)
      else if
          ...
          tclTHENLIST
            ...
            ; observe_tac "after_introduction" (fun g' ->
                  ...
                  let new_term_value_eq =
                    pf_unsafe_type_of g' (mkVar heq_id)
                  in
                  ...
                  let new_infos =
                    { dyn_infos with
                      info = new_body; eq_hyps = heq_id :: dyn_infos.eq_hyps }
                  in
...
let my_orelse tac1 tac2 g =
  try tac1 g with e when CErrors.noncritical e ->
...
let build_proof (interactive_proof : bool) (fnames : Constant.t list)
    ptes_infos dyn_infos : tactic =
          ...
          (fun g' ->
            ...
            let do_prove new_hyps =
              build_proof do_finalize
                { new_infos with
                  rec_hyps = new_hyps; nb_rec_hyps = List.length new_hyps }
            in
            (* 			   observe_tac "Lambda" *)
  ...
  and build_proof do_finalize dyn_infos g =
    ...
    observe_tac_stream
      (str "build_proof with " ++ pr_leconstr_fp dyn_infos.info)
      (build_proof_aux do_finalize dyn_infos)
      g
  and build_proof_args do_finalize dyn_infos : tactic =
   (* f_args'  args *)
   fun g ->
...
let do_replace (evd : Evd.evar_map ref) params rec_arg_num rev_args_id f
    fun_num all_funs g =
  let equation_lemma =
    try
 ...
 fun g ->
  ...
  let full_params, princ_params, fbody_with_full_params =
    if diff_params > 0 then
      ...
      , (* the params of the principle which are not params of the function *)
        substl
          (* function instantiated with real params *)
    ...
    | _, this_fix_info :: others_infos ->
      ...
      if List.is_empty other_fix_infos then
        if this_fix_info.idx + 1 = 0 then tclIDTAC
          ...
          observe_tac_stream
            (str "h_fix " ++ int (this_fix_info.idx + 1))
            (Proofview.V82.of_tactic
  ...
  let intros_after_fixes : tactic =
   fun gl ->
    ...
    try
      let pte =
        try destVar (project gl) pte with DestKO ->
          anomaly (Pp.str "Property is not a variable.")
      in
            ...
            tclTHENLIST
              ...
              ; (let do_prove =
                 ...
                 let prove_tac branches =
                   let dyn_infos =
                     { dyn_infos with
                       rec_hyps = branches; nb_rec_hyps = List.length branches
                     }
                   in
 ...
 fun gls ->
  (tclTHENLIST
     ...
     ; (* observe_tac ("new_prove_with_tcc ( applying "^(Id.to_string hrec)^" )" ) *)
       tclTHENS
         (* We must have exactly ONE subgoal !*)
...
let prove_principle_for_gen (f_ref, functional_ref, eq_ref) tcc_lemma_ref
    is_mes rec_arg_num rec_arg_type relation gl =
  ...
  in
  let wf_tac =
    if is_mes then fun b -> Recdef.tclUSER_if_not_mes tclIDTAC b None
  ...
  let start_tac gls =
    ...
    tclTHENLIST
      ...
      ; (fun g ->
          ...
          if List.is_empty !tcc_list then (
            tcc_list := [hid];
            tclIDTAC g )
          else thin [hid] g ) ]
      gls
