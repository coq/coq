...
let open_new_goal ~lemma build_proof sigma using_lemmas ref_ goal_name (gls_type,decompose_and_tac,nb_goal)   =
  ...
  in
  try
    Some (fst @@ Lemmas.by (Proofview.V82.tactic tclIDTAC) lemma) (* raises UserError _ if the proof is complete *)
  with UserError _ ->
    defined lemma;
    None
...
let com_terminate
  ...
  let start_proof env ctx (tac_start:tactic) (tac_end:tactic) =
    let pstate = Lemmas.start_lemma thm_name
...
(*      Vernacentries.interp (Vernacexpr.VernacShow Vernacexpr.ShowScript); *)
    Flags.silently (fun () -> Lemmas.save_lemma_proved ?proof:None ~lemma ~opaque:opacity ~idopt:None) ()
