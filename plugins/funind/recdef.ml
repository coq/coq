...
let compute_renamed_type gls c =
  rename_bound_vars_as_displayed (project gls) (*no avoid*) Id.Set.empty
    (*no rels*) [] (pf_unsafe_type_of gls c)
...
let le = function
  | () ->
    Coqlib.gen_reference_in_modules "RecursiveDefinition" Coqlib.init_modules
      "le"
...
let iter_ref () =
  try find_reference ["Recdef"] "iter" with Not_found ->
    user_err Pp.(str "module Recdef not loaded")
...
let max_constr = function
  | () -> EConstr.of_constr (constr_of_global (delayed_force max_ref))
...
let (declare_f :
      Id.t -> logical_kind -> Constr.t list -> GlobRef.t -> GlobRef.t) =
...
let do_observe_tac s tac g =
  ...
  with reraise ->
    ...
    if not (Stack.is_empty debug_queue) then
      print_debug_queue true
        (fst (ExplainErr.process_vernac_interp_error reraise));
    iraise reraise
...
let observe_tclTHENLIST s tacl =
  if do_observe () then
    let rec aux n = function
      ...
      | tac :: tacl ->
        observe_tac (s ++ spc () ++ int n) (tclTHEN tac (aux (succ n) tacl))
    in
    aux 0 tacl
...
let tclUSER tac is_mes l g =
  let clear_tac =
    ...
    | Some l ->
      tclMAP
        (fun id -> tclTRY (Proofview.V82.of_tactic (clear [id])))
        (List.rev l)
  in
  observe_tclTHENLIST (str "tclUSER1")
    [ clear_tac
    ; ( if is_mes then
        observe_tclTHENLIST (str "tclUSER2")
...
let tclUSER_if_not_mes concl_tac is_mes names_to_suppress =
  if is_mes then
    tclCOMPLETE (fun gl ->
        Proofview.V82.of_tactic
          (Simple.apply (delayed_force well_founded_ltof))
          gl )
  else
...
let check_not_nested sigma forbidden e =
  ...
  in
  try check_not_nested e with UserError (_, p) ->
...
let treat_case forbid_new_ids to_intros finalize_tac nb_lam e infos : tactic =
 fun g ->
  ...
  let ids =
    List.fold_left
      (fun acc (na, _) ->
        let pre_id = match na with Name x -> x | Anonymous -> ano_id in
        pre_id :: acc )
      [] rev_context
  in
    ...
    ; onLastHypId (fun heq ->
          observe_tclTHENLIST (str "treat_case2")
            ...
            ; (fun g' ->
                ...
                let teq_lhs, teq_rhs =
                  let _, args =
                    try destApp (project g') ty_teq with DestKO -> assert false
                  in
                  (args.(1), args.(2))
                in
                let new_b' =
  ...
  | Prod _ -> (
    ...
    with e when CErrors.noncritical e ->
      user_err ~hdr:"Recdef.travel"
        ( str "the term "
        ++ Printer.pr_leconstr_env (pf_env g) sigma expr_info.info
...
let rec prove_lt hyple g =
  ...
  ( try
      ...
      let y =
        List.hd
          (List.tl (snd (decompose_app sigma (pf_unsafe_type_of g (mkVar h)))))
      in
    ...
    with Not_found ->
      observe_tclTHENLIST (str "prove_lt2")
        ...
        ; observe_tac
            (str "assumption: " ++ Printer.pr_goal g)
               ...
               Proofview.V82.tactic
                 (observe_tac
                    ...
                    (tclTHENS
                       ...
                       ; observe_tclTHENLIST
                           (str "destruct_bounds_aux2")
                           [ observe_tac (str "clearing k ")
                               (Proofview.V82.of_tactic (clear [id]))
                           ; h_intros [k; h'; def]
                           ; observe_tac (str "simple_iter")
                               (Proofview.V82.of_tactic
                               ...
                               ; (* list_rewrite true *)
                                 ...
                                 observe_tac (str "finishing")
                                   (tclORELSE
                                      (Proofview.V82.of_tactic
                                         intros_reflexivity)
                                      (observe_tac (str "calling prove_lt")
                                         (prove_lt hyple))) ] ] ])) )) ]
      g
  | (_, v_bound) :: l ->
    observe_tclTHENLIST
      (str "destruct_bounds_aux3")
...
let mkDestructEq : Id.t list -> constr -> goal Evd.sigma -> tactic * Id.t list
    =
  ...
  observe_tac
    ...
    | ( UserError (Some "Refiner.thensn_tac3", _)
     |UserError (Some "Refiner.tclFAIL_s", _) ) ->
      observe_tac
        ( str "is computable "
        ++ Printer.pr_leconstr_env (pf_env g) sigma new_info.info )
        (next_step continuation_tac
           { new_info with
             info = Reductionops.nf_betaiotazeta (pf_env g) sigma new_info.info
           }) )
    g
...
let terminate_app_rec (f, args) expr_info continuation_tac _ g =
  let sigma = project g in
  List.iter
    (check_not_nested sigma (expr_info.f_id :: expr_info.forbidden_ids))
    args;
  try
    let v =
      List.assoc_f
    ...
    observe_tclTHENLIST (str "terminate_app_rec")
      ...
      ; ( if expr_info.is_final && expr_info.is_main_branch then
          observe_tclTHENLIST (str "terminate_app_rec1")
            ...
            ; observe_tac
                (str "destruct_bounds (3)")
                (destruct_bounds new_infos) ]
        else tclIDTAC ) ]
      g
  with Not_found ->
    observe_tac
      (str "terminate_app_rec not found")
      (tclTHENS
         (Proofview.V82.of_tactic
...
let rec make_rewrite_list expr_info max = function
  | [] -> tclIDTAC
  | (_, p, hp) :: l ->
    observe_tac (str "make_rewrite_list")
      (tclTHENS
         (observe_tac
            (str "rewrite heq on " ++ Id.print p)
            (fun g ->
              let sigma = project g in
         ...
         ; observe_tclTHENLIST (str "make_rewrite_list")
             [ (* x < S max proof *)
  ...
  tclTHENFIRST
    ...
    (observe_tac (str "make_rewrite")
       (tclTHENS
          ...
          ; observe_tclTHENLIST (str "make_rewrite1")
              [ (* x < S (S max) proof *)
                Proofview.V82.of_tactic
...
let equation_others _ expr_info continuation_tac infos =
  if expr_info.is_final && expr_info.is_main_branch then
    ...
    observe_tac
      (str "equation_others (cont_tac) " ++ pr_leconstr_rd expr_info.info)
      (continuation_tac infos)

let equation_app f_and_args expr_info continuation_tac infos =
  if expr_info.is_final && expr_info.is_main_branch then
    observe_tac
      (str "intros_values_eq equation_app")
      (intros_values_eq expr_info [])
  else continuation_tac infos
  ...
  with Not_found ->
    if expr_info.is_final && expr_info.is_main_branch then
      ...
      observe_tclTHENLIST (str "equation_app_rec1")
        [ Proofview.V82.of_tactic
            (simplest_case (mkApp (expr_info.f_terminate, Array.of_list args)))
        ; observe_tac (str "app_rec not_found")
            (continuation_tac
               { expr_info with
                 args_assoc =
...
let compute_terminate_type nb_args func =
  ...
  let left =
    mkApp
      ...
      , Array.of_list
          ( lift 5 a_arrow_b :: mkRel 3 :: constr_of_global func :: mkRel 1
    ...
    mkApp
      ...
      , [| delayed_force nat
         ; mkLambda
             ( Name p_id
             , delayed_force nat
             , mkProd (Name k_id, delayed_force nat, mkArrow cond result) )
        |] )
  in
  let value =
    mkApp
      ( constr_of_global (Util.delayed_force coq_sig_ref)
      , [|b; mkLambda (Name v_id, b, nb_iter)|] )
  in
...
let whole_start (concl_tac : tactic) nb_args is_mes func input_type relation
    rec_arg_num : tactic =
 fun g ->
  let sigma = project g in
...
let build_and_l sigma l =
  ...
  let rec f = function
    ...
    | p1 :: pl ->
      ...
      , tclTHENS
          (Proofview.V82.of_tactic
             (apply (EConstr.of_constr (constr_of_global conj_constr))))
          [tclIDTAC; tac]
      , nb + 1 )
  in
  f l
...
let open_new_goal build_proof sigma using_lemmas ref_ goal_name
  ...
  in
  Lemmas.start_proof na
    (Decl_kinds.Global, false (* FIXME *), Decl_kinds.Proof Decl_kinds.Lemma)
    sigma gls_type ~hook:(Lemmas.mk_hook hook);
         ...
         (Proofview.V82.tactic (fun g ->
              tclTHEN decompose_and_tac
                (tclORELSE
                         ...
                         (fun c ->
                           Proofview.V82.of_tactic
                             (Tacticals.New.tclTHENLIST
                                ...
                                ; Simple.apply
                                    (fst
                                       (interp_constr (Global.env ()) Evd.empty
                                          c))
                                  (*FIXME*)
                   ...
                   tclIDTAC)
                g )));
  try ignore (by (Proofview.V82.tactic tclIDTAC))
  with (* raises UserError _ if the proof is complete *)
  | UserError _ -> defined ()

let com_terminate tcc_lemma_name tcc_lemma_ref is_mes fonctional_ref input_type
    relation rec_arg_num thm_name using_lemmas nb_args ctx hook =
  let start_proof ctx (tac_start : tactic) (tac_end : tactic) =
    let evd, env = Pfedit.get_current_context () in
    Lemmas.start_proof thm_name
      (Global, false (* FIXME *), Proof Lemma)
  ...
  try
    ...
    open_new_goal start_proof sigma using_lemmas tcc_lemma_ref
...
let start_equation (f : GlobRef.t) (term_f : GlobRef.t)
  ...
  observe_tac (str "start_equation")
    (observe_tclTHENLIST (str "start_equation")
       [ h_intros x
       ; Proofview.V82.of_tactic
           (unfold_in_concl
       ...
       ; observe_tac (str "prove_eq") (cont_tactic x) ])
    g

let (com_eqn :
      int -> Id.t -> GlobRef.t -> GlobRef.t -> GlobRef.t -> Constr.t -> unit) =
 fun nb_arg eq_name functional_ref f_ref terminate_ref equation_lemma_type ->
  ...
  Lemmas.start_proof eq_name
    (Global, false, Proof Lemma)
    ~sign:(Environ.named_context_val env)
    evd
       ...
       (Proofview.V82.tactic
          (start_equation f_ref terminate_ref (fun x ->
               prove_eq
                 ...
                 { nb_arg
                 ; f_terminate =
                     EConstr.of_constr (constr_of_global terminate_ref)
  ...
  Flags.silently
    (fun () -> Lemmas.save_proof (Vernacexpr.Proved (opacity, None)))
    ()

(*      Pp.msgnl (str "eqn finished"); *)

let recursive_definition is_mes function_name rec_impls type_of_f r rec_arg_num
    eq generate_induction_principle using_lemmas : unit =
  let open Term in
  let open Constr in
  let open CVars in
  let env = Global.env () in
  ...
  let env =
    EConstr.push_named
      (Context.Named.Declaration.LocalAssum (function_name, function_type))
      env
  in
  ...
  let hook _ _ =
    ...
    let stop =
      try
        com_eqn (List.length res_vars) equation_id functional_ref f_ref
          term_ref
    ...
    if not stop then (
      ...
      Flags.if_verbose msgnl
        ( h 1 (Ppconstr.pr_id function_name ++ spc () ++ str "is defined")
        ++ fnl ()
        ++ h 1 (Ppconstr.pr_id equation_id ++ spc () ++ str "is defined") ) )
  in
  (* XXX STATE Why do we need this... why is the toplevel protection not enought *)
  funind_purify
    (fun () ->
      com_terminate tcc_lemma_name tcc_lemma_constr is_mes functional_ref
        (EConstr.of_constr rec_arg_type)
