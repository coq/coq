...
let locate_ind ref =
  match locate ref with IndRef x -> x | _ -> raise Not_found

let locate_constant ref =
  match locate ref with ConstRef x -> x | _ -> raise Not_found

let locate_with_msg msg f x =
  try f x with Not_found -> raise (CErrors.UserError (None, msg))
  ...
  let rec chop_lambda_n acc n rt =
    if n == 0 then (List.rev acc, rt)
      ...
      | _ ->
        raise
          (CErrors.UserError
             (Some "chop_rlambda_n", str "chop_rlambda_n: Not enough Lambdas"))
  in
  chop_lambda_n []
  ...
  let rec chop_prod_n acc n rt =
    if n == 0 then (List.rev acc, rt)
      ...
      | _ ->
        raise
          (CErrors.UserError
             (Some "chop_rprod_n", str "chop_rprod_n: Not enough products"))
  in
  chop_prod_n []
...
let const_of_id id =
  ...
  try Constrintern.locate_reference princ_ref with Not_found ->
         ...
         Printer.pr_lconstr_env env sigma
           (fst
              (Typeops.type_of_global_in_context env
                 (ConstRef f_info.function_constant)))
       with e when CErrors.noncritical e -> mt () )
  ++ fnl () ++ str "equation_lemma := "
  ++ pr_ocst f_info.equation_lemma
  ++ fnl ()
  ++ str "completeness_lemma :="
  ++ pr_ocst f_info.completeness_lemma
  ++ fnl ()
  ++ str "correctness_lemma := "
  ++ pr_ocst f_info.correctness_lemma
  ++ fnl () ++ str "rect_lemma := " ++ pr_ocst f_info.rect_lemma ++ fnl ()
...
let find_Function_infos f = Cmap_env.find f !from_function
let find_Function_of_graph ind = Indmap.find ind !from_graph
...
let ltof_ref = function
  | () -> find_reference ["Coq"; "Arith"; "Wf_nat"] "ltof"
