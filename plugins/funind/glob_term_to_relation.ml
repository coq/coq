...
let build_constructors_of_type ind' argl =
  ...
  Array.mapi
    (fun i _ ->
      ...
      in
      let cst_narg =
        Inductiveops.constructor_nallargs_env (Global.env ()) construct
      in
      let argl =
        if List.is_empty argl then
          Array.to_list (Array.init (cst_narg - npar) (fun _ -> mkGHole ()))
        else argl
      in
      let pat_as_term = mkGApp (mkGRef (ConstructRef (ind', i + 1)), argl) in
...
let raw_push_named (na, raw_value, raw_typ) env =
  ...
  | Name id -> (
    ...
    | None -> EConstr.push_named (NamedDecl.LocalAssum (id, typ)) env
    | Some value ->
...
let add_pat_variables pat typ env : Environ.env =
  let rec add_pat_variables env pat typ : Environ.env =
    observe
      ( str "new rel env := "
      ++ Printer.pr_rel_context_of env (Evd.from_env env) );
    ...
    fst
      (Context.Rel.fold_outside
         (fun decl (env, ctxt) ->
           ...
           match decl with
           | LocalAssum (Anonymous, _) | LocalDef (Anonymous, _, _) ->
             assert false
           ...
           | LocalDef (Name id, v, t) ->
             ...
             observe
               ...
               ++ fnl () );
             let open Context.Named.Declaration in
             ( Environ.push_named (LocalDef (id, new_v, new_t)) env
             , mkVar id :: ctxt ) )
  ...
  observe
    ( str "new var env := "
    ++ Printer.pr_named_context_of res (Evd.from_env env) );
  res
...
let rec pattern_to_term_and_type env typ =
  DAst.with_val (function
    ...
    | PatCstr (constr, patternl, _) ->
      let cst_narg =
        Inductiveops.constructor_nallargs_env (Global.env ()) constr
      in
      let (Inductiveops.IndType (indf, indargs)) =
        try
          Inductiveops.find_rectype env (Evd.from_env env)
      ...
      in
      mkGApp (mkGRef (ConstructRef constr), implicit_args @ patl_as_term) )
...
let rec build_entry_lc env funnames avoid rt : glob_constr build_entry_return =
  ...
  | GApp (_, _) -> (
    ...
    | GLetIn (n, v, t, b) ->
      ...
      let new_n, new_b, new_avoid =
        ...
        | _ -> (n, b, avoid)
      in
      build_entry_lc env funnames avoid
  ...
  | GLetIn (n, v, typ, b) ->
	...
	*)
    ...
    let new_env =
      ...
      | Anonymous -> env
      | Name id ->
        EConstr.push_named (NamedDecl.LocalDef (id, v_as_constr, v_type)) env
    in
    let b_res = build_entry_lc new_env funnames avoid b in
  ...
  | GLetTuple (nal, _, b, e) ->
    ...
    build_entry_lc env funnames avoid match_expr
  | GRec _ -> user_err Pp.(str "Not handled GRec")
  | GCast (b, _) -> build_entry_lc env funnames avoid b

and build_entry_lc_from_case env funname make_discr (el : tomatch_tuples)
	...
	*)
    let case_resl =
      List.fold_right
        (fun (case_arg, _) ctxt_argsl ->
          let arg_res =
            build_entry_lc env funname ctxt_argsl.to_avoid case_arg
          in
          combine_results combine_args arg_res ctxt_argsl )
        el (mk_result [] [] avoid)
    in
    ...
    let results =
      List.map
        (fun ca ->
          let res =
            build_entry_lc_from_case_term env types funname make_discr [] brl
              case_resl.to_avoid ca
          in
          res )
        case_resl.result
    in
...
and build_entry_lc_from_case_term env types funname make_discr
  ...
  | br :: brl' ->
	...
	*)
    let brl'_res =
      build_entry_lc_from_case_term env types funname make_discr
	...
	*)
    ...
    let this_branch_res =
      List.map
        (fun res ->
          { context =
              matched_expr.context @ those_pattern_preconds @ res.context
          ; value = res.value } )
        return_res.result
    in
...
let rec rebuild_cons env nb_args relname args crossed_types depth rt =
  ...
  | GProd (n, k, t, b) -> (
    ...
    | GApp (eq_as_ref, [ty; id; rt])
      ...
      try
        observe
          (str "computing new type for eq : " ++ pr_glob_constr_env env rt);
        let t' =
      ...
      with Continue ->
        let jmeq =
          Globnames.IndRef (fst (EConstr.destInd Evd.empty (jmeq ())))
        in
        ...
        let rt_typ =
          DAst.make
          @@ GApp
               ( DAst.make @@ GRef (Globnames.IndRef (fst ind), None)
               , List.map
                   (fun p ->
                     Detyping.detype Detyping.Now false Id.Set.empty env
  ...
  | GLambda (n, k, t, b) -> (
    ...
    let new_crossed_types = t :: crossed_types in
    observe
      (str "computing new type for lambda : " ++ pr_glob_constr_env env rt);
    let t', ctx = Pretyping.understand env (Evd.from_env env) t in
    match n with
    | Name id ->
      let new_env = EConstr.push_rel (LocalAssum (n, t')) env in
  ...
  | GLetIn (n, v, t, b) -> (
    ...
    let new_b, id_to_exclude =
      rebuild_cons new_env nb_args relname args (t :: crossed_types)
        (depth + 1) b
    in
    match n with
    | Name id when Id.Set.mem id id_to_exclude && depth >= nb_args ->
  ...
  let evd, env =
    Array.fold_right2
      (fun id (c, u) (evd, env) ->
        ...
        let t = EConstr.Unsafe.to_constr t in
        (evd, Environ.push_named (LocalAssum (id, t)) env) )
      funnames
    ...
    *)
    ...
    Util.Array.fold_left2
      (fun env rel_name rel_ar ->
        ...
        Environ.push_named (LocalAssum (rel_name, rex)) env )
    ...
    with_full_print
      (Flags.silently
         (ComInductive.do_mutual_inductive ~template:(Some false) None rel_inds
            false false false ~uniform:ComInductive.NonUniformParameters))
      Declarations.Finite
  with
