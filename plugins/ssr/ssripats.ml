...
let print_name_seed env = function
  | None -> Pp.str "-"
...
let isPRINT g =
  ...
  Pp.(str"{{ to_clear: " ++
      ...
      str"name_seed: " ++ print_name_seed env state.name_seed ++ str" }}")
...
let intro_anon_all = Goal.enter begin fun sigma gl ->
...
let intro_anon_deps = Goal.enter begin fun sigma gl ->
  let env = Goal.env gl in
  let g = Goal.concl gl in
...
let tacCHECK_HYPS_EXIST hyps = Goal.enter begin fun _ gl ->
  let ctx = Goal.hyps gl in
  List.iter (Ssrcommon.check_hyp_exists ctx) hyps;
  tclUNIT ()
end

let tacFILTER_HYP_EXIST hyps k = Goal.enter begin fun _ gl ->
  let ctx = Goal.hyps gl in
...
let tclTHENin t1 t2 =
  ...
  t1 <*> numgoals >>= fun n ->
  Goal.enter begin fun _ g -> incr i; t2 !i n end
end
...
(** [=> [^ seed ]] *********************************************************)
let tac_intro_seed interp_ipats fix = Goal.enter begin fun _ gl ->
...
let tclMK_ABSTRACT_VAR id = Goal.enter begin fun _ gl ->
...
let tclLOG p t =
  ...
  Goal.enter begin fun sigma g ->
    Ssrprinters.ppdebug (lazy Pp.(str" on state:" ++ spc () ++
      isPRINT g ++
      str" goal:" ++ spc () ++ Printer.pr_goal {Evd.it=Goal.goal g; sigma}));
    tclUNIT ()
  end
  <*>
    t p
...
let with_defective maintac deps clr = Goal.enter begin fun sigma g ->
...
let elim_intro_tac ipats ?seed what eqid ssrelim is_rec clr =
  let intro_eq =
    ...
    | Some (IPatId ipat) when not is_rec ->
       let rec intro_eq () = Goal.enter begin fun sigma g ->
         let env, concl = Goal.(env g, concl g) in
         match EConstr.kind_of_type sigma concl with
       ...
       intro_eq ()
    | Some (IPatId ipat) ->
       let intro_lhs = Goal.enter begin fun sigma g ->
       ...
       let rec gen_eq_tac () = Goal.enter begin fun sigma g ->
         let env, concl = Goal.(env g, concl g) in
         let sigma, eq =
...
let tclLAST_GEN ~to_ind ((oclr, occ), t) conclusion = tclINDEPENDENTL begin
  ...
  Goal.enter_one begin fun sigma g ->
  let pat = Ssrmatching.interp_cpattern sigma0 t None in
  let cl0, env, hyps = Goal.(concl g, env g, hyps g) in
...
(** [move] **************************************************************)
let pushmoveeqtac cl c = Goal.enter begin fun sigma g ->
  let env = Goal.env g in
...
let ssrsmovetac = Goal.enter begin fun sigma g ->
  let concl = Goal.concl g in
...
let ssrabstract dgens =
  let main _ (_,cid) = Goal.enter begin fun _ g ->
    Ssrcommon.tacMK_SSR_CONST "abstract" >>= fun abstract ->
    ...
    let tacFIND_HOLE = Goal.enter_one ~__LOC__ begin fun sigma g ->
      let env, concl = Goal.(env g, concl g) in
      let t = args_id.(0) in
      match EConstr.kind sigma t with
  ...
  let interp_gens { gens } ~conclusion = Goal.enter begin fun _ g ->
