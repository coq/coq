...
let unfoldintac occ rdx t (kt,_) gl = 
  ...
  let concl = 
    ...
    with Option.IsNone -> errorstrm Pp.(str"Failed to unfold " ++ pr_econstr_pat env0 sigma0 t) in
  let _ = conclude () in
  ...
  let cvtac' _ =
    ...
    | PRtype_error ->
      ...
      then errorstrm Pp.(str "Rewriting impacts evars")
        ...
        ++ pr_econstr_env (pf_env gl) (project gl) (EConstr.mkNamedLambda pattern_id rdxt cl))
  in
