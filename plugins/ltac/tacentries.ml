...
let load_tactic_notation i (tobj, body) =
  CDebug.debug_synterp (fun () -> Pp.(str"load_tactic_notation " ++ (KerName.debug_print tobj.tacobj_key)));
  let key = tobj.tacobj_key in
  let () = check_key key in
...
let subst_tactic_notation (subst, (tobj, body)) =
  CDebug.debug_synterp (fun () -> Pp.(str"subst_tactic_notation " ++ str(KerName.debug_to_string tobj.tacobj_key) ++ str " -> " ++ str (KerName.debug_to_string (Mod_subst.subst_kn subst tobj.tacobj_key))));
  let open Tacenv in
  { tobj with
...
let cache_tactic_syntax tobj =
  let key = tobj.tacobj_key in
  CDebug.debug_synterp (fun () -> Pp.(str"cache_tactic_syntax " ++ str (KerName.debug_to_string key)));
...
let open_tactic_syntax i tobj =
  let key = tobj.tacobj_key in
  CDebug.debug_synterp (fun () -> Pp.(str"open_tactic_syntax " ++ str (KerName.debug_to_string key)));
  if Int.equal i 1 && not tobj.tacobj_local then
    extend_tactic_grammar key tobj.tacobj_forml tobj.tacobj_tacgram

let load_tactic_syntax i tobj =
  let key = tobj.tacobj_key in
  CDebug.debug_synterp (fun () -> Pp.(str"load_tactic_syntax " ++ str (KerName.debug_to_string key)));
  (* Only add the printing and interpretation rules. *)
  Pptactic.declare_notation_tactic_pprule key (pprule tobj.tacobj_tacgram);
  if Int.equal i 1 && not tobj.tacobj_local then
    extend_tactic_grammar key tobj.tacobj_forml tobj.tacobj_tacgram
...
let subst_tactic_syntax (subst, tobj) =
  CDebug.debug_synterp (fun () -> Pp.(str"subst_tactic_syntax " ++ str(KerName.debug_to_string tobj.tacobj_key) ++ str " -> " ++ str (KerName.debug_to_string (Mod_subst.subst_kn subst tobj.tacobj_key))));
  { tobj with
    tacobj_key = Mod_subst.subst_kn subst tobj.tacobj_key
  }
