...
let evars_for_FO ~hack env sigma0 (ise0:evar_map) c0 =
  ...
  | Evar (k, a, che) ->
    ...
    put (existential_value0 !sigma ev)
    end
  | _ -> map put c in
...
let ungen_upat lhs (sigma, uc, t) u =
  ...
  | Const _ -> KpatConst
  | Evar (k, _, _) -> if is_defined sigma k then raise NoMatch else KpatEvar k
  | LetIn _ -> KpatLet
  | Lambda _ -> KpatLam
  | _ -> KpatRigid in
...
let isEvar_k k f =
  match kind f with Evar (k', _, _) -> k = k' | _ -> false
...
let interp_pattern ?wit_ssrpatternarg env sigma0 red redty =
  ...
  let cleanup_XinE h x rp sigma =
    let h_k = match kind h with Evar (k,_, _) -> k | _ -> assert false in
...
let eval_pattern ?raise_NoMatch env0 sigma0 concl0 pattern occ do_subst =
  ...
  let pop_evar sigma e p =
    ...
    sigma, e_body in
  let ex_value hole =
    match kind hole with Evar (e,_,_) -> e | _ -> assert false in
