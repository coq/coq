...
let unif_end ?(solve_TC=true) ~fresh env ise0 pt ok =
...
let unify_HO env sigma0 t1 t2 =
  ...
  let _, sigma, _ = unif_end ~solve_TC:false ~fresh env sigma t2 (fun _ -> true) in
  sigma
...
let ungen_upat lhs (c, sigma, t) u =
  ...
  | Lambda _ -> KpatLam
  | _ -> KpatRigid in
  c, sigma, {u with up_k = k; up_FO = lhs; up_f = f; up_a = a; up_t = t}
...
let match_upats_FO upats env ise orig_c =
  let dont_impact_evars = dont_impact_evars_in ise orig_c in
  let rec loop c =
      ...
      let one_match (u, np) =
        let open EConstr in
         ...
         if skip || not (EConstr.Vars.closed0 ise c') then () else try
           ...
           let pt' = unif_end ~fresh env ise' u.up_t (u.up_ok lhs) in
...
let match_upats_HO ~on_instance upats env ise c =
 let dont_impact_evars = dont_impact_evars_in ise c in
 ...
 let rec aux upats env ise c =
  ...
  iter_constr_LR ise (aux upats env ise) f;
...
type find_P =
  ...
  k:subst ->
     EConstr.t
type conclude = unit ->
  EConstr.t * ssrdir * (bool * Evd.evar_map * EConstr.t)
...
((fun () ->
  let env, (c, sigma, ({up_f = pf; up_a = pa} as u)) =
...
let eval_pattern ?raise_NoMatch ~rigid env0 concl0 pattern occ do_subst =
  ...
  let pop_evar sigma e p =
    ...
    | _ -> errorstrm (str "Matching the pattern " ++ pr_econstr_env env0 sigma p ++
  ...
  | Some (sigma, (X_In_T (hole, p) | In_X_In_T (hole, p))) ->
    ...
    let _ = end_X () in let _, _, (_, sigma, _) = end_T () in
    let us = Evd.evar_universe_context sigma in
    concl, us
  ...
  | Some (sigma, E_As_X_In_T (e, hole, p)) ->
    ...
    let _ = end_X () in let _, _ , (_, sigma, _) = end_TE () in
    let us = Evd.evar_universe_context sigma in
    concl, us
...
let fill_occ_pattern ?raise_NoMatch env sigma cl pat occ h =
  ...
  let cl, us =
    eval_pattern ?raise_NoMatch ~rigid env cl (Some pat) occ find_R in
...
let eval_pattern ?raise_NoMatch ~rigid env0 concl0 pattern occ do_subst =
  fst (eval_pattern ?raise_NoMatch ~rigid env0 concl0 pattern occ do_subst)
