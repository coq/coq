...
(*   let unevd = undefined_evars evd in *)
  let unevd' = Typeclasses.resolve_typeclasses ~with_goals:false ~split:true ~fail:true env evd in
  let unevd' = Typeclasses.resolve_typeclasses ~with_goals:true ~split:true ~fail:false env unevd' in
  let evm =  unevd' in
    isevars := unevd';
