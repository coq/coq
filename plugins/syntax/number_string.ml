...
let q_sum () = Rocqlib.lib_ref "core.sum.type"
...
let is_to_target env sigma f cty {kind; typ} =
  ...
  let leftsum x = DAst.make @@ GApp (gref (q_sum ()), [x; DAst.make @@ GHole GInternalHole]) in
  if has_type env sigma f (arrow typ cty) then
    Some (kind, Direct)
  else if has_type env sigma f (arrow typ (opt cty)) then
    Some (kind, Option)
  else if has_type env sigma f (arrow typ (leftsum cty)) then
    Some (kind, Error)
  else None
...
let is_from_target env sigma g cty {kind; typ} =
  ...
  if has_type env sigma g (arrow cty typ) then
    ...
    Some (kind, Option)
  else if has_type env sigma g (arrow cty (leftsum typ)) then
    Some (kind, Error)
  else None
...
let type_error_to f ty =
  ...
  CErrors.user_err
    (pr_qualid f ++ str " should go from Number.int to " ++
     ppty ++ str " or (option " ++ ppty ++ str "), or (sum " ++ ppty ++ str " _)." ++
...
let type_error_of g ty =
  CErrors.user_err
    (pr_qualid g ++ str " should go from " ++ pr_qualid ty ++
     str " to Number.int or (option Number.int), or (sum Number.int _)." ++ fnl () ++
...
let type_error_of g ty =
  CErrors.user_err
    (pr_qualid g ++ str " should go from " ++ pr_qualid ty ++
     strbrk " to T or (option T) or (sum T _), where T is either Byte.byte, (list Byte.byte), or PrimString.string.")
