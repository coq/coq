...
let q_sum () = Rocqlib.lib_ref_opt "core.sum.type"
...
let is_to_target env sigma f cty {kind; typ} =
  ...
  if has_type env sigma f (arrow typ cty) then
    ...
    Some (kind, Option)
  else if q_sum() >>? fun q_sum -> has_type env sigma f (arrow typ (app2 q_sum cty ghole)) then
    Some (kind, Error)
  else None
...
let is_from_target env sigma g cty {kind; typ} =
  ...
  if has_type env sigma g (arrow cty typ) then
    ...
    Some (kind, Option)
  else if q_sum() >>? fun q_sum -> has_type env sigma g (arrow cty (app2 q_sum typ ghole)) then
    Some (kind, Error)
  else None
