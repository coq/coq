...
let elaborate_to_post_via env sigma ty_name ty_ind l =
  ...
  let l =
    ...
    List.map read l in
  let eq_indc indc (_, _, _, _, indc', _, _) = GlobRef.CanOrd.equal indc indc' in
  ...
  let _ = (* check_for duplicate constructor and error *)
    List.fold_left (fun already_seen (_, cnst, _, loc, indc, _, _) ->
        try
          let cnst' = List.assoc_f GlobRef.CanOrd.equal indc already_seen in
...
let locate_global_inductive_or_int63_or_float allow_params qid =
  ...
  with Not_found ->
    ...
    if allow_params && Coqlib.has_ref int63n
       && GlobRef.CanOrd.equal (Smartlocate.global_with_alias qid) (Coqlib.lib_ref int63n)
    then TargetPrim (mkRefC (qualid_of_string int63w), [Coqlib.lib_ref int63c],
       ...
       && GlobRef.CanOrd.equal (Smartlocate.global_with_alias qid) (Coqlib.lib_ref floatn)
