...
| GTacCse _ | GTacOpn _ | GTacWth _ | GTacMut _ -> false
...
| CTacMut (qid, e) ->
  let not_mutable () =
    let loc, qid = match qid with
    | RelId qid -> qid.CAst.loc, pr_qualid qid
    | AbsKn _ -> loc, str "Reference"
    in
    user_err ?loc (qid ++ str " is not a mutable global definition")
  in
  let kn, sch = match get_variable env qid with
  | ArgVar _ | ArgArg (TacAlias _) -> not_mutable ()
  | ArgArg (TacConstant kn) ->
    let { Tac2env.gdata_type = sch; gdata_mutable = mut } =
      try Tac2env.interp_global kn
      with Not_found ->
        CErrors.anomaly (str "Missing hardwired primitive " ++ KerName.print kn)
    in
    let () = if not mut then not_mutable () in
    (kn, sch)
  in
  let (e, t) = intern_rec env e in
  let () = match sch with
  | (0, _) ->
    let u = fresh_type_scheme env sch in
    unify ?loc env t u
  | _ ->
    (* TODO: implement second-order subtyping *)
    user_err ?loc (str "Runtime replacement of the mutable constant " ++
      pr_qualid (Tac2env.shortest_qualid_of_ltac (TacConstant kn)) ++
      str " is not supported because it is polymorphic.")
  in
  (GTacMut (kn, e), GTypRef (Tuple 0, []))
| CTacExt (tag, arg) ->
...
| CTacMut (ref, e) ->
  let mem id = Id.Set.mem id ids in
  let ref = match get_variable0 mem ref with
  | ArgVar _ -> ref
  | ArgArg kn -> AbsKn kn
  in
  let e = globalize ids e in
  CAst.make ?loc @@ CTacMut (ref, e)
| CTacExt (tag, arg) ->
...
| GTacMut (kn, e) as e0 ->
  let kn' = subst_kn subst kn in
  let e' = subst_expr subst e in
  if kn' == kn && e' == e then e0 else GTacMut (kn', e')
| GTacExt (tag, arg) ->
...
| CTacMut (ref, e) ->
  let ref' = subst_tacref subst ref in
  let e' = subst_rawexpr subst e in
  if ref' == ref && e' == e then t else CAst.make ?loc @@ CTacMut (ref', e')
