...
let resolve_all_evars depth unique env p oevd fail =
  ...
  let rec docomp (failed:exn option) evd = function
    | [] ->
      let () = ppdebug 2 (fun () ->
          str"Final evar map: " ++
      ...
      try
        (try
          ...
          | Some (goals, nongoals) ->
            ...
            let res = solver.solver env evd ~goals:sorted_goals ~best_effort:true ~depth ~unique in
            begin match res with
            | Inl (finished, evd) ->
              let finished, evd = Search.post_process_goals ~goals ~nongoals ~old_metas ~sigma:evd ~finished in
            ...
            | Inr e -> (* Typeclass resolution failed *)
