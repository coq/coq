...
let hintmap_of env sigma ?allowed_evars hdc secvars concl =
  match hdc with
  | None -> fun db -> ModeMatch (Hint_db.map_none ~secvars db)
  | Some hdc ->
     fun db ->
       if Hint_db.use_dn db then (* Using dnet *)
         Hint_db.map_eauto env sigma ~secvars ?allowed_evars hdc concl db
...
and e_my_find_search db_list local_db secvars hdc complete only_classes env sigma concl =
  ...
  in
  let hint_of_db = hintmap_of env sigma hdc ~allowed_evars secvars concl in
