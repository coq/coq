...
let cutSubstInHyp l2r eqn id =
  ...
  tclTHEN (Proofview.Unsafe.tclEVARS sigma)
    (tclTHENFIRST
    (tclTHENLIST [
       (change_in_hyp ~cast:false ~check:true None (make_change_arg typ) (id,InHypTypeOnly));
       (replace_core (onHyp id) l2r eqn)
    ])
    (change_in_hyp ~cast:false ~check:true None (make_change_arg expected) (id,InHypTypeOnly)))
  end
