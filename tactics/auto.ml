...
let check_typeable_in_env env sigma c =
  let nctx = Environ.named_context env in
  let rec aux c =
    match kind sigma c with
    | Term.Var id -> ignore(Context.Named.lookup id nctx)
    | _ -> EConstr.iter sigma (fun c -> aux c) c
  in aux c

let make_hint_clenv poly ?len (c, cty, ctx) gl =
  (** Drop the clenv and create a fresh one. *)
  let env = Proofview.Goal.env gl in
  let sigma = Proofview.Goal.sigma gl in
  let sigma, c, cty =
    if poly then
      (** Refresh the instance of the hint *)
      let (usubst, ctx) = Universes.fresh_universe_context_set_instance ctx in
      let c = Vars.subst_univs_level_constr usubst c in
      let cty = Vars.subst_univs_level_constr usubst cty in
      let sigma = Evd.merge_context_set Evd.univ_flexible sigma ctx in
      sigma, c, cty
    else sigma, c, cty
  in
  check_typeable_in_env env sigma c; (* raise NotFound if variables are missing *)
  make_clenv_from_env env sigma ?len (c,cty)

let make_clenv poly c kont =
  let open Ftactic in
  let tac =
    enter begin fun gl ->
     let res =
       try Some (make_hint_clenv poly c gl)
       with Not_found -> None
     in
     match res with
     | None -> lift (Tacticals.New.tclZEROMSG (str "Hint not applicable to this goal"))
     | Some (evd, clenv) ->
	bind (lift (Proofview.Unsafe.tclEVARS evd))
	     (fun () -> return clenv) end
  in run tac kont
      
let unify_resolve poly flags (c : raw_hint) =
  make_clenv poly c
   (fun clenv ->
    Clenvtac.clenv_refine2 ~with_evars:false (* MS: fixme compat broken? *) ~flags clenv)
