...
let connect_hint_clenv ~poly (c, _, ctx) clenv sigma gl =
...
and tac_of_hint dbg db_list local_db concl (flags, ({pat=p; code=t;poly=poly;db=dbname})) =
  let tactic = function
    ...
    | ERes_pf _ -> Proofview.Goal.enter (fun _ _ -> Tacticals.New.tclZEROMSG (str "eres_pf"))
...
let extend_local_db decl db sigma gl =
...
let intro_register dbg kont db =
  Tacticals.New.tclTHEN (dbg_intro dbg)
    (Proofview.Goal.enter begin fun sigma gl ->
      let extend_local_db decl db = extend_local_db decl db sigma gl in
