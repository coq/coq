...
let atomize_param_of_ind_then ((indref,deppos),nparams,_) hyp0 tac =
  ...
  in
  if prods = [] then atomize_one (List.length argl) [] [] Id.Set.empty
  else Proofview.tclUNIT ()
  end
  ...
  let seek_deps env decl rhyp =
    ...
    then begin
      ...
      let dephyp0 = List.is_empty inhyps &&
        (Option.exists (fun id -> occur_var_in_decl env sigma id decl) hyp0_opt)
      in
...
(* The basic general form of an induction principle (possibly up to
   permutation of arguments) is the following:

   forall prm1 prm2 ... prmp,                          (induction parameters)
   forall Q1...,(Qi:Ui_1 -> Ui_2 ->...-> sort),...Qq, (predicates)
   branch1, branch2, ... , branchr,                    (branches of the principle)
   forall (x1:Ti_1) (x2:Ti_2) ... (xni:Ti_ni),         (induction arguments)
   -> (Qi x1...xni)                                    (conclusion)

   A "standard" induction principle has only one main argument:

   forall prm1 prm2 ... prmp,                          (induction parameters)
   forall Q1...,(Qi:Ui_1 -> Ui_2 ->...-> sort),...Qq, (predicates)
   branch1, branch2, ... , branchr,                    (branches of the principle)
   forall (x1:Ti_1) (x2:Ti_2) ... (xni:Ti_ni),         (indices/realparams of the main induction arg)
   forall (HI: I prm1..prmp x1...xni)                  (main induction arg - indarg/indref)
      (Qi x1...xni HI)                                 (conclusion)
                   ^^
                optional
           (indarg_in_concl)

   The general form of a "functional" induction principle is the following

   forall prm1 prm2 ... prmp,                          (induction parameters)
   forall Q1...,(Qi:Ui_1 -> Ui_2 ->...-> sort),...Qq, (predicates)
   branch1, branch2, ... , branchr,                    (branches of the principle)
   forall (x1:Ti_1) (x2:Ti_2) ... (xni:Ti_ni),         (induction arguments)
      (Qi x1...xni (f prm1...prmp x1...xni))           (conclusion)
                    ^^^^^^^^^^^^^^^^^^^^^^^^
                           [farg]
   (this can be seen as a "standard" non-dependent induction principle
    with two implicit induction arguments
         (res:Vi) (y: inductive_graph_of_f prm1...prmp x1...xni res)
    that have been pre-instantiated by (f prm1...prmp x1...xni) of a
    proof that f is correct wrt its inductive graph)

   The general form includes other forms, such as well-foundedness principles:
   ...
   or double induction principles, e.g.:

   forall P:nat -> nat -> Set,
   (forall n m, (forall p q, p < n -> P p q) ->
...
type scheme_main_arg = {
  indarg: types;          (* (headref prm1..prmp x1...xni) *)
  indref: GlobRef.t;      (* head of the type (not necessarily inductive actually) *)
  indarg_in_concl: bool;  (* true if dependent in conclusion *)
  deppos: int list;       (* which arguments of I behave as indices *)
}

type scheme_premisses =
  | IsParam of types
  | IsPredicate of types
  | IsBranch of types * int
  | IsAuxArg of types
  | IsMainArg of scheme_main_arg

type elim_scheme_concl = {
  concl_hd : int;                (* index of predicate *)
  concl_args : constr list;      (* arguments of the predicate (w/o the functional arg if any) *)
  farg_in_concl : constr option; (* extra optional (f...) at the end of conclusion *)
}

type elim_scheme = {
  elimt : types;
  sign : scheme_premisses list;
  nparams: int;                  (* number of parameters *)
  nauxargs: int;                 (* number of indices of main arguments *)
  nmainargs: int;                (* number of main arguments *)
  concl : elim_scheme_concl;
}

let rec occur_branch_head_ccl sigma n ccl_hd = function
  | [] ->
    Int.equal n ccl_hd
  | (IsParam _ | IsPredicate _ | IsAuxArg _ | IsMainArg _) :: sign ->
    occur_branch_head_ccl sigma (n+1) ccl_hd sign
  | IsBranch (_, k) :: sign ->
    Int.equal n k || occur_branch_head_ccl sigma (n+1) ccl_hd sign

let rec occur_args sigma n = function
  | [] -> false
  | IsMainArg {indarg} :: sign -> not (noccurn sigma n indarg) || occur_args sigma (n+1) sign
  | (IsParam _ | IsPredicate _ | IsBranch _ | IsAuxArg _) :: sign -> occur_args sigma (n+1) sign

let decompose_elim_scheme sigma elimt =
  let rec aux elimt =
    match EConstr.kind sigma elimt with
      | Prod(nme,tpe,elimt') ->
        let nsign, {sign; nparams; nauxargs; nmainargs; concl} = aux elimt' in
        let decl =
          let tpe_hyps, tpe_ccl = decompose_prod_assum sigma tpe in
          let ntpe_hyps = List.length tpe_hyps in
          let tpe_hd, tpe_args = decompose_app sigma tpe_ccl in
          (* A branch is not not dependent and end with a predicate *)
          (* Note: with HIT, a branch would be dependent *)
          if noccurn sigma 1 elimt' && isRel sigma tpe_hd then
            IsBranch (tpe, destRel sigma tpe_hd - ntpe_hyps)
          else
          (* A predicate is at the head of the conclusion or *)
          (* (in case if mutual induction) of a branch) *)
          if occur_branch_head_ccl sigma 1 concl.concl_hd sign then
            IsPredicate tpe
          else
            let exists_ccl k c = List.exists (dependent sigma (lift (nsign+k) c)) concl.concl_args in
            let dep_in_concl = exists_ccl 0 (mkRel 1) in
            let toparg =
              not (occur_args sigma 1 sign) &&
              not (Option.exists (dependent sigma (mkRel (nsign+1))) concl.farg_in_concl) in
            (* A main argument is one which is not dependent in another main argument *)
            (* considering that the optional functional argument is an implicit main argument *)
            if toparg then
              let indref =
                try fst (destRef sigma tpe_hd)
                with Not_found -> error_ind_scheme "the applied form of a main argument" in
              let f i l c =
                (* We a priori accept functional main arguments (i.e. ntpe_hyps<>0) but it
                   is not clear what it means for their indices *)
                if noccur_between sigma 1 ntpe_hyps c && exists_ccl 1 c then i::l else l in
              let deppos = List.fold_left_i f 1 [] tpe_args in
              IsMainArg {indarg = tpe; indref; deppos; indarg_in_concl = dep_in_concl}
            else
            (* An auxiliary argument occurs in a main argument and *)
            (* in the conclusion (excepting the optional functional argument) *)
            if dep_in_concl then
              IsAuxArg tpe
            (* A parameter occurs only in a main argument *)
            else
              IsParam tpe
        in
        let nparams = match decl with IsParam _ -> nparams+1 | _ -> nparams in
        let nmainargs = match decl with IsMainArg _ -> nmainargs+1 | _ -> nmainargs in
        let nauxargs = match decl with IsAuxArg _ -> nauxargs+1 | _ -> nauxargs in
        (nsign+1, {elimt; sign = decl::sign; nparams; nauxargs; nmainargs; concl})
      | LetIn(nme,tpe,_,elimt') -> aux (subst1 tpe elimt')
      | _ ->
          let c, args = decompose_app sigma elimt in
          match kind sigma c with
            | Rel k ->
                let farg_in_concl, args =
                  match List.rev args with
                  | a::args when isApp sigma a -> Some a, List.rev args
                  | _ -> None, args in
                let concl = {concl_hd = k; concl_args = args; farg_in_concl} in
                (0, {elimt; sign = []; nparams = 0; nauxargs = 0; nmainargs = 0; concl})
            | _ -> error_ind_scheme "the conclusion of" in
  snd (aux elimt)

let compute_elim_metadata sigma scheme =
  List.map_filter_i (fun i -> function IsMainArg c -> Some (i,c) | _ -> None) scheme.sign

let compute_elim_type sigma _ind_guess scheme =
  match compute_elim_metadata sigma scheme with
  | [i,{indref; deppos}] ->
    (i,(indref,deppos)) (* Do we need to do some weak check that it matches ind_guess? *)
  | _ -> assert false (* Not a single main argument *)

let compute_scheme_signature evd (scheme,ext_scheme) names_info ind_type_guess =
  (* Vérifier que les arguments de Qi sont bien les xi. *)
  let cond, indref =
    match ext_scheme with
      | [_,{indarg; indref}] when scheme.concl.farg_in_concl = None ->
          (* Standard scheme from an inductive type *)
          let indhd,indargs = decompose_app evd indarg in
          let cond hd = EConstr.eq_constr evd hd indhd in
          (cond, Some indref)
      | _ -> (* Non standard scheme *)
          let cond hd = EConstr.eq_constr evd hd ind_type_guess && scheme.concl.farg_in_concl = None
          in (cond, None)
  in
  ...
  let is_pred env n c =
    ...
    match EConstr.kind evd hd with
      | Rel q when n < q && is_predicate (List.nth env (q-n-1)) -> IndArg
      | _ when cond hd -> RecArg
      | _ -> OtherArg
  in
  let rec check_branch env p c =
    match EConstr.kind evd c with
      | Prod (_,t,c) ->
        (is_pred env p t, true, not (Vars.noccurn evd 1 c)) :: check_branch env (p+1) c
      | LetIn (_,_,_,c) ->
        (OtherArg, false, not (Vars.noccurn evd 1 c)) :: check_branch env (p+1) c
      | _ when is_pred env p c == IndArg -> []
      | _ -> raise Exit
  in
  let rec find_branches env = function
      | IsBranch (t,_) as d :: sign ->
        (try
           let lchck_brch = check_branch env 0 t in
           let n = List.fold_left
             (fun n (b,_,_) -> if b == RecArg then n+1 else n) 0 lchck_brch in
           let recvarname, hyprecname, avoid =
             make_up_names n indref names_info in
           let namesign =
             List.map (fun (b,is_assum,dep) ->
               (b,is_assum,dep,if b == IndArg then hyprecname else recvarname))
               lchck_brch in
           (avoid,namesign) :: find_branches (d::env) sign
         with Exit-> error_ind_scheme "the branches of")
      | d :: sign -> find_branches (d::env) sign
      | [] -> []
  in
  Array.of_list (find_branches [] scheme.sign)
...
type eliminator_source =
  | ElimUsing of EConstr.t with_bindings * elim_scheme * Id.t * EConstr.types * int
...
let find_induction_type isrec elim hyp0 gl =
  ...
  | None ->
    ...
    let _,indref = compute_elim_type sigma' ind_guess scheme in
  ...
  | Some e ->
    ...
    let scheme = decompose_elim_scheme sigma elimt in
    let index,indref = compute_elim_type sigma ind_guess scheme in
    sigma, (indref, scheme.nparams, ElimUsing (elimc,scheme,hyp0,ind_guess,index))

let is_functional_or_multi_induction elimc gl =
  let sigma = Tacmach.New.project gl in
  let scheme = decompose_elim_scheme sigma (Tacmach.New.pf_get_type_of gl (fst elimc)) in
  (* The test is not safe: with non-functional induction on non-standard
     induction scheme, this may fail *)
  scheme.nmainargs <> 1 || scheme.concl.farg_in_concl <> None
...
let get_eliminator elim dep s gl =
  match elim with
  | ElimUsing (elimc,scheme,id,ind_guess,index) ->
    let sigma = Tacmach.New.project gl in
    ...
    let elim = ({ elimindex = Some index; elimbody = elimc }, scheme.elimt) in
...
let apply_induction_in_context with_evars hyp0 inhyps elim indvars names induct_tac =
  Proofview.Goal.enter begin fun gl ->
    ...
    let dep_in_concl = Option.exists (fun id -> occur_var env sigma id concl) hyp0 in
...
let msg_not_right_number_induction_arguments scheme =
  let nargs = scheme.nmainargs + scheme.nauxargs in
  str"Not the right number of induction arguments (expected " ++
  pr_enum (fun x -> x) [
    if scheme.concl.farg_in_concl <> None then str "the function name" else mt();
    if scheme.nparams != 0 then int scheme.nparams ++ str (String.plural scheme.nparams " parameter") else mt ();
    if nargs != 0 then int nargs ++ str (String.plural nargs " argument") else mt ()] ++ str ")."
...
let induction_without_atomization isrec with_evars elim names lid =
  ...
  let nargs_indarg_farg =
    scheme.nauxargs + scheme.nmainargs + (if scheme.concl.farg_in_concl <> None then 1 else 0) in
  ...
  let realindvars = List.rev (if scheme.concl.farg_in_concl <> None then List.tl indvars else indvars) in
  let lidcstr = List.map mkVar (List.rev indvars) in
  let params = List.rev lid_params in
  let indvars = Id.Set.of_list indvars in
  ...
  let elim = ElimUsing (elimc,scheme,List.hd lid,ind_type_guess,(-1)) in
...
let check_expected_type env sigma i (elimc,bl) elimt =
  (* Compute the expected template type of the term in case a using
     clause is given *)
  let sigma,cl = make_evar_clause env sigma ~len:i elimt in
...
let check_enough_applied env sigma elim =
  ...
  | Some elimc ->
      ...
      | [i,_] ->
          ...
          check_expected_type env sigma i elimc elimt
      | _ ->
...
let induction_destruct isrec with_evars (lc,elim) =
  ...
  | [c,(eqname,names as allnames),cls] ->
    ...
    let sigma = Tacmach.New.project gl in
    match elim with
    | Some elim when is_functional_or_multi_induction elim gl ->
