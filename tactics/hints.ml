...
type hint_action =
  ...
  | AddHints of bool (* superglobal? *) * hint_entry list
...
let subst_autohint (subst, obj) =
  ...
  let action = match obj.hint_action with
    ...
    | AddHints (superglobal, hintlist) ->
      let hintlist' = List.Smart.map subst_hint hintlist in
      if hintlist' == hintlist then obj.hint_action else AddHints (superglobal, hintlist')
    | RemoveHints grs ->
...
let add_resolves env sigma clist ~local ~superglobal dbnames =
  List.iter
    (fun dbname ->
      ...
      in
      let hint = make_hint ~local dbname (AddHints (superglobal, r)) in
      Lib.add_anonymous_leaf (inAutoHint hint))
    dbnames
...
let add_unfolds l ~local ~superglobal dbnames =
  List.iter
    (fun dbname ->
      let hint = make_hint ~local dbname (AddHints (superglobal, List.map make_unfold l)) in
      Lib.add_anonymous_leaf (inAutoHint hint))
    dbnames
...
let add_extern info tacast ~local ~superglobal dbname =
  ...
  let hint = make_hint ~local dbname
                       (AddHints (superglobal, [make_extern (Option.get info.hint_priority) pat tacast])) in
  Lib.add_anonymous_leaf (inAutoHint hint)
...
let add_trivials env sigma l ~local ~superglobal dbnames =
  List.iter
    (fun dbname ->
      ...
      let hint = make_hint ~local dbname (AddHints (superglobal, l)) in
      Lib.add_anonymous_leaf (inAutoHint hint))
    dbnames
