...
Section Powers_of_2.
  ...
  Proof.
    simple induction x; intros;
      [ simpl in |- *; omega
	| simpl in |- *; unfold two_power_pos in |- *; apply Old_zorder.Zgt_pos_0
	| absurd (0 <= Zneg p);
  ...
  Proof.
    ...
    rewrite (two_p_is_exp x 1 H (Old_zorder.Zle_0_pos 1)).
    apply Zmult_comm.
  Qed.
	...
	| intros; elim (Zle_lt_or_eq 0 x0 H0);
	  [ intros;
		...
		[ rewrite (two_p_S x0); [ omega | assumption ]
		  | apply Old_zorder.Zlt_0_le_0_pred; assumption ]
