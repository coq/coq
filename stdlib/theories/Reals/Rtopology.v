    ...
    + split.
      ...
      * red; intro; elim H0; intros; unfold intersection_domain in H1;
        ...
        -- change (Rabs (x - y) < D);
           ...
           ++ rewrite <-(Rplus_half_diag D); apply Rplus_lt_compat.
...
Proof.
  ...
  1:intro; set (f0 := mkfamily D g H3); assert (H4 := H f0);
    cut (covering_open_set X f0).
  1:intro; assert (H6 := H4 H5); elim H6; clear H6; intros D' H6.
  1:unfold covering_finite in H6; decompose [and] H6;
  ...
  1:intro; assert (H10 := H0 (disc x (mkposreal _ H9)));
    cut (neighbourhood (disc x (mkposreal alp H9)) x).
  1:intro; assert (H12 := H10 H11); elim H12; clear H12; intros y H12;
    ...
    cut (alp <= Rabs (y0 - x) / 2).
  1:intro; assert (H18 := Rlt_le_trans _ _ _ H12 H17);
  ...
  - apply Rle_lt_trans with (Rabs (y0 - y) + Rabs (y - x)).
    ...
    + rewrite <-(Rplus_half_diag (Rabs (y0 - x))); apply Rplus_lt_compat; assumption.
...
Proof.
  ...
  - unfold compact; intros f0 (H,H5);
    ...
    cut (A a); [intro H0|].
    1:cut (bound A); [intro H1|].
    1:cut (exists a0 : R, A a0); [intro H2|].
    1:pose proof (completeness A H1 H2) as (m,H3); unfold is_lub in H3.
    1:cut (a <= m <= b); [intro H4|].
    1:unfold covering in H; pose proof (H m H4) as (y0,H6).
    1:unfold family_open_set in H5; pose proof (H5 y0 m H6) as (eps,H8).
    1:cut (exists x : R, A x /\ m - eps < x <= m);
                 ...
                 { apply H8; unfold disc, Rabs; destruct (Rcase_abs (x0 - m)) as [Hlt|Hge].
                   - rewrite Ropp_minus_distr; apply Rlt_trans with (m - x).
                     + unfold Rminus; apply Rplus_lt_compat_l; apply Ropp_lt_gt_contravar;
                         auto with real.
                     + apply Rplus_lt_reg_l with (x - eps);
                         replace (x - eps + (m - x)) with (m - eps) by ring.
                       replace (x - eps + eps) with x by ring.
                       assumption.
                   - apply Rle_lt_trans with (m' - m).
                     + unfold Rminus; do 2 rewrite <- (Rplus_comm (- m));
                         apply Rplus_le_compat_l; elim H14; intros; assumption.
                     + apply Rplus_lt_reg_l with m; replace (m + (m' - m)) with m' by ring.
...
Proof.
  ...
  - set (D := ind f0); set (g := f f0); unfold closed_set in H0.
    ...
    1:intro; set (f' := mkfamily D' g' H3); cut (covering_open_set X f').
...
Proof.
  ...
  cut (forall x:R, (exists y : R, g x y) -> D x).
  1:intro; set (f' := mkfamily D g H3).
  1:cut (covering_open_set X f').
  ...
  - set
    ...
    + unfold continuity; intro; case (Rtotal_order x a); intro.
      * unfold continuity_pt; unfold continue_in;
          ...
          split.
        -- change (0 < a - x); apply Rlt_0_minus; assumption.
      ...
      * elim H3; intro.
        -- assert (H5 : a <= a <= b). {
           ...
           ++ unfold Rmin; case (Rle_dec x0 (b - a)); intro.
              ...
              ** change (0 < b - a); apply Rlt_0_minus; assumption.
        ...
        -- case (Rtotal_order x b); intro.
           ++ assert (H6 : a <= x <= b). {
              ...
              ** unfold Rmin; case (Rle_dec (x - a) (b - x)) as [Hle|Hnle].
                 { case (Rle_dec x0 (x - a)) as [Hlea|Hnlea]; assumption. }
                 case (Rle_dec x0 (b - x)) as [Hleb|Hnleb]; assumption.
           ...
           ++ elim H5; intro.
              ...
              ** unfold continuity_pt; unfold continue_in;
                   ...
                   split.
                 { change (0 < x - b); apply Rlt_0_minus; assumption. }
...
Proof.
  ...
  - intro HypProl.
    ...
    1:cut (exists x : R, image_dir g (fun c:R => a <= c <= b) x).
...
Proof.
  ...
  - assert (H1 :  exists z : R, X z). {
    ...
    + cut (family_closed_set f0).
      1:intro; cut (intersection_vide_in X f0).
              ...
              ** { unfold family_open_set; intro; simpl; elim (classic (X x));
                   ...
                   - unfold g; unfold open_set; intros; elim H4; clear H4;
                     ...
                     + set (d := x1 / 2 - Rabs (x0 - x)); assert (H7 : 0 < d).
                       * unfold d; apply Rlt_0_minus; elim H5; clear H5; intros;
           ...
           ++ elim (H0 _ H3); intros DF H4; unfold covering_finite in H4; elim H4; clear H4;
                         ...
                         -- assert (H19 := H8 i H17); elim H19; clear H19; intros; rewrite <- H18 in H20;
                            ...
                            ++ unfold Rdiv; apply Rmult_lt_reg_l with 2.
                               ...
                               { rewrite Rmult_1_r; pattern (pos_Rl l' i) at 1; rewrite <- Rplus_0_r;
                                 rewrite <-Rplus_diag; apply Rplus_lt_compat_l; apply H19. }
                               discrR.
                         -- assert (H19 := H8 i H17); elim H19; clear H19; intros; rewrite <- H18 in H20;
                            ...
                            ++ rewrite <-(Rplus_half_diag (pos_Rl l' i)); apply Rplus_lt_compat.
              ...
              ** { intros; elim (H5 x); intros; elim (H8 H6); intros;
                   ...
                   - assert (H12 :  exists x : R, E x). {
                       ...
                       - split;
                         [ unfold Rmin; case (Rle_dec x0 (M - m)); intro;
                           [ apply H12 | apply Rlt_0_minus; apply Hyp ]
                         | apply Rmin_r ].
                       - intros; case (Req_dec x z); intro.
                         ...
                         + apply H14; split;
                             [ unfold D_x, no_cond; split; [ trivial | assumption ]
                             | apply Rlt_le_trans with (Rmin x0 (M - m));
                               [ apply H15 | apply Rmin_l ] ].
                     }
