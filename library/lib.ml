...
type frozen = {
  synterp_state : Synterp.frozen;
  interp_state : Interp.frozen;
}

let freeze () = {
  synterp_state = Synterp.freeze ();
  interp_state = Interp.freeze ();
}

let unfreeze st =
  Synterp.unfreeze st.synterp_state;
  Interp.unfreeze st.interp_state
...
let drop_objects { synterp_state; interp_state } =
  let synterp_state = Synterp.drop_objects synterp_state in
  let interp_state = Interp.drop_objects interp_state in
