...
let constr_display csr =
  ...
  | Rel (n, ans) -> "Rel("^(string_of_int n)^","^(Annots.show ans)^")"
  | Meta n -> "Meta("^(string_of_int n)^")"
  | Var (id, ans) -> "Var("^(Id.to_string id)^","^(Annots.show ans)^")"
  ...
  | Const ((c,u), ans) -> "Const("^(Constant.to_string c)^","^(universes_display u)^","^(Annots.show ans)^")"
  | Ind (((sp,i),u), stg) ->
...
let print_pure_constr csr =
  ...
  | Rel (n, ans) -> print_string "#"; print_int n; pp (Annots.pr ans)
  | Meta n -> print_string "Meta("; print_int n; print_string ")"
  | Var (id, ans) -> print_string (Id.to_string id); pp (Annots.pr ans)
