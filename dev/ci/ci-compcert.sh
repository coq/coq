...
if [ -z "${COQLIB}" ]; then COQLIB="${CI_INSTALL_DIR}/lib/coq"; fi
( cd "${CI_BUILD_DIR}/compcert"
  [ -e Makefile.config ] || ./configure -ignore-coq-version x86_32-linux -install-coqdev -clightgen -use-external-MenhirLib -use-external-Flocq -prefix ${CI_INSTALL_DIR} -coqdevdir ${COQLIB}/user-contrib/compcert
  make
