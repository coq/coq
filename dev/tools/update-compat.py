...
NUMBER_OF_OLD_VERSIONS = 2
NUMBER_OF_COMPAT_VERSIONS = NUMBER_OF_OLD_VERSIONS+1
...
TEST_SUITE_PATHS = tuple(os.path.join(ROOT_PATH, 'test-suite', 'success', i)
                         for i in ('CompatOldFlag.v', 'CompatPreviousFlag.v', 'CompatCurrentFlag.v'))
TEST_SUITE_DESCRIPTIONS = ('current-minus-two', 'current-minus-one', 'current')
...
assert(len(TEST_SUITE_PATHS) == NUMBER_OF_COMPAT_VERSIONS)
...
def get_new_versions(known_versions, cur_version):
    if cur_version in known_versions:
        assert(known_versions[-1] == cur_version)
        assert(len(known_versions) == NUMBER_OF_COMPAT_VERSIONS)
        return known_versions
    assert(len(known_versions) >= NUMBER_OF_OLD_VERSIONS)
    return tuple(list(known_versions[-NUMBER_OF_OLD_VERSIONS:]) + [cur_version])

def update_compat_files(old_versions, new_versions, assert_unchanged=False):
...
def update_parse_compat_version(new_versions, contents, relpath):
    line_count = NUMBER_OF_COMPAT_VERSIONS+2 # 1 for the first line, 1 for the invalid flags
...
def update_if_changed(contents, new_contents, path, assert_unchanged=False):
...
def update_flags_mli(old_versions, new_versions, assert_unchanged=False):
    ...
    update_if_changed(contents, new_contents, FLAGS_MLI_PATH, assert_unchanged=assert_unchanged)

def update_flags_ml(old_versions, new_versions, assert_unchanged=False):
    ...
    update_if_changed(contents, new_contents, FLAGS_ML_PATH, assert_unchanged=assert_unchanged)

def update_coqargs_ml(old_versions, new_versions, assert_unchanged=False):
    ...
    update_if_changed(contents, new_contents, COQARGS_ML_PATH, assert_unchanged=assert_unchanged)

def update_g_vernac(old_versions, new_versions, assert_unchanged=False):
    with open(G_VERNAC_PATH, 'r') as f: contents = f.read()
    new_contents = update_parse_compat_version(new_versions, contents, os.path.relpath(G_VERNAC_PATH, ROOT_PATH))
    check_no_old_versions(old_versions, new_versions, new_contents, os.path.relpath(G_VERNAC_PATH, ROOT_PATH))
    update_if_changed(contents, new_contents, G_VERNAC_PATH, assert_unchanged=assert_unchanged)

def update_flags(old_versions, new_versions, assert_unchanged=False):
    update_flags_mli(old_versions, new_versions, assert_unchanged=assert_unchanged)
    update_flags_ml(old_versions, new_versions, assert_unchanged=assert_unchanged)
    update_coqargs_ml(old_versions, new_versions, assert_unchanged=assert_unchanged)
    update_g_vernac(old_versions, new_versions, assert_unchanged=assert_unchanged)

def update_test_suite(new_versions, assert_unchanged=False):
    ...
    for i, (v, path, descr) in enumerate(zip(new_versions, TEST_SUITE_PATHS, TEST_SUITE_DESCRIPTIONS)):
        ...
        lines.append('')
        new_contents = '\n'.join(lines)
        update_if_changed(contents, new_contents, path, assert_unchanged=assert_unchanged)

def update_doc_index(new_versions, assert_unchanged=False):
    ...
    update_if_changed(contents, new_contents, DOC_INDEX_PATH, assert_unchanged=assert_unchanged)

def update_bug_4789(new_versions, assert_unchanged=False):
...
""" % new_versions[0]
    update_if_changed(contents, new_contents, BUG_4798_PATH, assert_unchanged=assert_unchanged)
...
def update_compat_notations(old_versions, new_versions, assert_unchanged=False):
    for root, dirs, files in os.walk(os.path.join(ROOT_PATH, 'theories')):
        for fname in files:
            ...
            update_if_changed(contents, new_contents, os.path.join(root, fname), assert_unchanged=assert_unchanged)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        if len(sys.argv) > 2 or sys.argv[1] != '--assert-unchanged':
            print('USAGE: %s [--assert-unchanged]' % sys.argv[0])
        else:
            assert_unchanged = True
    cur_version = get_version()
    known_versions = get_known_versions()
    new_versions = get_new_versions(known_versions, cur_version)
    update_compat_files(known_versions, new_versions, assert_unchanged=assert_unchanged)
    update_flags(known_versions, new_versions, assert_unchanged=assert_unchanged)
    update_test_suite(new_versions, assert_unchanged=assert_unchanged)
    update_doc_index(new_versions, assert_unchanged=assert_unchanged)
    update_bug_4789(new_versions, assert_unchanged=assert_unchanged)
    update_compat_notations(known_versions, new_versions, assert_unchanged=assert_unchanged)
