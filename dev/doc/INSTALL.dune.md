...
- `dune build @check` : build all ml targets as fast as possible
- `dune build` : build a complete Coq distribution
- `dune exec -- dev/shim/coqtop-prelude` : build and launch coqtop + prelude [equivalent to `make -f Makefile.dune states`].
- `dune build $target`: where `$target` can refer to the build
  directory or the source directory [but will be placed under
  `_build`]

Optionally, you can view the helper targets available in `Makefile.dune`;
`make -f Makefile.dune` will display help. The command invokes various
`dune` commands for you with the right arguments. However, do not call
`./configure` by hand.
...
Unlike make, Dune distributes information on how to build a project in
local build files present in each directory (named `dune`) to do a global
build, for example `plugins/ltac/dune` or `kernel/dune`.
...
Dune reads custom settings from `~/.config/dune/config`; see `man dune-config`. Among other things, this file lets you set the number of build threads `(jobs N)` and display options `(display _mode_)`.
...
If you built the full standard library with the default target (via `dune build`),
then you can run the commands in the `_build/install/default/bin` directories (including `coq_makefile`).
...
To build a single package, use `dune build $PACKAGE.install`.
...
- After building Coq with `dune build`, you can run the test-suite
...
- You can also run the test suite in a hygienic way by invoking
  `dune runtest`. This is convenient for
...
- `dune build` build all targets with an install overlay
