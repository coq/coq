...
let add ((((s,eid),(sid,verbose)),offset),(line_nb,bol_pos)) =
  let doc = get_doc () in
  let open Loc in
  let loc = { (initial ToplevelInput) with line_nb } in
  let pa = Pcoq.Parsable.make ~loc (Gramlib.Stream.of_string ~offset s) in
