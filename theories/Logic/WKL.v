...
- induction 1 as [|* HP _ (l'&Hl'&HPl')|* HP _ (l'&Hl'&HPl')].
  + exists []. split. reflexivity. intros n Hyp. apply le_n_0_eq in Hyp as <-. assumption.
...
- intros (l'& <- &HPl'). induction l' as [|[|]] in l, HPl' |- *.
  ...
  + eapply next_left.
    * apply (HPl' 0), le_0_n.
    * fold (length l'). apply IHl'. intros n' H. apply le_n_S, HPl' in H. simpl in H. rewrite <- app_assoc in H. assumption.
  + apply next_right.
    * apply (HPl' 0), le_0_n.
    * fold (length l'). apply IHl'. intros n' H. apply le_n_S, HPl' in H. simpl in H. rewrite <- app_assoc in H. assumption.
Qed.
