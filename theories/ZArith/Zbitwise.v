...
Local Ltac to_bitwise :=
  ...
  repeat match goal with H : ?a = ?b :> Z |- _ =>
    apply (f_equal (fun x => Z.testbit x i)) in H;
    autorewrite with bitwise in H end.
...
Lemma pred_opp x : -x-1 = Z.lnot x.
Proof. pose proof lnot_pred (x+1). replace (x+1-1) with x in * by lia; lia. Qed.
...
Proof. pose proof pred_opp y; pose proof lnot_lnot y; lia. Qed.
...
Definition addcarries' (x y : Z) (c : bool) := (x + y + Z.b2z c) .^ (x .^ y).
Definition addcarries (x y : Z) := (x + y) .^ (x .^ y).

Lemma addcarries_ok x y : addcarries x y = addcarries' x y false.
Proof. cbv [addcarries addcarries'] in *; rewrite Z.add_0_r; trivial. Qed.

Lemma testbit_addcarries'_0 x y c : Z.testbit (addcarries' x y c) 0 = c.
Proof. cbv [addcarries']. autorewrite with bitwise. btauto. Qed.

Lemma shiftr_addcarries'_1 x y c : Z.shiftr (addcarries' x y c) 1 =
  (x .& y) .| ((x .^ y) .& (addcarries' x y c)).
Proof. cbv [addcarries']. rewrite addcarryeqn. p. Qed.

Lemma testbit_addcarries'_pos x y c i (Hi : 0 < i) : Z.testbit (addcarries' x y c) i =
  Z.testbit ((x .& y) .| ((x .^ y) .& (addcarries' x y c))) (i-1).
Proof.
  pose proof shiftr_addcarries'_1 x y c.
  apply (f_equal (fun x => Z.testbit x (i-1))) in H; rewrite <-H.
  rewrite Z.shiftr_spec; try lia. f_equal; lia.
Qed.

Lemma testbit_addcarries_0 x y : Z.testbit (addcarries x y) 0 = false.
Proof. rewrite addcarries_ok; apply testbit_addcarries'_0. Qed.

Lemma shiftr_addcarries_1 x y : Z.shiftr (addcarries x y) 1 =
  (x .& y) .| ((x .^ y) .& (addcarries x y)).
Proof. rewrite addcarries_ok; apply shiftr_addcarries'_1. Qed.

Lemma testbit_addcarries_pos x y i (Hi : 0 < i) : Z.testbit (addcarries x y) i =
  Z.testbit ((x .& y) .| ((x .^ y) .& (addcarries x y))) (i-1).
Proof. rewrite addcarries_ok; apply testbit_addcarries'_pos, Hi. Qed.

Local Hint Rewrite testbit_addcarries'_0 testbit_addcarries_0 : bitwise.

Lemma addcarries_lor_land x y : addcarries (x .| y) (x .& y) = addcarries x y.
Proof.
  apply Z.bits_inj'; refine (Wf_Z.natlike_ind _ _ _); [prove_bitwise|intros i Hi IH].
  rewrite 2 testbit_addcarries_pos by lia; replace (Z.succ i-1) with i by lia.
  prove_bitwise.
Qed.
...
Lemma lnot_sub' x y : Z.lnot (x - y) = y - x -1.
Proof. rewrite <-!pred_opp; lia. Qed.
