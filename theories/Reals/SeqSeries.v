...
Lemma Cesaro :
  forall (An Bn:nat -> R) (l:R),
    ...
    l.
Proof.
  unfold Un_cv; intros; assert (H3 : forall n:nat, 0 < sum_f_R0 An n); trivial.
  intro; apply tech1; trivial.
  assert (H4 : forall n:nat, sum_f_R0 An n <> 0); trivial.
  intro; red; intro; assert (H5 := H3 n); rewrite H4 in H5;
    elim (Rlt_irrefl _ H5); trivial.
  assert (H5 := cv_infty_cv_R0 _ H4 H1); assert (H6 : 0 < eps / 2); trivial.
  unfold Rdiv; apply Rmult_lt_0_compat; trivial.
  apply Rinv_0_lt_compat; prove_sup; trivial.
  elim (H _ H6); clear H; intros N1 H;
    set (C := Rabs (sum_f_R0 (fun k:nat => An k * (Bn k - l)) N1));
      assert
        (H7 :
          exists N : nat,
            (forall n:nat, (N <= n)%nat -> C / sum_f_R0 An n < eps / 2)); trivial.
  case (Req_dec C 0); intro; trivial.
  exists 0%nat; intros; trivial.
  rewrite H7; unfold Rdiv; rewrite Rmult_0_l; apply Rmult_lt_0_compat; trivial.
  apply Rinv_0_lt_compat; prove_sup; trivial.
  assert (H8 : 0 < eps / (2 * Rabs C)); trivial.
  unfold Rdiv; apply Rmult_lt_0_compat; trivial.
  apply Rinv_0_lt_compat; apply Rmult_lt_0_compat; trivial.
  prove_sup; trivial.
  ...
  elim (H5 _ H8); intros; exists x; intros; assert (H11 := H9 _ H10);
    unfold R_dist in H11; unfold Rminus in H11; rewrite Ropp_0 in H11;
      rewrite Rplus_0_r in H11; trivial.
  apply Rle_lt_trans with (Rabs (C / sum_f_R0 An n)); trivial.
  apply RRle_abs; trivial.
  unfold Rdiv; rewrite Rabs_mult; apply Rmult_lt_reg_l with (/ Rabs C); trivial.
  apply Rinv_0_lt_compat; apply Rabs_pos_lt; trivial.
  rewrite <- Rmult_assoc; rewrite <- Rinv_l_sym; trivial.
  rewrite Rmult_1_l; replace (/ Rabs C * (eps * / 2)) with (eps / (2 * Rabs C)); trivial.
  unfold Rdiv; rewrite Rinv_mult_distr; trivial.
  ring; trivial.
  discrR; trivial.
  ...
  elim H7; clear H7; intros N2 H7; set (N := max N1 N2); exists (S N); intros;
    unfold R_dist;
      ...
      (sum_f_R0 (fun k:nat => An k * (Bn k - l)) n / sum_f_R0 An n); trivial.
  assert (H9 : (N1 < n)%nat); trivial.
  apply lt_le_trans with (S N); trivial.
  apply le_lt_n_Sm; unfold N; apply le_max_l; trivial.
      ...
      apply Rle_lt_trans with
        (Rabs (sum_f_R0 (fun k:nat => An k * (Bn k - l)) N1 / sum_f_R0 An n) +
          ...
          (sum_f_R0 (fun i:nat => An (S N1 + i)%nat * (Bn (S N1 + i)%nat - l))
            (n - S N1) / sum_f_R0 An n)); trivial.
  apply Rabs_triang; trivial.
  rewrite (double_var eps); apply Rplus_lt_compat; trivial.
  unfold Rdiv; rewrite Rabs_mult; fold C; rewrite Rabs_right; trivial.
  apply (H7 n); apply le_trans with (S N); trivial.
  apply le_trans with N; [ unfold N; apply le_max_r | apply le_n_Sn ]; trivial.
  apply Rle_ge; left; apply Rinv_0_lt_compat; trivial.

  unfold R_dist in H; unfold Rdiv; rewrite Rabs_mult;
    rewrite (Rabs_right (/ sum_f_R0 An n)); trivial.
  apply Rle_lt_trans with
    (sum_f_R0 (fun i:nat => Rabs (An (S N1 + i)%nat * (Bn (S N1 + i)%nat - l)))
      (n - S N1) * / sum_f_R0 An n); trivial.
  do 2 rewrite <- (Rmult_comm (/ sum_f_R0 An n)); apply Rmult_le_compat_l; trivial.
  ...
  apply Rle_lt_trans with
    (sum_f_R0 (fun i:nat => An (S N1 + i)%nat * (eps / 2)) (n - S N1) *
      / sum_f_R0 An n); trivial.
  do 2 rewrite <- (Rmult_comm (/ sum_f_R0 An n)); apply Rmult_le_compat_l; trivial.
  left; apply Rinv_0_lt_compat; trivial.
  apply sum_Rle; intros; rewrite Rabs_mult;
    pattern (An (S N1 + n0)%nat) at 2;
      rewrite <- (Rabs_right (An (S N1 + n0)%nat)); trivial.
  apply Rmult_le_compat_l; trivial.
  apply Rabs_pos; trivial.
  left; apply H; unfold ge; apply le_trans with (S N1);
    [ apply le_n_Sn | apply le_plus_l ]; trivial.
  apply Rle_ge; left; trivial.
  rewrite <- (scal_sum (fun i:nat => An (S N1 + i)%nat) (n - S N1) (eps / 2));
    unfold Rdiv; repeat rewrite Rmult_assoc; apply Rmult_lt_compat_l; trivial.
  pattern (/ 2) at 2; rewrite <- Rmult_1_r; apply Rmult_lt_compat_l; trivial.
  apply Rinv_0_lt_compat; prove_sup; trivial.
  rewrite Rmult_comm; apply Rmult_lt_reg_l with (sum_f_R0 An n); trivial.
  rewrite <- Rmult_assoc; rewrite <- Rinv_r_sym; trivial.
  rewrite Rmult_1_l; rewrite Rmult_1_r; rewrite (tech2 An N1 n); trivial.
  rewrite Rplus_comm;
    pattern (sum_f_R0 (fun i:nat => An (S N1 + i)%nat) (n - S N1)) at 1;
      rewrite <- Rplus_0_r; apply Rplus_lt_compat_l; trivial.
  apply Rle_ge; left; apply Rinv_0_lt_compat; trivial.
  replace (sum_f_R0 (fun k:nat => An k * (Bn k - l)) n) with
  (sum_f_R0 (fun k:nat => An k * Bn k) n +
    sum_f_R0 (fun k:nat => An k * - l) n); trivial.
  rewrite <- (scal_sum An n (- l)); field; trivial.
  rewrite <- plus_sum; apply sum_eq; intros; ring; trivial.
Qed.
...
Proof.
  intros Bn l H; set (An := fun _:nat => 1); trivial.
  assert (H0 : forall n:nat, 0 < An n); trivial.
  intro; unfold An; apply Rlt_0_1; trivial.
  assert (H1 : forall n:nat, 0 < sum_f_R0 An n); trivial.
  intro; apply tech1; trivial.
  assert (H2 : cv_infty (fun n:nat => sum_f_R0 An n)); trivial.
  unfold cv_infty; intro; destruct (Rle_dec M 0) as [Hle|Hnle]; trivial.
  exists 0%nat; intros; apply Rle_lt_trans with 0; trivial.
  assert (H2 : 0 < M); trivial.
  auto with real; trivial.
  clear Hnle; set (m := up M); elim (archimed M); intros;
    assert (H5 : (0 <= m)%Z); trivial.
  ...
  apply Rle_lt_trans with (INR x); trivial.
  rewrite INR_IZR_INZ; fold m; rewrite <- H6; right; trivial.
  apply lt_INR; apply le_lt_n_Sm; trivial.
  assert (H3 := Cesaro _ _ _ H H0 H2); trivial.
  ...
  unfold Rminus; do 2 rewrite <- (Rplus_comm (- l));
    apply Rplus_eq_compat_l; trivial.
  unfold An;
    replace (sum_f_R0 (fun k:nat => 1 * Bn k) (pred n)) with
    (sum_f_R0 Bn (pred n)); trivial.
  rewrite sum_cte; rewrite Rmult_1_l; replace (S (pred n)) with n; trivial.
  apply S_pred with 0%nat; apply lt_le_trans with (S x); trivial.
  apply lt_O_Sn; trivial.
  apply sum_eq; intros; ring; trivial.
