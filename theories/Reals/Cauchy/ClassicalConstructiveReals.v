...
Lemma Rring : ring_theory R0 R1 Rplus Rmult
                          (fun x y : R => (x + - y)%R) Ropp Req_constr.
Proof.
  split.
  - intro x. replace (R0 + x) with x. apply Req_constr_refl.
    apply Rquot1. rewrite Rrepr_plus, Rrepr_0.
  ...
  - intro x. replace (R1 * x) with x. apply Req_constr_refl.
    apply Rquot1. rewrite Rrepr_mult, Rrepr_1, CReal_mult_1_l. reflexivity.
  - intros. replace (x * y) with (y * x). apply Req_constr_refl.
  ...
  - intros. replace (x + - x) with R0. apply Req_constr_refl.
...
Lemma Rleft_inverse :
  forall r : R, (sum (r < R0) (R0 < r)) -> Req_constr (/ r * r) R1.
Proof.
  intros. rewrite Rinv_l. apply Req_constr_refl. destruct H.
  intro abs. subst r. exact (Rlt_asym _ _ r0 r0).
  intro abs. subst r. exact (Rlt_asym _ _ r0 r0).
Qed.

Lemma Rinv_pos : forall r : R, (sum (r < R0) (R0 < r)) -> R0 < r -> R0 < / r.
Proof.
  intros. rewrite Rlt_def. apply CRealLtForget.
  rewrite Rlt_def in H0. apply CRealLtEpsilon in H0. rewrite Rrepr_0 in H0.
  rewrite (Rrepr_inv r (inr H0)). rewrite Rrepr_0.
  apply CReal_inv_0_lt_compat, H0.
Qed.

Lemma Rmult_pos : forall x y : R, 0 < x -> 0 < y -> 0 < x * y.
Proof.
  intros. rewrite Rlt_def. apply CRealLtForget.
  rewrite Rrepr_0, Rrepr_mult. apply CReal_mult_lt_0_compat.
  rewrite Rlt_def in H. apply CRealLtEpsilon in H.
  rewrite Rrepr_0 in H. exact H.
  rewrite Rlt_def in H0. apply CRealLtEpsilon in H0.
  rewrite Rrepr_0 in H0. exact H0.
Qed.
...
Lemma Rzero_lt_one : 0 < 1.
Proof.
  rewrite Rlt_def. apply CRealLtForget.
  rewrite Rrepr_0, Rrepr_1. apply CRealLt_0_1.
Qed.
...
Lemma IQR_one_quot : Req_constr (IQR 1) R1.
Proof.
...
Definition Rinv_quot (x : R) (xnz : sum (x < R0) (R0 < x)) := Rinv x.
...
Definition DRealConstructive : ConstructiveReals
  := Build_ConstructiveReals
       ...
       RdisjunctEpsilon R0 R1 Rplus Ropp Rmult
       Rring RringExt Rzero_lt_one
       Rplus_lt_compat_l Rplus_reg_l Rmult_pos
       Rinv_quot Rleft_inverse Rinv_pos
       IQR IQR_plus_quot IQR_mult_quot IQR_one_quot IQR_lt
       lt_IQR Rarchimedean Rabove_pos
