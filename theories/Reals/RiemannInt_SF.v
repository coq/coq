...
Proof.
  ...
  2:{ split with (pred x0); unfold E in H13; elim H13; intros; elim H12; intros;
  ...
  { intro H13; assert (H14 := H5 _ H13); lra. }
  ...
  2:{ rewrite H15 in H13; elim H12; assumption. }
...
Proof.
  ...
  { exists (cons a nil); exists nil; unfold adapted_couple_opt;
    ...
    1,2:simpl; rewrite <- H2; unfold Rmin,Rmax; case (Rle_dec a a); intro;
      reflexivity. }
  ...
  assert (H6 := H _ _ _ H5 H4); case (Req_dec t1 t2); intro Hyp_eq.
  { replace a with t2.
    { apply H6. }
  ...
  { exists (cons a (cons b nil)); exists (cons r1 nil);
    ...
    - intros; simpl in H8; inversion H8;
        [ simpl; assumption | elim (Nat.nle_succ_0 _ H10) ]. }
  assert (Hyp_min : Rmin t2 b = t2) by apply Rmin_left,H5.
...
Proof.
  ...
  assert (H19 : r3 = r5).
  2:{ elim H2; intros; assert (H22 := H20 0%nat); simpl in H22;
  ...
  assert (H19 := H7 1%nat); simpl in H19;
    ...
    intro.
  2:{ elim H2; clear H2; intros; assert (H23 := H22 1%nat); simpl in H23;
  ...
  1:apply (H23 (proj1 (Nat.succ_lt_mono _ _) (Nat.lt_0_succ _)) x).
  1,2:unfold open_interval; simpl; unfold x;
    unfold Rmin;destruct (Rle_dec r1 r0);lra.
Qed.
...
Proof.
  ...
  assert (H7 : r3 = r4 \/ r = r1). {
    ...
    unfold adapted_couple in H2, H; decompose [and] H; decompose [and] H2;
  ...
  elim H7; intro.
  2:{ simpl; rewrite H9; unfold Rminus; rewrite Rplus_opp_r;
      ...
      - assert (H10 : r = a).
        ...
        + rewrite <- H9; rewrite H10; apply StepFun_P7 with a r r3;
            ...
            | pattern a at 2; rewrite <- H10; pattern r at 2; rewrite H9;
              apply H2 ].
      - rewrite H5 in H3; unfold adapted_couple_opt; split; assumption. }
  simpl; elim H8; intro.
...
Proof.
  ...
  - rewrite Hyp_max; apply Rle_antisym.
    ...
    { symmetry; apply Nat.lt_succ_pred with 0%nat.
      apply Nat.neq_0_lt_0; red; intro;
  ...
  - intros; unfold constant_D_eq, open_interval; intros;
    ...
    + intros; elim H11; clear H11; intros; assert (H12 := H11);
      ...
      elim Hyp_cons; clear Hyp_cons; intros r [r0 Hyp_cons]; rewrite Hyp_cons;
        unfold FF; rewrite RList_P12.
      2:{ rewrite RList_P14; rewrite Hyp_cons in H8; simpl in H8; apply H8. }
      ...
      2:{ elim H10; intros; rewrite H14 in H15;
        ...
        assert
          (H15 :
            pos_Rl (cons_ORlist lf lg) i <
              ...
              pos_Rl (cons_ORlist lf lg) (S i)) by lra.
      rewrite (H11 _ H15); reflexivity.
    + assert (H11 : a < b). {
        apply Rle_lt_trans with (pos_Rl (cons_ORlist lf lg) i).
        2:apply Rlt_le_trans with (pos_Rl (cons_ORlist lf lg) (S i)).
      ...
      assert (H13 :  exists n : nat, I n). {
        ...
        2:{ apply Nat.neq_0_lt_0; red; intro; rewrite H13 in H5;
      ...
      { elim H14; clear H14; intros; unfold I in H14; elim H14; clear H14; intros;
        ...
        { inversion H18.
          ...
          rewrite H19 in H14; rewrite H5 in H14;
      ...
      apply Rlt_le_trans with (pos_Rl (cons_ORlist lf lg) (S i)).
      { assumption. }
      assert (H22 : (S x0 < length lf)%nat) by lia.
      ...
      * assert (H23 : pos_Rl (cons_ORlist lf lg) i < pos_Rl lf (S x0)) by auto with real.
...
Proof.
  ...
  - unfold constant_D_eq, open_interval; intros;
    ...
    assert (H23 : pos_Rl (cons_ORlist lf lg) i < pos_Rl lg (S x0)) by auto with real.
    clear a0; apply RList_P17; try assumption;
      [ apply RList_P2; assumption
...
Proof.
  intros a b l f g l1 (x0,(H0,(H1,(H2,(H3,H4)))))
         (x,(_,(_,(_,(_,H9))))).
  exists (FF l1 (fun x:R => f x + l * g x)); repeat split.
  1,2,3:assumption.
  ...
  - intros i H8 x1 H10; unfold open_interval in H10, H9, H4;
    ...
    { rewrite RList_P13.
      { rewrite <- H12; rewrite (H9 _ H8); try rewrite (H4 _ H8); lra. }
...
Proof.
  ...
  2:{ simpl in H; apply H with r0 b.
    ...
    + inversion H12.
      ...
      elim (RList_P20 _ H18); intros r1 [r2 [r3 H19]]; rewrite H19;
        change
          (f x = pos_Rl (map f (mid_Rlist (app (cons r2 r3) l2) r1)) i)
      ; rewrite RList_P12.
      2:{ rewrite RList_P14; rewrite H19 in H1; simpl in H1; simpl; apply H1. }
      induction  i as [| i Hreci].
      { assert (H20 := le_S_n _ _ H15); assert (H21 := Nat.le_trans _ _ _ H18 H20);
          elim (Nat.nle_succ_0 _ H21). }
      clear Hreci; rewrite RList_P13.
      2:{ simpl; rewrite H19 in H1; simpl in H1; apply Nat.succ_lt_mono; assumption. }
...
Proof.
  ...
  1:replace (RiemannInt_SF (mkStepFun pr2)) with
    match Rle_dec b c with
    | left _ => Int_SF lf2 l2
    | right _ => - Int_SF lf2 l2
    end.
  1:replace (RiemannInt_SF (mkStepFun pr3)) with
    match Rle_dec a c with
    | left _ => Int_SF lf3 l3
    | right _ => - Int_SF lf3 l3
    end.
  2,3,4:unfold RiemannInt_SF; case (Rle_dec _ _); intro;[|apply Ropp_eq_compat];
    (eapply StepFun_P17;[|apply StepFun_P1]);assumption.
  case (Rle_dec a b) as [Hle|Hnle]; case (Rle_dec b c) as [Hle'|Hnle']; case (Rle_dec a c) as [Hle''|Hnle''].
  - elim Hle; intro.
    1:elim Hle'; intro.
    + replace (Int_SF lf3 l3) with
        (Int_SF (FF (app l1 l2) f) (app l1 l2)).
      2:{ eapply StepFun_P17; [ apply (StepFun_P40 H H0 H1 H2) | apply H3 ]. }
