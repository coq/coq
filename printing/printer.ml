...
let universe_binders_with_opt_names orig names =
  let open Univ in
  let orig = UVars.AbstractContext.names orig in
...
let pr_universe_ctx sigma ?variance c =
  if !Detyping.print_universes && not (UVars.UContext.is_empty c) then
    fnl()++pr_in_comment (v 0 (UVars.pr_universe_context (Termops.pr_evd_level sigma) ?variance c))
  else
    mt()
...
let pr_abstract_universe_ctx sigma ?variance ?priv c =
  ...
  if !Detyping.print_universes && (not (UVars.AbstractContext.is_empty c) || has_priv) then
    ...
    let pub = (if has_priv then str "Public universes:" ++ fnl() else mt()) ++ v 0 (UVars.pr_abstract_universe_context prlev ?variance c) in
...
let pr_universe_instance_constraints evd inst csts =
  ...
  in
  str"@{" ++ UVars.Instance.pr prlev inst ++ pcsts ++ str"}"
