...
module Vernac_ =
  struct
    ...
    let parse_generic_tactic strm =
      ...
      let v = Pcoq.Entry.parse_token_stream mode.tactic_expr_entry strm in
      Genarg.(in_gen (rawwit mode.wit_tactic_expr) v)

    let command_entry =
      Pcoq.Entry.(of_parser "command_entry"
        { parser_fun = (fun _kwstate strm -> Pcoq.Entry.parse_token_stream (select_command_entry !command_entry_ref) strm) })

    let generic_tactic =
      Pcoq.Entry.(of_parser "generic_tactic"
