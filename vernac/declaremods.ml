...
module SynterpActions : ModActions with
  type env = unit with type typexpr = Constrexpr.constr_expr * Constrexpr.universe_decl_expr option =
struct
  type typexpr = Constrexpr.constr_expr * Constrexpr.universe_decl_expr option
  type env = unit
...
module ModVisitor(Actions : ModActions) = struct
...
let rec load_object i (prefix, obj) =
  ...
  | ModuleObject (id,sobjs) ->
    CDebug.debug_synterp (fun () -> Pp.(str"load_object module prefix=" ++ DirPath.print prefix.Nametab.obj_dir ++ str", name=" ++ Id.print id ++ spc ()));
...
and open_module f i obj_dir obj_mp sobjs =
  (* Printf.eprintf "Opening module\n"; *)
  Actions.open_module f obj_mp obj_dir i;
  (* If we're not a functor, let's iter on the internal components *)
  if sobjs_no_functor sobjs then begin
    let modobjs = ModObjs.get obj_mp in
    (* Printf.eprintf "Found %i subst objects, %i keep objects inside module\n" (List.length modobjs.module_substituted_objects) (List.length modobjs.module_keep_objects); *)
...
let cache_object (prefix, obj) =
  (* Printf.eprintf "cache_object %s\n" (string_of_path (fst name)); *)
  match obj with
...
module SynterpVisitor : ModVisitorS
  with type env = SynterpActions.env
  with type typexpr = Constrexpr.constr_expr * Constrexpr.universe_decl_expr option
  = ModVisitor(SynterpActions)

module InterpVisitor : ModVisitorS
  with type env = InterpActions.env
  with type typexpr = Constr.t * Univ.AbstractContext.t option
 = ModVisitor(InterpActions)

(** {6 Modules : start, end, declare} *)

type current_module_syntax_info = {
  cur_mp : ModPath.t;
  cur_typ : ((Constrexpr.constr_expr, Constrexpr.universe_decl_expr) module_struct_entry_gen * int option) option;
...
let end_module_core id (m_info : current_module_syntax_info) objects fs =
  CDebug.debug_synterp Pp.(fun () -> str"before end_module (synterp)" ++ Lib.State.print ());
...
let declare_module id args res mexpr_o fs =
  ...
  Summary.unfreeze_summaries ~partial:true fs; (* TODO check this *)
...
let intern_arg (acc, cst) (mbidl,(mty, base, kind, inl)) =
  ...
  let fold acc mbid =
    ...
    let dir = DirPath.make [id] in
    let mp = MPbound mbid in
    CDebug.debug_synterp (fun () -> Pp.(str"add_module_parameter mbid=" ++ str (ModPath.debug_to_string mp)));
...
let end_module_core id m_info objects fs =
  CDebug.debug_synterp Pp.(fun () -> str"before end_module (interp)" ++ Lib.State.print ());
  CDebug.debug_synterp (fun () -> Pp.(str "end_module"));
  let {Lib.substobjs = substitute; keepobjs = keep; anticipateobjs = special; } = objects in
...
let register_library dir (objs:library_objects) =
  CDebug.debug_synterp (fun () -> Pp.(str"register_library_syntax " ++ DirPath.print dir));
...
let import_modules ~export mpl =
  ...
  CDebug.debug_synterp (fun () -> Pp.(str"Import collected " ++ int (List.length objs) ++ str" objects (synterp)"));
...
let start_module export id args res =
  protect_summaries Summary.Stage.Interp (RawModOps.Interp.start_module export id args res)
...
let register_library dir cenv (objs:library_objects) digest univ =
  CDebug.debug_synterp (fun () -> Pp.(str"register_library " ++ DirPath.print dir));
  let mp = MPfile dir in
  let () =
    try
    ...
    with Not_found ->
      (CDebug.debug_synterp (fun () -> Pp.(str"Load module in global env"));
...
let import_modules ~export mpl =
  ...
  CDebug.debug_synterp (fun () -> Pp.(str"Import collected " ++ int (List.length objs) ++ str" objects (interp)"));
...
let end_library ~output_native_objects dir =
  ...
  let {Lib.substobjs = substitute_syntax; keepobjs = keep_syntax; anticipateobjs = _; } = lib_stack_syntax in
