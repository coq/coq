  ...
  | _ ->
    ...
    let pm, proof =
      ComFixpoint.do_mutually_recursive ~program_mode ~use_inference_hook:program_mode
        ~scope ?opaque ?clearbody ~kind:(Decls.IsProof kind) ~poly ?typing_flags
...
let vernac_fixpoint ~atts ~pm (rec_order,fixl) =
  ...
  with_obligations program_mode
    (fun pm -> ComFixpoint.do_mutually_recursive ?pm ~scope ?opaque ?clearbody ~kind:(IsDefinition Fixpoint) ~poly ?typing_flags ?user_warns ?using (CFixRecOrder rec_order, fixl))
    pm
...
let vernac_cofixpoint ~pm ~atts cofixl =
  ...
  with_obligations program_mode
    (fun pm -> ComFixpoint.do_mutually_recursive ?pm ~scope ?opaque ?clearbody ~kind:(IsDefinition CoFixpoint) ~poly ?typing_flags ?user_warns ?using (CCoFixRecOrder, cofixl))
    pm
