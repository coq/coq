...
let find_mutually_recursive_statements sigma thms =
    ...
    let maybecofix = not (List.is_empty same_indccl) in (* all conclusions are coinductive *)
    let possible_guard =
    ...
    if not maybecofix && List.is_empty possible_guard then
      CErrors.user_err Pp.(str
         ("Cannot find common (mutual) inductive premises or coinductive" ^
          " conclusions in the statements."));
    ((maybecofix, possible_guard), None)
