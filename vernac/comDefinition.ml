...
let do_definition ?hook ~name ?scope ?clearbody ~poly ?typing_flags ~kind ?using ?user_warns udecl bl ?arity red_option c ctypopt =
  ...
  let cinfo = Declare.CInfo.make ~name ~impargs ~typ:types ?arity () in
...
let do_definition_program ?hook ~pm ~name ~scope ?clearbody ~poly ?typing_flags ~kind ?using ?user_warns udecl bl ?arity red_option c ctypopt =
  ...
  let pm, _ =
    let cinfo = Declare.CInfo.make ~name ~typ ~impargs ?arity () in
