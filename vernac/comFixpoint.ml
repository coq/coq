...
let interp_rec_annot fixl ctxl (structonly, rec_order) = match rec_order with
  (* If recursive argument was not given by user, we try all args.
     An earlier approach was to look only for inductive arguments,
     but doing it properly involves delta-reduction, and it finally
     doesn't seem to worth the effort (except for huge mutual
     fixpoints ?) *)
  | CFixRecOrder fix_orders -> Decls.Fixpoint, (false, List.map3 (find_rec_annot ~structonly) fixl ctxl fix_orders)
  | CCoFixRecOrder -> Decls.CoFixpoint, (true, List.map (fun _ -> []) fixl)
  | CUnknownRecOrder -> Decls.Definition, (true, List.map2 (fun fix ctx -> find_rec_annot ~structonly fix ctx None) fixl ctxl)
(*  | CNoRecOrder -> (false, [])*)
...
let do_mutually_recursive ?pm ?scope ?clearbody ~poly ?typing_flags ?user_warns ?using (rec_order, fixl)
  ...
  let uctx = Evd.evar_universe_context sigma in
  let info = Declare.Info.make ?scope ?clearbody ~kind ~poly ~udecl ?typing_flags ?user_warns ~ntns:fix.fixntns ?using () in
