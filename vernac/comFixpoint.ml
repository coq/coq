...
let encapsulate_Fix_sub env sigma recname ctx body ccl (extradecl, rel, relargty, measure_body) =
  ...
  let sigma, letbinders, {telescope_type = argtyp; telescope_value = argvalue} =
    telescope env sigma ctx in
  let curried_ctx = letbinders @ [LocalAssum (make_annot (Name argname) Sorts.Relevant, argtyp)] in
  (* The function measure has type [argtyp -> relargty] *)
  let measure = it_mkLambda_or_LetIn measure_body curried_ctx in
  ...
  let rel_measure = mkApp (comb, [| argtyp; relargty; rel; measure |]) in
  (* The statement that rel_measure is well-founded *)
  let sigma, wf_term = well_founded sigma in
  let wf_type = mkApp (wf_term, [| argtyp ; rel_measure |]) in
  (* A combinator building [rel (measure x) (measure y)] *)
  let curried_measure_body = it_mkLambda_or_LetIn measure_body letbinders in
  let make_applied_rel x y =
    mkApp (rel, [| Vars.subst1 x curried_measure_body; Vars.subst1 y curried_measure_body |]) in
  (* Conclusion of fixpoint in currified context *)
  let curried_ccl = it_mkLambda_or_LetIn ccl letbinders in
  ...
  let sigma, fix_sub =
    ...
    let ccl_pred = mkLambda (make_annot (Name argname) Sorts.Relevant, argtyp, curried_ccl) in
    let def = mkApp (fix_sub_term, [| argtyp ; rel_measure ; wf_proof ; ccl_pred |]) in
    Typing.solve_evars env sigma def in
  let arg = RelDecl.LocalAssum (make_annot (Name argname) Sorts.Relevant, argtyp) in
  let argid' = Id.of_string (Id.to_string argname ^ "'") in
  let sigma, wfa =
    let sigma, ss_term = mkSubset sigma (Name argid') argtyp (make_applied_rel (mkRel 1) (mkRel 2)) in
    sigma, RelDecl.LocalAssum (make_annot (Name argid') Sorts.Relevant, ss_term) in
  let sigma, fix_sub_F_sub_ctx =
    let sigma, proj = Evd.fresh_global (Global.env ()) sigma (delayed_force build_sigma).Coqlib.proj1 in
    let wfargpred = mkLambda (make_annot (Name argid') Sorts.Relevant, argtyp, make_applied_rel (mkRel 1) (mkRel 3)) in
    ...
    let smaller_arg = RelDecl.LocalAssum (make_annot (Name recname') Sorts.Relevant,
                                          ccl_on_smaller_arg) in
    sigma, Vars.lift_rel_context 1 letbinders @ smaller_arg :: [arg] in
  let sigma, curryfier_body, curryfier_ty =
    (* In curried_context where the function argument of Fix_sub (argid'), is inserted,
       that is, all expanded: [recarg;argid';letbinders], build the curryfying combinator
       [fun ctx (recproof : rel (measure ctx) (measure curried_context)) => argid' (argvalue,recproof)]
       of type
       [forall ctx (recproof : rel (measure ctx) (measure curried_context)) => ccl] *)
    let sigma, intro = Evd.fresh_global (Global.env ()) sigma (delayed_force build_sigma).Coqlib.intro in
    let app =
      let wfpred = mkLambda (make_annot (Name argid') Sorts.Relevant, argtyp, make_applied_rel (mkRel 1) (mkRel (2 * len + 4))) in
      (* Build the sig pair [exist _ argvalue recproof] *)
      let arg = mkApp (intro, [| argtyp; wfpred; Vars.lift 1 argvalue; mkRel 1 |]) in
  ...
  let body_ctx = RelDecl.LocalDef (make_annot (Name recname) Sorts.Relevant, curryfier_body, curryfier_ty) :: fix_sub_F_sub_ctx in
  ...
  sigma, curried_ctx, argvalue, mkApp (fix_sub, [|intern_body_lam|])
...
let build_wellfounded env sigma poly udecl recname ctx body ccl impls rel_measure =
  ...
  let sigma, curried_ctx, argvalue, def = encapsulate_Fix_sub env sigma recname ctx body ccl rel_measure in
  (* Turn everything to constr *)
  let ctx = Evarutil.nf_rel_context_evar_with_relevance sigma ctx in
  let curried_ctx = Evarutil.nf_rel_context_evar_with_relevance sigma curried_ctx in
  let ccl = Evarutil.nf_evar sigma ccl in
  let argvalue = Evarutil.nf_evar sigma argvalue in
  (* Decide if using a curryfied indirection via recname_func *)
  let hook, recname, impls, typ =
    if len > 1 then
      ...
      let hook { Declare.Hook.S.dref; uctx; _ } =
        ...
        let body = it_mkLambda_or_LetIn (mkApp (h_body, [|argvalue|])) ctx in
        let ty = it_mkProd_or_LetIn ccl ctx in
      ...
      in
      let typ = it_mkProd_or_LetIn ccl curried_ctx in
      Some (Declare.Hook.make hook), name, [], typ
    else
      let typ = it_mkProd_or_LetIn ccl ctx in
      None, recname, impls, typ
  in
  RetrieveObl.check_evars env sigma;
  (* Replace evars by obligations *)
  let obls, _, body, typ = RetrieveObl.retrieve_obligations env recname sigma 0 def typ in
  sigma, recname, body, typ, impls, obls, hook
...
let interp_rec_annot ~program_mode ~function_mode fixl ctxl rec_order =
  let fixkind, (maybecofix, fixwf_guard) =
    match rec_order with
  ...
  let fixwf, possible_guard = List.split fixwf_guard in
  (fixkind, fixwf, (maybecofix, possible_guard))
...
type ('constr, 'types) recursive_preentry =
  (Id.t list * Sorts.relevance list * 'constr option list * 'types list * EConstr.rel_context list * Impargs.manual_implicits list) *
...
let encapsulate env sigma r t =
  ...
  let fix_proto_relevance = Sorts.Relevant in
...
type 'constr fix_data = {
  fixnames : Names.Id.t list;
  fixrs    : Sorts.relevance list;
  ...
  | Some (r, measure) ->
    ...
    let sigma, after, extradecl =
      if program_mode then
        ...
        let extradecl = RelDecl.LocalAssum (make_annot (Name recproofid) Sorts.Relevant, applied_rel_measure) in
  ...
  let sigma, rec_sign =
    List.fold_left4
      (fun (sigma, rec_sign) id r t (_,extradecl) ->
         let sigma, r, ccl = if program_mode && List.is_empty extradecl then encapsulate env sigma r t else sigma, r, t in
...
let finish_program env sigma rec_sign possible_guard poly udecl = function
...
let do_mutually_recursive ?pm ?scope ?clearbody ~poly ?typing_flags ?user_warns ?using (rec_order, fixl)
  ...
  let sigma, ({fixdefs=bodies;fixrs;fixwfs} as fix), obls, hook =
    match pm with
    | Some pm -> finish_program env sigma rec_sign possible_guard poly udecl fix
    | None -> finish_regular env sigma fix in
  let using = make_using ?using fix in
  let uctx = Evd.evar_universe_context sigma in
  let info = Declare.Info.make ?scope ?clearbody ~kind ~poly ~udecl ?hook ?typing_flags ?user_warns ~ntns:fix.fixntns ?using () in
  ...
  | Some pm ->
    ...
    | [Some _], [Some body], [cinfo], [obls] ->
      ...
      let pm, _ = Declare.Obls.add_definition ~pm ~cinfo ~info ~opaque:false ~body ~uctx obls in
      Some pm, None
    | _ ->
      (* Program Fixpoint struct *)
      let bodies = List.map Option.get bodies in
      Some (Declare.Obls.add_mutual_definitions ~pm ~cinfo ~info ~opaque:false ~uctx ~bodies ~possible_guard obls), None)
  | None ->
    try
      let bodies = List.map Option.get bodies in
      (* All bodies are defined *)
      let _ : GlobRef.t list =
        Declare.declare_mutual_definition ~cinfo ~info ~opaque:false ~uctx
          ~possible_guard ~bodies:(bodies,fixrs)
      in
      None, None
    with Option.IsNone ->
      (* At least one undefined body *)
      let evd = Evd.from_ctx uctx in
      let lemma = Declare.Proof.start_mutual_definition_with_initialization ~info ~cinfo
          ~bodies ~possible_guard evd in
      None, Some lemma
