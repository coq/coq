...
let interp_fix_context ~program_mode ~cofix env sigma fix =
  ...
  let sigma, (impl_env, ((env', ctx), imps)) =
    interp_context_evars ~flags:Pretyping.{all_no_fail_flags with program_mode} env sigma before
  in
  let sigma, (impl_env', ((env'', ctx'), imps')) =
    interp_context_evars ~flags:Pretyping.{all_no_fail_flags with program_mode} env' sigma after ~impl_env
  in
...
let interp_fix_body ~program_mode env_rec sigma impls (_,ctx) fix ccl =
  ...
  Option.cata (fun body ->
    ...
    let sigma, body =
      interp_constr_evars_gen ~flags:Pretyping.{all_no_fail_flags with program_mode}
