...
let get_hide_obligations =
  Goptions.declare_bool_option_and_ref ~depr:false
    ~name:"Hidding of Program obligations" ~key:["Hide"; "Obligations"]
    ~value:false
...
let declare_definition ~ontop prg =
  ...
  DeclareDef.declare_definition ~ontop prg.prg_name prg.prg_kind ce ubinders
    prg.prg_implicits ?hook_data

let rec lam_index n t acc =
  match Constr.kind t with
  | Lambda (Name n', _, _) when Id.equal n n' -> acc
...
let declare_mutual_definition l =
  ...
  let fixdecls =
    (Array.of_list (List.map (fun x -> Name x.prg_name) l), arrrec, recvec)
  in
