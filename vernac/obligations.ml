...
let rec solve_obligation ~ontop prg num tac =
  ...
  let lemma = Lemmas.start_lemma ~ontop ~sign:prg.prg_sign obl.obl_name kind evd (EConstr.of_constr obl.obl_type) ~terminator ~hook in
  let lemma = fst @@ Lemmas.by !default_tactic lemma in
  let lemma = Option.cata (fun tac -> Lemmas.pf_map Proof_global.(set_endline_tactic tac) lemma) lemma tac in
  lemma
