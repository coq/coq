...
let declare_mutual_definitions ~info ~cinfo ~uctx ~bodies ~possible_guard ?using () =
  ...
  in
  let csts = CList.map2
      (fun CInfo.{ name; typ; impargs; opaque } body ->
...
let declare_mutual_definitions ~pm l =
  ...
  let defobl x =
    ...
    let def = (x.prg_reduce term, r, x.prg_reduce typ, (x.prg_cinfo.CInfo.impargs, x.prg_cinfo.CInfo.opaque)) in
  ...
  let fixitems = List.map2 (fun (d, relevance, typ, (impargs, opaque)) name -> CInfo.make ~name ~typ ~impargs ~opaque ()) defs first.prg_deps in
  ...
  let kns =
    declare_mutual_definitions ~info:first.prg_info
      ~uctx:first.prg_uctx ~bodies:(fixdefs, fixrs) ~possible_guard
...
let build_constant_by_tactic ~name ?warn_incomplete ~sigma ~sign ~poly (typ : EConstr.t) tac =
  ...
  let cinfo = [CInfo.make ~name ~typ:typ_ ~opaque:None ()] in
