...
let declare_mutual_definitions ~info ~cinfo ~opaque ~uctx ~bodies ~possible_guard ?using () =
  ...
  let csts = CList.map2
      (fun CInfo.{ name; typ; impargs } (body, _) ->
         let univs, body = make_univs_immediate ~poly ~opaque ~uctx ~udecl ~eff:Evd.empty_side_effects body (Some typ) in
...
let prepare_definition ~info ~opaque ?using ~name ~body ~typ sigma =
  ...
  let univs, body = make_univs_immediate ~poly ~opaque ~uctx ~udecl ~eff:Evd.empty_side_effects body typ in
