...
let pf_map f ( { proof; terminator; hook }, psl) = ( { proof = f proof; terminator; hook } , psl)
let pf_fold f (ps, _) = f ps.proof
let pstate_map f (pf, pfl) = (f pf, List.map f pfl)

let with_current_proof f ( { proof; terminator; hook }, psl) =
  let proof, res = Proof_global.with_current_proof f proof in
  ({ proof; terminator; hook }, psl), res
...
let by tac ({ proof; terminator; hook }, psl) =
...
let start_proof ~ontop id ?pl kind sigma ?terminator ?sign ?(compute_guard=[]) ?hook c =
  ...
  push ~ontop { proof ; terminator; hook }
...
let save_proof_proved ?proof ?pstate ~opaque ~idopt =
  ...
  let proof_obj,terminator, hook =
    ...
    | None ->
      (* XXX uh! *)
      let { proof; terminator; hook }, _ = Option.get pstate in
