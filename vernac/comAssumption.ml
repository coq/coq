...
let interp_context_gen ~poly ~program_mode ~kind ~autoimp_enable ~coercions env sigma l =
...
let do_assumptions ~program_mode ~poly ~scope ~kind ?user_warns ~inline l =
  ...
  let sigma, ctx = interp_context_gen ~poly ~program_mode ~kind ~autoimp_enable:true ~coercions env sigma ctx in
...
let do_context ~program_mode ~poly ctx =
  ...
  in
  let sigma, ctx = interp_context_gen ~poly ~program_mode ~kind:Context ~autoimp_enable:false ~coercions:Id.Set.empty env sigma ctx in
...
let interp_context env sigma ctx =
  ...
  let sigma, ctx = interp_context_gen ~poly:true ~program_mode:false ~kind:Context ~autoimp_enable:false ~coercions:Id.Set.empty env sigma ctx in
