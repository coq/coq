...
let declare_assumption ~ontop is_coe (local,p,kind) (c,ctx) pl imps impl nl {CAst.v=ident} =
match local with
| Discharge when Lib.sections_are_opened () ->
  ...
  let () =
    if not !Flags.quiet && Option.has_some ontop then
...
let declare_assumptions ~ontop idl is_coe k (c,uctx) pl imps nl =
  let refs, status, _ =
    List.fold_left (fun (refs,status,uctx) id ->
      let ref',u',status' =
        declare_assumption ~ontop is_coe k (c,uctx) pl imps false nl id in
...
let do_assumptions ~ontop ~program_mode kind nl l =
  ...
  pi2 (List.fold_left (fun (subst,status,uctx) ((is_coe,idl),t,imps) ->
      ...
      let refs, status' = declare_assumptions ~ontop idl is_coe kind (t,uctx) ubinders imps nl in
      let subst' = List.map2
...
let context ~ontop poly l =
  ...
  let fn status (id, b, t) =
    ...
    if Lib.is_modtype () && not (Lib.sections_are_opened ()) then
      ...
      | None ->
        pi3 (declare_assumption ~ontop false decl (t, univs) UnivNames.empty_binders [] impl
               Declaremods.NoInline (CAst.make id))
      | Some b ->
        let decl = (Discharge, poly, Definition) in
        let entry = Declare.definition_entry ~univs ~types:t b in
        let _gr = DeclareDef.declare_definition ~ontop id decl entry UnivNames.empty_binders [] in
