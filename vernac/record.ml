...
let typecheck_params_and_fields def poly udecl ps (records : DataI.t list) : tc_result =
  ...
  let env0 = if not poly && not def && is_template && env0.env_typing_flags.cumulative_prop then Environ.set_universes_lbound env0 UGraph.Bound.Prop else env0 in
  ...
  let fold sigma (typ, esort) (_, newfs) =
    ...
    if not def
    && (is_impredicative_sort env0 sort ||
        (Sorts.is_prop univ && not (Sorts.is_sprop sort)))
    then
      sigma, (sort, typ)
    else
