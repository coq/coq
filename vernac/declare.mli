...
module Proof : sig
  ...
  (** Qed or Define a proof. This API expects to run in an environment where
      side effects generated by the tactics which build the proof are present,
      that is the state has not been rewound to the proof start (as the STM
      does).  *)
  val save
    : pm:OblState.t
    -> proof:t
  ...
  (* The STM rewinds the system state between closing the proof and saving it, both in
     the transparent and opaque case. These APIs expect the caller to do so.
     If proof construction generated side effects, then not respecting this contract
     results in assertion (from the kernel) that the side effects are already
     present *)
