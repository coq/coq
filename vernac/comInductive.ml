...
let interp_cstrs env (sigma, ind_rel) impls params ind arity =
  ...
  let interp_cstr sigma ctyp =
    ...
    let sigma_with_model_evars, model = model_conclusion concl_env sigma ind_rel params (Context.Rel.length ctx) arity_indices in
    (* unify the expected with the provided conclusion *)
    (* If unification fails, the constructor is ill-formed, which we ignore for now and let the kernel complain about it later. *)
    (* let sigma = Option.default sigma (Evarconv.conv concl_env sigma_with_model_evars concl model) in *)
    let sigma =
      try Evarconv.the_conv_x concl_env concl model sigma_with_model_evars
