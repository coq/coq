...
let interp_rule (pattern, rhs) =
  ...
  let evd, Info rr_info, pattern, j_pat = Pretyping.eval_pretyper_pattern env evd pattern in

  let evd = Evd.minimize_universes ~except_qvars:(Sorts.QVar.Map.domain rr_info.qualities) evd in
  ...
  let uctx_pre = Evd.evar_universe_context evd in
  ...
  let () = UState.check_uctx_impl ~fail uctx_pre (Evd.evar_universe_context evd) in
