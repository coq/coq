    ...
    let refresh_template env sigma resj =
      ...
      | App (f,args) ->
        if Termops.is_template_polymorphic_ind !!env sigma f then
          ...
          let sigma, c = Evarsolve.refresh_universes ?allowed_evars (Some true) !!env sigma c in
...
struct
  ...
  let pretype_instance env evd sigma ?loc ref u =
    ...
    | None ->
      ...
      evd, sigma, mask, u
    | Some l -> pretype_instance_mask ?loc evd sigma l
  ...
  and pretype_ref (ref, u) =
    ...
    let p, j =
      match ref with
      | IndRef ind ->
          PInd (ind, mask), Typeops.judge_of_inductive !!env (ind, u)
      | ConstructRef c ->
          PConstr (c, mask), Typeops.judge_of_constructor !!env (c, u)
      | ConstRef c ->
          if not @@ Environ.is_symbol !!env c then user_err ?loc Pp.(str"Invalid pattern: " ++ GlobRef.print ref ++ str".");
          PSymbol (c, mask), Typeops.judge_of_constant !!env (c, u)
  ...
  and pretype_sort s =
    ...
    let j = make_judge (mkSort s) (Typeops.type_of_sort ~under_typing:true s) in
    evd, sigma, PSort ps, j
  ...
  and pretype_prod (name, bk, c1, c2) =
    ...
    let resj = make_judge (mkProd (binder, jdom.uj_val, jcod.uj_val)) (mkSort @@ Typeops.sort_of_product !!env ~under_typing:true sdom scod) in
    evd, sigma, PProd (binder.binder_name, pdom, (qdom, udom), pcod, (qcod, ucod)), resj
  ...
  and pretype_lettuple (nal, (na, po), c, d) =
    fun ?loc env evd sigma ->
    user_err ?loc Pp.(str"Invalid pattern (let-destruct patterns not supported).")
  ...
  and pretype_cases (sty, po, tml, eqns)  =
    ...
    let (evd, sigma, pp, p) =
      ...
      let evd, sigma, pp, j_p = eval_pretyper_arg_pattern (mkSort sr, Relevant) p_env evd sigma ret in
      let pp = (Array.map_of_list Context.Rel.Declaration.get_name pctx, pp) in
      let p = Array.map_of_list (Context.Rel.Declaration.get_annot %> Context.map_annot_relevance (ERelevance.kind evd)) pctx, j_val j_p in
      (evd, sigma, pp, p)
    in
    let rp = Sorts.RelevanceVar qr in

    let brs = NoDupArray.make (Array.length mip.mind_nf_lc) in
    ...
    let build_one_branch i (evd, sigma) (loc, brnas, br) =
      ...
      let evd, sigma, pbr, j_br = eval_pretyper_arg_pattern (brty, rp) br_env evd sigma br in
      let pbr = Array.map_of_list Context.Rel.Declaration.get_name bctx, pbr in
      let br = Array.map_of_list (Context.Rel.Declaration.get_annot %> Context.map_annot_relevance (ERelevance.kind evd)) bctx, j_val j_br in
      (evd, sigma), (pbr, br)
    in
    ...
    evd, sigma, PCase (pc, ind, annots, (pp, (qr, ur)), pbrs), make_judge body ty



  and pretype_if (c, (na, po), b1, b2) =
    fun ?loc env sigma ->
    user_err ?loc Pp.(str"Invalid pattern (if/then/else patterns not supported).")
  ...
  and pretype_int i =
    fun ?loc env evd sigma ->
    let resj = Typeops.judge_of_int !!env i in
    evd, sigma, PInt i, resj

  and pretype_float f =
    fun ?loc env evd sigma ->
    let resj = Typeops.judge_of_float !!env f in
    evd, sigma, PFloat f, resj

  and pretype_string s =
    fun ?loc env evd sigma ->
    let resj = Typeops.judge_of_string !!env s in
  ...
  let push_constraints env sigma =
    let push_evar_definitions sigma defs =
      Evar.Map.fold (fun evk eqs (sigma, defs, flag) ->
        let (ctx, _, _, na) = Evar.Map.find evk sigma.evar_map in
        if Name.is_name na then sigma, defs, flag else
        match List.find_opt (fun (pb, c) -> pb <> LESS && noccur_evar (Environ.push_rel_context ctx env) sigma defs evk c) eqs with
        | None -> sigma, defs, flag
        | Some (_, def) ->
          let sigma, defs = add_evar_definition env sigma defs evk def in
          sigma, defs, true
        ) sigma.evar_candidates (sigma, defs, false)
    in
    ...
    let rec round sigma defs =
      let sigma, defs, b1 = push_evar_definitions sigma defs in
      ...
      if b1 || b2 || b3 then
        ...
        sigma, defs
    in
    let sigma, defs = round sigma Evar.Map.empty in
  ...
  let merge_into_evd evd defs qcstrs ucstrs =
    ...
    let evd = Evar.Map.fold (fun evk def evd ->
...
let eval_pretyper_pattern env evd c =
  ...
  let evd, sigma, p, j = RRPattern.eval_pretyper_pattern env evd sigma c in

  let sigma, defs, ucstrs = RRPattern.push_constraints !!env sigma in
  ...
  let evd = RRPattern.merge_into_evd evd defs qcstrs ucstrs in
  let evar_map = nf_evar_map sigma.evar_list sigma.qgraph defs sigma.evar_map in
  evd, Declarations.Info { qualities = sigma.qualities; univs = sigma.univs; evars = sigma.evar_list; qgraph; qabove_prop; ucstrs; evar_map; evar_defs = defs}, p, j
