...
let rec stack_concat stk1 stk2 =
  match stk1 with
    ...
    | PROJ (p,unf,pinfo,stk1') -> PROJ (p,unf,pinfo,stack_concat stk1' stk2)
...
let rec norm_head info env t stack =
  ...
  | Proj (p, unf, c) ->
    ...
    let pinfo = Environ.lookup_projection p (info_env info.infos) in
      norm_head info env c (PROJ (p, unf', pinfo, stack))
...
and cbv_stack_value info env = function
  ...
  | (LAM (nlams,ctxt,b,env), APP (args, stk))
    ...
    | (CONSTR(((sp,n),u),[||]), APP(args,PROJ(p,unf,pi,stk)))
        when red_set (info_flags info.infos) fMATCH && unf ->
...
let rec apply_stack info t = function
  ...
  | CASE (ty,br,ci,env,st) ->
      apply_stack info
        ...
        st
  | PROJ (p, unf, pinfo, st) ->
       apply_stack info (mkProj (p, unf, t)) st
