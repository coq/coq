...
let pr_key = function
  | ConstKey sp -> Names.pr_con sp
  | VarKey id -> Pp.str (string_of_id id)
...
and norm_head_ref k info env stack normt =
  if red_set_ref (info_flags info) normt then
    match ref_value_cache info normt with
      | Some body ->
         if !debug_cbv then Pp.msgnl Pp.(str "Unfolding " ++ pr_key normt);
         strip_appl (shift_value k body) stack
      | None ->
         if !debug_cbv then Pp.msgnl Pp.(str "Not unfolding " ++ pr_key normt);
