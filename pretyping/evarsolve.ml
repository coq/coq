...
let check_evar_instance evd evk1 body conv_algo =
  ...
  in
  match conv_algo true evenv evd Reduction.CUMUL ty concl with
  | Success evd -> evd
  | UnifFailure _ -> raise (IllTypedInstance (evenv,ty,concl))
...
let reconsider_unif_constraints conv_algo evd =
  ...
  List.fold_left
    (fun p (pbty,env,t1,t2 as x) ->
       match p with
       | Success evd ->
           (match conv_algo true env evd pbty (EConstr.of_constr t1) (EConstr.of_constr t2) with
           | Success _ as x -> x
