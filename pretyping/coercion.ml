...
let new_flexible_sort env sigma =
  let sigma, s = Evd.new_sort_variable Evd.univ_flexible sigma in
  let sigma, hole = GlobEnv.new_evar env sigma ~relevance:ERelevance.relevant (EConstr.mkSort s) in
  (sigma, hole, s)

let default_hypnaming () =
  RenameExistingBut (VarSet.variables (Global.env ()))
...
let apply_coercion_args env sigma isproj bo ty arg arg_ty nparams =
  ...
  let rec apply_rec sigma acc typ nparams =
    if nparams <= 0 then sigma, List.rev acc, typ else
      ...
      let sigma, x = GlobEnv.new_evar env sigma c1 in
...
let make_existential ?loc ?(opaque = not (get_proofs_transparency ())) na env sigma c =
  ...
  let sigma, v = GlobEnv.new_evar env sigma ~src c in
...
let coerce ?loc env sigma (x : EConstr.constr) (y : EConstr.constr)
  ...
  and coerce' env sigma x y : evar_map * (evar_map -> EConstr.constr -> evar_map * EConstr.constr) option =
    ...
    | Prod (name, a, b), Prod (name', a', b') ->
      ...
      in
      let hypnaming = default_hypnaming () in
      let _, env' = GlobEnv.push_rel ~hypnaming sigma (LocalAssum (name', a')) env in
         ...
         then
           ...
           then
             begin
               ...
               let hypnaming = default_hypnaming () in
               let _, env' = GlobEnv.push_rel ~hypnaming sigma
...
let unify_product env sigma typ =
  ...
  | Evar ev ->
    ...
    Inl sigma
  | _ ->
    let sigma, domain_hole, s = new_flexible_sort env sigma in
    let r = ESorts.relevance_of_sort s in
    let na = make_annot Anonymous r in
    let hypnaming = default_hypnaming () in
    let _, env' = GlobEnv.push_rel ~hypnaming sigma (Context.Rel.Declaration.LocalAssum (na, domain_hole)) env in
    let sigma, codomain_hole, _ = new_flexible_sort env' sigma in
    ...
    try
      let sigma, v', v'_ty, trace =
        ...
        with Not_found ->
          ...
          let sigma, x = GlobEnv.new_evar env sigma target_type in
  ...
  with UnableToUnify (best_failed_sigma,e) ->
    ...
    with NoCoercion as exn ->
      ...
      | Prod (name,t1,t2), Prod (_,u1,u2) ->
          ...
          let open Context.Rel.Declaration in
          let hypnaming = default_hypnaming () in
          let _, env1 = GlobEnv.push_rel ~hypnaming sigma (LocalAssum (name,u1)) env in
          let (sigma, v1, trace1) =
