...
let rec elt = function
  | Leaf k -> k
  | Empty | Branch (_, _,_,_) -> raise Not_found


let rec choose_remove = function
    | Empty -> assert false
    | Leaf j -> j,Empty
    | Branch (p,m,t0,t1) -> 
	let h,r = choose_remove t0 in
	h,branch (p, m, r, t1)

let case s f1 f2 = match s with
  | Empty -> f1()
  | Leaf k -> f2 k Empty
  | Branch (p,m,t0,t1) -> 
      let h,r = choose_remove t0 in 
      f2 h (branch (p, m, r, t1))

let case1 f0 f1 f s = match s with
  | Empty -> f0()
  | Leaf k -> f1 k
  | _ -> f ()
