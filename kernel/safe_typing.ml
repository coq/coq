...
let add_constant kn ce env =
  let _ =
    try
      let _ = lookup_constant kn env in
      error ("constant "^string_of_kn kn^" already declared")
    with Not_found -> () in
...
let add_mind kn mie env =
  let _ =
    try 
      let _ = lookup_mind kn env in
      error ("inductive "^string_of_kn kn^" already declared")
    with Not_found -> () in
