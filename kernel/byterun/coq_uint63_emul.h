...
#define DECLARE_NULLOP(name)                                                   \
  value uint63_##name() {                                                      \
    ...
    CAMLparam0();                                                              \
    if (!cb)                                                                   \
      cb = caml_named_value("uint63 " #name);                                  \
    CAMLreturn(*cb);                                                           \
  }
...
#define CALL_UNOP(name, x)                                                     \
  do {                                                                         \
    CALL_UNOP_NOASSIGN(name, x);                                               \
    accu = uint63_return_value__;                                              \
  } while (0)

#define CALL_PREDICATE(r, name, x)                                             \
  do {                                                                         \
    CALL_UNOP_NOASSIGN(name, x);                                               \
...
#define CALL_BINOP(name, x, y)                                                 \
  do {                                                                         \
    CALL_BINOP_NOASSIGN(name, x, y);                                           \
    accu = uint63_return_value__;                                              \
  } while (0)

#define CALL_RELATION(r, name, x, y)                                           \
  do {                                                                         \
    CALL_BINOP_NOASSIGN(name, x, y);                                           \
    (r) = Int_val(uint63_return_value__);                                      \
  } while (0)
...
#define CALL_TEROP(name, x, y, z)                                              \
  do {                                                                         \
    ...
    uint63_return_value__ =                                                    \
        uint63_##name##_ml(uint63_arg_x__, uint63_arg_y__, uint63_arg_z__);    \
    Restore_after_gc;                                                          \
    accu = uint63_return_value__;                                              \
  } while (0)
...
#define Uint63_mulc(x, y, h)                                                   \
  do {                                                                         \
    value uint63_return_value__;                                               \
    value uint63_arg_x__ = (x);                                                \
    value uint63_arg_y__ = (y);                                                \
    Setup_for_gc;                                                              \
    uint63_return_value__ = uint63_mulc_ml_ml(uint63_arg_x__, uint63_arg_y__); \
    Restore_after_gc;                                                          \
