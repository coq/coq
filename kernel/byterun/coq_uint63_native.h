...
/* 2^63 * y + x as a value */
// #define Val_intint(x,y) ((value)(((uint64_t)(x)) << 1 + ((uint64_t)(y) <<
// 64)))
...
#define Uint63_addcarry(x, y)                                                  \
  (accu = (value)((uint64_t)(x) + (uint64_t)(y) + 1))
#define Uint63_sub(x, y) (accu = (value)((uint64_t)(x) - (uint64_t)(y) + 1))
#define Uint63_subcarry(x, y)                                                  \
  (accu = (value)((uint64_t)(x) - (uint64_t)(y) - 1))
#define Uint63_mul(x, y)                                                       \
  (accu = Val_long(uint63_of_value(x) * uint63_of_value(y)))
#define Uint63_div(x, y)                                                       \
  (accu = Val_long(uint63_of_value(x) / uint63_of_value(y)))
#define Uint63_mod(x, y)                                                       \
  (accu = Val_long(uint63_of_value(x) % uint63_of_value(y)))
#define Uint63_divs(x, y)                                                      \
  (accu = Val_long(int63_of_value(x) / int63_of_value(y)))
#define Uint63_mods(x, y)                                                      \
  (accu = Val_long(int63_of_value(x) % int63_of_value(y)))
...
/* TODO: is - or ^ better? */
#define Uint63_lsl(x, y)                                                       \
  do {                                                                         \
    value uint63_lsl_y__ = (y);                                                \
    if (uint63_lsl_y__ < (uint64_t)127)                                        \
      accu =                                                                   \
          (value)((((uint64_t)(x) - 1) << uint63_of_value(uint63_lsl_y__)) |   \
                  1);                                                          \
    else                                                                       \
      accu = uint63_zero();                                                    \
  } while (0)
#define Uint63_lsr(x, y)                                                       \
  do {                                                                         \
    value uint63_lsl_y__ = (y);                                                \
    if (uint63_lsl_y__ < (uint64_t)127)                                        \
...
#define lt128(xh, xl, yh, yl)                                                  \
  (uint63_lt(xh, yh) || (uint63_eq(xh, yh) && uint63_lt(xl, yl)))
#define le128(xh, xl, yh, yl)                                                  \
  (uint63_lt(xh, yh) || (uint63_eq(xh, yh) && uint63_leq(xl, yl)))
...
double coq_uint63_to_float(value x) { return (double)uint63_of_value(x); }

value coq_uint63_to_float_byte(value x) {
  return caml_copy_double(coq_uint63_to_float(x));
}

#define Uint63_of_double(f)                                                    \
  do {                                                                         \
