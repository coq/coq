...
#define Uint63_to_double(x) Rocq_copy_double((double) uint63_of_value(x))

double rocq_uint63_to_float(value x) {
  return (double) uint63_of_value(x);
}

value rocq_uint63_to_float_byte(value x) {
  return caml_copy_double(rocq_uint63_to_float(x));
}
