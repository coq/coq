...
#define Enter_gc(dom_st, wosize)                                               \
  do {                                                                         \
    Setup_for_gc;                                                              \
    Alloc_small_enter_GC(dom_st, wosize);                                      \
    Restore_after_gc;                                                          \
  } while (0)
#define Coq_alloc_small(result, wosize, tag)                                   \
  Alloc_small(result, wosize, tag, Enter_gc)
#else
...
#define CheckInt3()                                                            \
  CheckPrimArgs(Is_uint63(accu) && Is_uint63(sp[0]) && Is_uint63(sp[1]), apply3)
...
#define AllocCarry(cond)                                                       \
  Coq_alloc_small(accu, 1, (cond) ? coq_tag_C1 : coq_tag_C0)
...
/* Turn a code pointer into a stack value usable as a return address, and
   conversely. The least significant bit is set to 1 so that the GC does not
   mistake return addresses for heap pointers. */
...
value coq_interprete(code_t coq_pc, value coq_accu, value coq_atom_tbl,
                     value coq_global_data, value coq_env,
                     long coq_extra_args) {
  ...
  Instruct(GRABREC) {
    ...
    if (rec_pos <= coq_extra_args && !Is_accu(sp[rec_pos])) {
      ...
      if (coq_extra_args < rec_pos) {
        ...
        for (i = 0; i < rec_pos; i++)
          Field(accu, i + 3) = *sp++; // Storing args
    ...
    if (Is_accu(accu)) {
      *--sp = accu;          // Save matched block on stack
      ...
      case ATOM_COFIX_TAG: // We are forcing a cofix
      {
        ...
        nargs = Wosize_val(accu) - 3; // Number of args = size of accumulator -
                                      // 2 (accumulator closure) - 1 (atom)
  ...
  Instruct(MAKESWITCHBLOCK) {
    ...
    *--sp = accu;          // Save matched block on stack
    ...
    case ATOM_COFIX_TAG: // We are forcing a cofix
    {
      ...
      accu = sp[2];                     // Save accumulator to accu register
      sp[2] = Val_long(coq_extra_args); // Push number of args for return
      nargs = Wosize_val(accu) - 3; // Number of args = size of accumulator - 2
                                    // (accumulator closure) - 1 (atom)
      // Push arguments to stack
      CHECK_STACK(nargs + 1);
      sp -= nargs;
      for (i = 0; i < nargs; i++)
    ...
    default: {
      ...
      *--sp = Field(coq_global_data, annot);
      ...
      if (sz == 0)
        ...
        if (Is_tailrec_switch(*sp)) {
          for (i = 0; i < sz; i++)
            caml_initialize(&Field(accu, i), sp[i + 2]);
        } else {
          for (i = 0; i < sz; i++)
            caml_initialize(&Field(accu, i), sp[i + 5]);
        }
      }
      *--sp = accu;
...
value coq_push_arguments(value args) {
  ...
  CHECK_STACK(nargs);
  coq_sp -= nargs;
  print_instr("push_args");
  print_int(nargs);
  for (i = 0; i < nargs; i++)
    coq_sp[i] = Field(args, i + 3);
  return Val_unit;
}
...
value coq_push_vstack(value stk, value max_stack_size) {
  ...
  CHECK_STACK(len);
  coq_sp -= len;
  print_instr("push_vstack");
  print_int(len);
  for (i = 0; i < len; i++)
    coq_sp[i] = Field(stk, i);
  sp = coq_sp;
...
value coq_interprete_ml(value tcode, value a, value t, value g, value e,
                        value ea) {
  // Registering the other arguments w.r.t. the OCaml GC is done by
  // coq_interprete
  CAMLparam1(tcode);
...
value coq_interprete_byte(value *argv, int argn) {
  return coq_interprete_ml(argv[0], argv[1], argv[2], argv[3], argv[4],
                           argv[5]);
}
