...
let get_cumulativity_constraints cv_pb ~nargs variance u u' =
  ...
  debug Pp.(fun () -> str"get_cumulativity_constraints: " ++ pri u ++ spc () ++ pr_conv_pb cv_pb ++ spc () ++ pri u' ++ str" variances: " ++ UVars.ApplicationVariances.pr variance);
  match cv_pb with
  | CONV ->
...
let convert_inductives_gen cmp_instances cmp_cumul env cv_pb (mind, ind) ~nargs u1 u2 s =
  ...
  | Some variances ->
    ...
    if not (UVars.is_applied nargs num_param_arity) then
      ...
      cmp_cumul ~nargs cv_pb (UVars.Variances.application_variances variances) u1 u2 s
...
let convert_constructors_gen cmp_instances cmp_cumul env ((mind, ind), cns) ~nargs u1 u2 s =
  ...
  | Some _ ->
    ...
    if not (UVars.is_applied nargs num_cnstr_args) then
      ...
      let variance = UVars.ApplicationVariances.make (snd (UVars.Instance.length u1)) UVars.(VariancePos.make Variance.Irrelevant Position.InTerm) in
...
let convert_constants_gen cmp_instances cmp_cumul env cv_pb cst ~nargs u1 u2 s =
  ...
  | Some variance ->
    ...
    let variance = UVars.Variances.application_variances variance in
