...
val unify_lax : relevance Range.t -> Conversion.conv_pb -> constr Evar.Map.t -> Environ.env ->
  extra_env -> constr -> constr -> extra_env

val typecheck_pattern : Environ.env -> pattern -> extra_env * Environ.unsafe_judgment

val noccur_evar : Environ.env -> extra_env -> constr Evar.Map.t -> Evar.t -> constr -> bool

val add_evar_definition : Environ.env -> extra_env -> constr Evar.Map.t -> Evar.t -> constr -> extra_env * constr Evar.Map.t

val push_equality :
  Environ.env -> extra_env -> constr Evar.Map.t -> (relevance Range.t * QVar.t * constr * Conversion.conv_pb * constr) ->
  extra_env * bool
...
val nf_evar_map : Evar.t list -> QGraph.t -> types Evar.Map.t ->
