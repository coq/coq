...
let enter2 s pr_fun lft1 lft2 x1 x2 =
  let prf (lft,x) = prch '<'; prlft lft; prch '>'; pr_fun x in
  enter_pr s (fun l -> prlist prf " = " l) [lft1,x1;lft2,x2]
...
let rec ccnv cv_pb infos lft1 lft2 term1 term2 cuniv =
  enter2 "ccnv" (prfc infos) lft1 lft2 term1 term2; leave "ccnv" (eqappr cv_pb infos
    (lft1, whd_stack infos (el_dom lft1) term1 [])
    (lft2, whd_stack infos (el_dom lft2) term2 [])
    cuniv)

(* Conversion between [lft1](hd1 v1) and [lft2](hd2 v2)
when hd1 = FFlex fl1
and no rewriting is possible *)
and eqappr_flex1 cv_pb infos appr1 appr2 fl1 cuniv =
  let (lft1,(hd1,v1)) = appr1 and (lft2,(hd2,v2)) = appr2 in
    enter2 "eqappr_flex1" (prfcst infos) lft1 lft2 (hd1,v1) (hd2,v2); leave "eqappr_flex1" (match unfold_reference infos fl1 with
    | Some def1 ->
	branch "eqappr_flex1" "unfold";let (lft1,(_,v1)) = appr1 in
	  eqappr cv_pb infos (lft1, whd_stack infos (el_dom lft1) def1 v1) appr2
	    cuniv
...
and no rewriting is possible *)
and eqappr_flex2 cv_pb infos appr1 appr2 fl1 fl2 cuniv =
  let (lft1,(hd1,v1)) = appr1 and (lft2,(hd2,v2)) = appr2 in
  let call_oracle() =
          let (app1,app2) =
            if Conv_oracle.oracle_order fl1 fl2 then
              (match unfold_reference infos fl1 with
                | Some def1 -> ((lft1, whd_stack infos (el_dom lft1) def1 v1), appr2)
                | None ->
                    (match unfold_reference infos fl2 with
                      | Some def2 ->
			  (appr1, (lft2, whd_stack infos (el_dom lft2) def2 v2))
		      | None -> raise NotConvertible))
            else
	      (match unfold_reference infos fl2 with
                | Some def2 -> (appr1, (lft2, whd_stack infos (el_dom lft2) def2 v2))
                | None ->
                    (match unfold_reference infos fl1 with
                    | Some def1 ->
			((lft1, whd_stack infos (el_dom lft1) def1 v1), appr2)
		    | None -> raise NotConvertible)) in
          eqappr cv_pb infos app1 app2 cuniv
  in
    enter2 "eqappr_flex2" (prfcst infos) lft1 lft2 (hd1,v1) (hd2,v2); leave "eqappr_flex2" (
...
and eqappr cv_pb infos appr1 appr2 cuniv =
  ...
  let el2 = el_stack lft2 v2 in
    enter2 "eqappr" (prfcst infos) lft1 lft2 (hd1,v1) (hd2,v2); leave "eqappr" (
  match (fterm_of hd1, fterm_of hd2) with
	...
	branch"eqappr" "flex-flex";begin
	  match betaiota_rewrite_nf infos lft1 hd1 v1 fl1 with
	    | _,Some hdv1' -> (* 1: rewrite nf of the beta-iota nf *)
		begin match betaiota_rewrite_nf infos lft2 hd2 v2 fl2 with
		  | _,Some hdv2' -> (* rewrite nf of the beta-iota nf *)
		      eqappr cv_pb infos (lft1,hdv1') (lft2,hdv2') cuniv
		  | Some hdv2',_ -> (* beta-iota nf *)
		      eqappr cv_pb infos (lft1,hdv1') (lft2,hdv2') cuniv
		  | _ -> (* not head rewritable *)
		      eqappr cv_pb infos (lft1,hdv1') appr2 cuniv
		end
		...
		begin match betaiota_rewrite_nf infos lft2 hd2 v2 fl2 with
		  | _,Some hdv2' -> (* rewrite nf of the beta-iota nf *)
		      eqappr cv_pb infos (lft1,hdv1') (lft2,hdv2') cuniv
		  | Some hdv2',_ -> (* beta-iota nf *)
		      eqappr_flex2 cv_pb infos (lft1,hdv1') (lft2,hdv2')
		      fl1 fl2 cuniv
		  | _ -> (* not head rewritable *)
		      eqappr_flex2 cv_pb infos (lft1,hdv1') appr2
		      fl1 fl2 cuniv
		end
	    | _ -> (* 1: not head rewritable *)
		begin match betaiota_rewrite_nf infos lft2 hd2 v2 fl2 with
		  | _,Some hdv2' -> (* rewrite nf of the beta-iota nf *)
		      eqappr cv_pb infos appr1 (lft2,hdv2') cuniv
		  | Some hdv2',_ -> (* beta-iota nf *)
		      eqappr_flex2 cv_pb infos appr1 (lft2,hdv2')
		      fl1 fl2 cuniv
		  | _ -> (* not head rewritable *)
		      eqappr_flex2 cv_pb infos appr1 appr2 fl1 fl2 cuniv
		end
	end
	...
	branch"eqappr" "flex-?";begin
	  match betaiota_rewrite_nf infos lft1 hd1 v1 fl1 with
	    | _,Some hdv1' -> (* 1: rewrite nf of the beta-iota nf *)
		branch"eqappr" "1";eqappr cv_pb infos (lft1,hdv1') appr2 cuniv
	    | Some hdv1',_ -> (* 1: beta-iota nf *)
		branch"eqappr" "2";eqappr_flex1 cv_pb infos (lft1,hdv1') appr2 fl1 cuniv
	    | _ -> (* 1: not head rewritable *)
		branch"eqappr" "3";eqappr_flex1 cv_pb infos appr1 appr2 fl1 cuniv
	end
    | (_, FFlex fl2)      ->
	branch"eqappr" "?-flex";begin
	  match betaiota_rewrite_nf infos lft2 hd2 v2 fl2 with
	    | _,Some hdv2' -> (* 2: rewrite nf of the beta-iota nf *)
		branch"eqappr" "1";eqappr cv_pb infos appr1 (lft2,hdv2') cuniv
	    | Some hdv2',_ -> (* 2: beta-iota nf *)
		branch"eqappr" "2";eqappr_flex1 cv_pb infos (lft2,hdv2') appr1 fl2 cuniv
	    | _ -> (* 2: not head rewritable *)
		branch"eqappr" "3";eqappr_flex1 cv_pb infos appr2 appr1 fl2 cuniv
	end
...
let fconv cv_pb env t1 t2 =
  init();if eq_constr t1 t2 then 
    ...
    ccnv cv_pb infos (ELID 0) (ELID 0) (inject t1) (inject t2)
      Constraint.empty
