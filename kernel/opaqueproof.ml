...
let iter_direct_opaque f = function
  ...
  | Direct cu ->
      Direct (Future.chain cu (fun (c, u) -> f c; c, u))

let map_direct_opaque f = function
  | Indirect _ -> CErrors.anomaly (Pp.str "Not a direct opaque.")
  | Direct cu -> Direct (Future.chain cu f)

let join_opaque { opaque_val = prfs; opaque_dir = odp; _ } = function
  | Direct cu -> ignore(Future.join cu)
  | Indirect (_,dp,i) ->
...
let uuid_opaque { opaque_val = prfs; opaque_dir = odp; _ } = function
  | Direct cu -> Some (Future.uuid cu)
  | Indirect (_,dp,i) ->
      if DirPath.equal dp odp
      then Some (Future.uuid (Int.Map.find i prfs))
      else None
...
let dump { opaque_val = otab; opaque_len = n; _ } =
  ...
  otab;
  opaque_table, univ_table, !f2t_map
