...
let rec translate_modtype env mte =
  match mte with
	...
	let env' = add_module (MPbid arg_id) (module_body arg_b) env in
  ...
  let do_entry env (l,e) = 
    match e with
	  ...
	  let cb = translate_constant env ce in
	    add_constant (make_ln mp l) cb env, (l, SPBconst cb)
      | SPEmind mie -> 
	  let mib = translate_mind env mie in
	    add_mind (make_ln mp l) mib env, (l, SPBmind mib)
      | SPEmodule me ->
	  ...
	  let mtb = translate_modtype env mte in
	    add_modtype (make_ln mp l) mtb env, (l, SPBmodtype mtb)
  in
  let _,sig_b = Univ.with_module mp (list_fold_map do_entry env) sig_e
  in
    sig_b
