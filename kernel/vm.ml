...
(*  + Les tableaux persistant : reference sur des parray_kind =>         *)
(*                   = 0_[parray_kind]                                    *)
(* + Les produits :                                                       *)
(*   -   vprod = 0_[ dom | codom]                                         *)
...
(*     -- 7_[Vsort s] pas un accumulateur mais une sort                   *)
...
let rec whd_accu a stk =
  ...
  | 7 -> 
      if stk = [] then (Obj.obj (Obj.field at 0)) (*Vsort s*)
      else assert false
  | _ -> assert false
...
let whd_val : values -> whd =
  fun v -> 
	...
	(
	if Obj.size o = 1 then Varray (Obj.obj o)
	else 
...
let obj_atom_tbl = Hashtbl.create 100 

let obj_of_atom : atom -> Obj.t =
  fun a -> 
    try Hashtbl.find obj_atom_tbl a 
    with Not_found ->
      let res = Obj.new_block accu_tag 2 in
      Obj.set_field res 0 (Obj.repr accumulate);
      Obj.set_field res 1 (Obj.repr a);
      Hashtbl.add obj_atom_tbl a res;
      res 

let sort_tbl = Hashtbl.create 100 
let obj_of_sort s =
  try Hashtbl.find sort_tbl s 
  with Not_found ->
    let a = Obj.new_block 7 1 in
    Obj.set_field a 0 (Obj.repr (Vsort s));
    let res = Obj.new_block accu_tag 2 in
    Obj.set_field res 0 (Obj.repr accumulate);
    Obj.set_field res 1 (Obj.repr a);
    Hashtbl.add sort_tbl s res;
    res 
...
let val_of_sort s = val_of_obj (obj_of_sort s)
let val_of_ind ind = val_of_atom (Aind ind)

let val_of_str_const = function
  | Const_sorts s -> val_of_sort s
  | Const_ind ind -> val_of_ind ind
  | Const_b0 i -> (Obj.magic i : values)
  | Const_val v -> v

let val_of_idkey key = val_of_atom (Aid key)
