...
module ClausesOf = struct
  module ClauseInfo = struct
    ...
    (** [subsumes cl cl'] is true if [cl] subsums [cl'], i.e. [cl'] is implied by [cl] *)
    let subsumes (i, prems) (i', prems') =
...
module PartialClausesOf = struct
  module ClauseInfo = struct
    ...
    let pr pr_pointint prem concl (k, prems) =
      let open Pp in
      hov 0 (prlist_with_sep (fun () -> str ",") pr_pointint (prem :: Option.cata Premises.to_list [] prems)
        ++ str " → " ++ pr_pointint (concl, k))
  end

  module SWC = SetWithCardinal(ClauseInfo)
  include SWC

  let pr pr_pointint prem concl cls =
    let open Pp in
    v 0 (prlist_with_sep spc (ClauseInfo.pr pr_pointint prem concl) (elements cls))
...
struct
  ...
  let repr_clause_info m ((k, prems as x) : ClausesOf.ClauseInfo.t) : ClausesOf.ClauseInfo.t =
  ...
  let clauseinfo_equal_upto m ((k, prems) : ClausesOf.ClauseInfo.t) (k', prems') =
    Int.equal k k' &&
...
let pr_fwd_clause m prem (cls : ForwardClauses.t) =
  ForwardClauses.pr (pr_pointint m) prem cls
        ...
        (fun (k, prems) ->
          ...
          let check_prem (l, lk) =
            ...
            ForwardClauses.IntMap.exists (fun k fwd ->
              ...
              can' == can &&
              PartialClausesOf.exists (fun (k', prems') -> Int.equal k k' &&
                NeList.equal (eq_pointint model)
                prems (simplify_can_premises (cons_opt_nelist (lcan, k) (Option.map (canonical_premises model) prems'))))
                (PartialClausesOf.shift shift kprem))
                fwd)
              lcan.clauses_fwd
          in
          Premises.iter (fun kprem -> if (check_prem kprem) then () else
            CErrors.user_err Pp.(str"Clause " ++ pr_fwd_clause model (fst kprem)
              (ForwardClauses.singleton { concl = can.canon; kprem = (snd kprem);
                prems = PartialClausesOf.singleton (k, Some prems) }) ++ str" is not registered as a forward clauses for "
  ...
  | k0 ->
    ...
    | _ ->
      ...
      debug_updates Pp.(fun () -> str" due to clause " ++
        pr_clause (pr_pointint m) (cons_opt_nelist (prem, premk) premises, (concl.canon, conclk)));
      Some newk
...
let _pr_canset m (cls : CanSet.t) : Pp.t =
  ...
  CanSet.fold (fun idx fwd acc ->
    hov 1 (str "For " ++ pr_index_point m idx ++ str": " ++
...
let pr_can_prems m = Premises.pr (pr_incr (pr_can m))

(** [find_to_merge_bwd model status u v] Search for an equivalence class of universes backward from u to v.

  This does not handle cases where an equality would follow from a max() constraint, e.g.
  v >= x, v >= y, max(x, y) >= u, u >= v

  Search for v starting from u will ignore the max() constraint. This means that check_eq_level cannot
  strictly rely on equality of representatives
...
(* let switch_prefix can newp oldp =
  let rec aux oldp =
    match oldp with
    | [] -> []
    | (p, _ as hd) :: oldp ->
      if p == can then hd :: newp
      else hd :: aux oldp
  in aux oldp *)
  ...
  let rec backward status path (can, k) : Status.t * PathSet.t =
    ...
    | exception Not_found ->
      ...
      if ClausesOf.is_empty cls then status, merge else begin
        ...
        let merge_fn (clk, prems) (status, merge as acc) =
...
let find_to_merge_fwd model status (canu, ku) (canv, kv) =
  ...
  let rec forward (can, k) : bool =
    ...
    | exception Not_found ->
      ...
      if ForwardClauses.is_empty cls then merge else
      let canvalue = defined_expr_value model (can, k) in
      debug Pp.(fun () -> str"Forward universes: " ++ int (ForwardClauses.cardinal cls) ++
        str " Canonical: " ++ int (PSet.cardinal (clauses_bwd_univs model cls)));
      let merge =
        ForwardClauses.fold (fun ~kprem ~concl ~prems
          (* { can + kprem, prems -> concl + k } *) merge ->
          ...
          if (* Avoid self-loops *) conclcan == can then merge
            ...
            PartialClausesOf.fold (fun (clk, prems) merge ->
              incr nb_cstrs;
              match prems with
              | None ->
                (* Stay in the same equivalence class *)
...
let check_eq_level_expr u v m =
  let vl = v in debug_global Pp.(fun () -> str"Checking equality of representatives " ++ pr_with_incr Level.raw_pr u ++ str " and " ++ pr_with_incr Level.raw_pr vl);
...
let remove_fwd_clauses_to model prems target =
  Premises._fold (fun le model ->
    let can, _k = repr_expr_can model le in
    let fwd' = ForwardClauses.filter (fun _k concl _ -> Index.equal concl target) can.clauses_fwd in
    change_node model { can with clauses_fwd = fwd' })
    prems model

let remove_bwd_clauses_from model prem target =
  let can, _k = repr model prem in
  let bwd' = ClausesOf.filter_map (fun (_, prems as cli) ->
...
(** [minimize idx model] returns a new model where the universe level idx has been removed and
  ...
  It returns an approximate lower bound on idx in the original constraints.  *)
let _minimize idx model =
        ...
        (* premsfwd, can + kprem -> concl + conclk *)
          let model =
            ClausesOf.fold (fun (cank, premsbwd) model ->
              ...
              model) bwd model
          in
          let glb = (concl.canon, max (conclk - kprem) 0) :: glb in
          (model, glb))
        fwd (model, glb)
      in
      let model = remove_bwd_clauses_from model concl.canon can.canon in
      (model, glb))
    fwd (model, [])
  in
