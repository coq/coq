...
module ClausesOf = struct
  module ClauseInfo = struct
    ...
    let pr pr_index_point concl (k, prem) =
      let open Pp in
      hov 0 (Premises.pr pr_index_point prem ++ str " → " ++ pr_index_point (concl, k))
  end
        ...
        (fun (k, prems) ->
          ...
          Premises.iter (fun kprem -> if (check_prem kprem) then () else
            CErrors.user_err Pp.(str"Clause " ++ pr_fwd_clause model (fst kprem)
              ...
              ++ pr_index_point model kprem)) prems) cls;
      incr n_canon
    | Equiv (idx', k) ->
      assert (k >= 0);
...
let pr_clauses_all m =
  ...
  PMap.fold (fun p e acc ->
    match e with
    | Equiv (p', k) ->
      pr_raw_index_point m p ++ str " = " ++ pr_raw_index_point_expr m (p',k) ++ spc () ++ acc
  ...
  let modelfwd =
     ForwardClauses.fold (fun ~kprem ~concl ~prems model ->
      ...
      PartialClausesOf.fold (fun (conclk, premsfwd) model ->
        ...
        | Some model -> remove_bwd_clauses_from model concl.canon can.canon
