...
let add_can_clause_model m ((prems, (canl, conclk)) : can_clause) : can_clause * model =
  ...
  let prems', m' = (* Add clause to the forward clauses from the premises *)
    NeList.fold_map (fun ((idx0, kprem) as prem) entries ->
      ...
      prems m'
  in (prems', repr_expr_can m' (canl'.canon, conclk)), m'
...
(** [shrink_premises prems concl conclk] Simplifies the clause [prems -> concl + conclk]
...
let _simplify_clause m prems concl conclk : Premises.t option =
  let prems' = Premises.smart_map (repr_expr m) prems in
  ...
  let rec backward (can, k) : bool =
    ...
    | exception Not_found ->
      ...
      let merge_fn (clk, prems) merge =
        ...
        in
        match prems with
        | NeList.Tip p -> merge_prem p || merge
        | NeList.Cons _ -> merge
      in
      let merge = ClausesOf.fold merge_fn cls merge in
...
let _infer_clause_extension cl m =
    ...
    ForwardClauses.fold (fun ~kprem ~concl ~prems model ->
      ...
      PartialClausesOf.fold (fun (conclk, premsfwd) model ->
        ...
        ClausesOf.fold (fun (cank, premsbwd) model ->
          ...
          let _, model = add_can_clause_model model (repr_clause model cl) in
          model)
        bwd model)
      fwd model)
    fwd model
