...
let rec execute env sigma cstr =
  ...
  match kind cstr with
    ...
    | App (f,args) ->
      let argst = execute_array env sigma args in
        let ft =
          match kind f with
    ...
    | Lambda (name,c1,c2) ->
      let s = execute_is_type env sigma c1 in
      let () = check_assum_annot env s name c1 in
      let env1 = push_rel (LocalAssum (name,c1)) env in
      let c2t = execute env1 sigma c2 in
      type_of_abstraction env name c1 c2t

    | Prod (name,c1,c2) ->
      let vars = execute_is_type env sigma c1 in
      let () = check_assum_annot env vars name c1 in
      let env1 = push_rel (LocalAssum (name,c1)) env in
      let vars' = execute_is_type env1 sigma c2 in
      type_of_product env ~under_typing:sigma.under_typing name vars vars'

    | LetIn (name,c1,c2,c3) ->
      let c1t = execute env sigma c1 in
      let c2s = execute_is_type env sigma c2 in
      let () = check_let_annot env c2s name c1 c2 in
      let () = check_cast env c1 c1t DEFAULTcast c2 in
      let env1 = push_rel (LocalDef (name,c1,c2)) env in
      let c3t = execute env1 sigma c3 in
      subst1 c1 c3t

    | Cast (c,k,t) ->
      let ct = execute env sigma c in
      let _ts : Sorts.t = execute_is_type env sigma t in
    ...
    | Case (ci, u, pms, (p,rp), iv, c, lf) ->
        ...
        let () = match iv with
          ...
          | CaseInvert {indices} ->
            ...
            let _ : Sorts.t = execute_is_type env sigma ct' in
        ...
        let pmst = execute_array env sigma pms in
        ...
        let (pctx, pt) =
          ...
          let pt = execute p_env sigma p in
          (realdecls, pt)
        in
        let () =
        ...
        let build_one_branch i (nas, br) =
          ...
          let brt = execute br_env sigma br in
    ...
    | Fix ((_,i),recdef as fix) ->
      let fix_ty = execute_recdef env sigma recdef i in
      check_fix env fix;
      fix_ty
    ...
    | Evar evargs ->
        sigma.evar_types evargs

and execute_is_type env sigma constr =
  let t = execute env sigma constr in
  check_type env constr t

and execute_recdef env sigma (names,lar,vdef) i =
  let lart = execute_array env sigma lar in
  ...
  let vdeft = execute_array env1 sigma vdef in
  let () = check_fixpoint env1 names lar vdef vdeft in
  lar.(i)

and execute_array env sigma cs =
  Array.map (fun c -> execute env sigma c) cs
...
and type_of_global_reference_knowing_parameters env sigma evars c args =
  ...
  | Ind (ind, u) ->
    ...
    let paramtyps = make_param_univs env (ind, u) mip args argsty in
