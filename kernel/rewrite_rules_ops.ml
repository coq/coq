...
module QGraph = struct
  ...
  let check_eq_quality q1 q2 qmap =
    Quality.equal (repr q1 qmap) (repr q2 qmap)

  let check_constraints (q1, knd, q2) qmap =
...
type eq_cmp = LESS | EQUAL | GREATER
...
let define_evar ev cmp t evd =
  let evar_candidates = Evar.Map.update ev (function Some l -> Some ((cmp, t) :: l) | None -> assert false) evd.evar_candidates in
...
let enforce_cmp_quality cv_pb q1 q2 evd =
  ...
  let qcstrs = if b then evd.qcstrs else QConstraints.add (q1, cv_pb, q2) evd.qcstrs in
...
let cmp_of_pb = function CUMUL -> GREATER | CONV -> EQUAL
let cmp_of_pb_r : _ -> eq_cmp = function CUMUL -> LESS | CONV -> EQUAL
...
let rec fterm_relevance infos evd lft c =
  ...
  | FLOCKED | FLIFT _ | FCLOS _ -> CErrors.anomaly Pp.(str "is_fterm_irrelevant called on improper fterm")
...
let rec unify cv_pb infos ?(nargs=0) lft1 t1 lft2 t2 evd =
  ...
  | RelevanceVar q, RelevanceVar q' when QGraph.check_eq_quality q q' evd.qgraph ->
  ...
  | FInd (ind1, u1 as pind1), FInd (ind2, u2 as pind2) ->
    if not (Ind.CanOrd.equal ind1 ind2) then
      ...
      begin match convert_inductives cv_pb (mind, snd ind1) nargs u1 u2 evd with
      | evd -> evd
      | exception MustExpand ->
  ...
  | FConstruct ((ind1, j1), u1 as pctor1), FConstruct ((ind2, j2), u2 as pctor2) ->
    if not (Int.equal j1 j2 && Ind.CanOrd.equal ind1 ind2) then
      ...
      let mind = Environ.lookup_mind (fst ind1) (info_env infos.cnv_inf) in
      begin match convert_constructors (mind, snd ind1, j1) nargs u1 u2 evd with
      | evd -> evd
      | exception MustExpand ->
        let env = info_env infos.cnv_inf in
  ...
  | FProj (p1, r1, c1), FProj (p2, r2, c2) ->
    if is_irrelevant infos.cnv_inf r1 && is_irrelevant infos.cnv_inf r2 then
      ...
      unify CONV infos lft1 c1 lft2 c2 evd
  ...
  | FCaseT (ci1, u1, pms1, p1, c1, br1, e1), FCaseT (ci2, u2, pms2, p2, c2, br2, e2) ->
    ...
    in
    let evd = unify CONV infos lft1 c1 lft2 c2 evd in
  ...
  | FApp (hd1, args1), FApp (hd2, args2) ->
    ...
    | FConstruct _, FConstruct _ ->
        ...
        let evd = unify cv_pb infos ~nargs:nargs1 lft_hd1 hd1 lft_hd2 hd2 evd in
...
let reduce_to_prod env evd ((evkty, qty, _uty), (evkret, _qret, _uret)) t =
  ...
  | FProd (na, c1, c2, e) ->
      ...
      let evd = ExtraEnv.define_evar evkty EQUAL ty evd in
      let evd = ExtraEnv.define_evar evkret EQUAL ret evd in
  ...
  | FEvar (ev, inst, e, _) ->
    ...
    let evd =
      if is_frel_inst e inst then
        ExtraEnv.define_evar ev EQUAL prodty evd
      else evd
    in
...
let reduce_to_appind env evd ind (qinst_annot, uinst_annot, args_annot) t =
  ...
  | FInd (ind', u) when Ind.CanOrd.equal ind ind' ->
      ...
      let args = [||] in
      let evd = Array.fold_left2 (fun evd evk arg -> ExtraEnv.define_evar evk EQUAL arg evd) evd args_annot args in
      evd, (u, args)
  | FApp (hd, args) ->
  ...
  | FEvar (ev, inst, e, _) ->
    ...
    let evd =
      if is_frel_inst e inst then
        ExtraEnv.define_evar ev EQUAL indargs evd
      else evd
    in
    evd, (u, args)
  | _ -> CErrors.anomaly (Pp.str "Typing in rewrite rules")
...
let rec typecheck_pattern env evd = function
  ...
  | PSort s ->
      ...
      evd, make_judge (mkSort s) (Typeops.type_of_sort ~under_typing:true s)
  | PInd (ind, u) ->
  ...
  | PInt i ->
      evd, Typeops.judge_of_int env i
  | PFloat f ->
      evd, Typeops.judge_of_float env f
  | PString s ->
      evd, Typeops.judge_of_string env s
  ...
  | PProd (na, argty, (argq, argu), retty, (retq, retu)) ->
      let evd = evd |> ExtraEnv.add_quality argq ~bound:false |> ExtraEnv.add_universe argu ~bound:false in
      let args = mk_sort_qvar argq argu in
      let evd, argtyj = typecheck_arg_pattern env evd (mkSort args, Relevant) argty in
      let decl = Context.Rel.Declaration.LocalAssum (Context.make_annot na (Sorts.RelevanceVar argq), j_val argtyj) in
      let env = Environ.push_rel decl env in

      let evd = evd |> ExtraEnv.add_quality retq ~bound:false |> ExtraEnv.add_universe retu ~bound:false in
      let rets = mk_sort_qvar retq retu in
      let evd, bodj = typecheck_arg_pattern env evd (mkSort rets, Relevant) retty in
      let tm = Term.mkProd_or_LetIn decl (j_val bodj) in
      let ty = Typeops.sort_of_product env ~under_typing:true args rets in
      evd, make_judge tm (mkSort ty)
  ...
  | PCase (c, ind, annots, (ret, (qr, ur)), brs) ->
    ...
    let build_one_branch i evd (brnas, br) =
      ...
      let cargs = Context.Rel.instance mkRel 0 ctx in
...
let noccur_evar env evd defs evk c =
  ...
  | Evar (evk', args') ->
    if Evar.equal evk evk' then raise Occur
      ...
      | Some def -> occur_rec check_types acc def
      | None ->
          if check_types then
...
let add_evar_definition env evd defs evk def =
  ...
  let evd = List.fold_left (fun evd (cmp, t) ->
    match cmp with
    | LESS    -> unify_lax rels CUMUL defs env evd t def
    | EQUAL   -> unify_lax rels CONV  defs env evd t def
    | GREATER -> unify_lax rels CUMUL defs env evd def t
    ) evd eqs
  in
  evd, defs
...
let find_evar_definition env evd defs evk def =
  ...
  | Some def ->
      match List.find_map (fun (cmp, t) -> if cmp != LESS && Constr.equal def t then Some t else None) eqs with
      | None -> evd, defs, false
      | Some def ->
...
let check_inferred_constraints env evd (Info info) =
  ...
  in
  let evd, _defs' = push_constraints env evd info.evar_defs in
  ...
  in
  let () = Level.Map.symmetric_diff_fold (fun _ q1 q2 () ->
    match q1, q2 with
  ...
  if not @@ QConstraints.for_all (fun qcstr -> QGraph.check_constraints qcstr evd.qgraph) qcstrs then
    CErrors.anomaly (Pp.str "Quality equality could not be confirmed");
...
let test_projection_apps env pat =
  ...
  if List.for_all_i (fun i arg ->
    match arg with
    | PVar _ -> true
    | Pat PProj (_, p, _) -> Ind.CanOrd.equal (Projection.inductive p) ind && Projection.arg p = i
    | Pat _ -> false
  ) 0 args then
...
let rec check_pattern_redex env = function
  ...
  | PProd (_, ty, _, bod, _) -> check_pattern_redex_aux env ty; check_pattern_redex_aux env bod
