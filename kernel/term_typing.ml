...
let skip_trusted_seff sl b e =
  let rec aux sl b e acc =
    ...
    if CInt.equal sl 0 then b, e, acc
...
let build_constant_declaration env result =
  ...
  let tps = 
    ...
    COption.map Cemitcodes.from_val res
  in
  { const_hyps = hyps;
    const_body = def;
    const_type = typ;
...
let translate_recipe env _kn r =
  ...
  let tps = COption.map Cemitcodes.from_val res in
  let hyps = COption.get result.cook_context in
