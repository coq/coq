...
| ExplByPos (i1, id1), ExplByPos (i2, id2) ->
  CInt.equal i1 i2 && COption.equal Id.equal id1 id2
...
let rec cases_pattern_expr_eq p1 p2 =
  ...
  | CPatCstr(c1,a1,b1), CPatCstr(c2,a2,b2) ->
      qualid_eq c1 c2 &&
      COption.equal (List.equal cases_pattern_expr_eq) a1 a2 &&
      List.equal cases_pattern_expr_eq b1 b2
  | CPatAtom(r1), CPatAtom(r2) ->
    COption.equal qualid_eq r1 r2
  | CPatOr a1, CPatOr a2 ->
...
let rec constr_expr_eq e1 e2 =
  if CAst.(e1.v == e2.v) then true
    ...
    | CLetIn(na1,a1,t1,b1), CLetIn(na2,a2,t2,b2) ->
      ...
      constr_expr_eq a1 a2 &&
      COption.equal constr_expr_eq t1 t2 &&
      constr_expr_eq b1 b2
    | CAppExpl((proj1,r1,_),al1), CAppExpl((proj2,r2,_),al2) ->
      COption.equal CInt.equal proj1 proj2 &&
      qualid_eq r1 r2 &&
    ...
    | CCases(_,r1,a1,brl1), CCases(_,r2,a2,brl2) ->
      ...
      COption.equal constr_expr_eq r1 r2 &&
    ...
    | CLetTuple (n1, (m1, e1), t1, b1), CLetTuple (n2, (m2, e2), t2, b2) ->
      ...
      COption.equal (eq_ast Name.equal) m1 m2 &&
      COption.equal constr_expr_eq e1 e2 &&
      constr_expr_eq t1 t2 &&
      constr_expr_eq b1 b2
    | CIf (e1, (n1, r1), t1, f1), CIf (e2, (n2, r2), t2, f2) ->
      constr_expr_eq e1 e2 &&
      COption.equal (eq_ast Name.equal) n1 n2 &&
      COption.equal constr_expr_eq r1 r2 &&
      constr_expr_eq t1 t2 &&
  ...
  | CCast(t1,c1), CCast(t2,c2) ->
    ...
    | CGeneralization (bk1, ak1, e1), CGeneralization (bk2, ak2, e2) ->
      ...
      COption.equal abstraction_kind_eq ak1 ak2 &&
      constr_expr_eq e1 e2
...
and args_eq (a1,e1) (a2,e2) =
  COption.equal (eq_ast explicitation_eq) e1 e2 &&
  constr_expr_eq a1 a2

and case_expr_eq (e1, n1, p1) (e2, n2, p2) =
  constr_expr_eq e1 e2 &&
  COption.equal (eq_ast Name.equal) n1 n2 &&
  COption.equal cases_pattern_expr_eq p1 p2
...
and fix_expr_eq (id1,r1,bl1,a1,b1) (id2,r2,bl2,a2,b2) =
  (eq_ast Id.equal id1 id2) &&
  COption.equal recursion_order_expr_eq r1 r2 &&
  List.equal local_binder_eq bl1 bl2 &&
  constr_expr_eq a1 a2 &&
  constr_expr_eq b1 b2
...
and recursion_order_expr_eq_r r1 r2 = match r1, r2 with
  ...
  | CMeasureRec (i1, e1, o1), CMeasureRec (i2, e2, o2) ->
    COption.equal (eq_ast Id.equal) i1 i2 &&
    constr_expr_eq e1 e2 && COption.equal constr_expr_eq o1 o2
  | _ -> false
...
and local_binder_eq l1 l2 = match l1, l2 with
  | CLocalDef (n1, e1, t1), CLocalDef (n2, e2, t2) ->
    eq_ast Name.equal n1 n2 && constr_expr_eq e1 e2 && COption.equal constr_expr_eq t1 t2
...
let rec cases_pattern_fold_names f a pt = match CAst.(pt.v) with
  ...
  | CPatCstr (_,patl1,patl2) ->
    List.fold_left (cases_pattern_fold_names f)
      (COption.fold_left (List.fold_left (cases_pattern_fold_names f)) a patl1) patl2
...
let ids_of_cases_tomatch tms =
  List.fold_right
    (fun (_, ona, indnal) l ->
       COption.fold_right (fun t ids -> cases_pattern_fold_names Id.Set.add ids t)
         indnal
         (COption.fold_right (CAst.with_val (Name.fold_right Id.Set.add)) ona l))
    tms Id.Set.empty
...
let rec fold_local_binders g f n acc b = let open CAst in function
  ...
  | CLocalDef ( { v = na },c,t)::l ->
    COption.fold_left (f n) (f n (fold_local_binders g f (Name.fold_right g na n) acc b l) c) t
  | CLocalPattern { v = pat,t }::l ->
    let acc = fold_local_binders g f (cases_pattern_fold_names g n pat) acc b l in
    COption.fold_left (f n) acc t
  | [] ->
    f n acc b
...
let fold_constr_expr_with_binders g f n acc = CAst.with_val (function
    ...
    | CLetIn (na,a,t,b) ->
      f (Name.fold_right g (na.CAst.v) n) (COption.fold_left (f n) (f n acc a) t) b
    ...
    | CCases (sty,rtnpo,al,bl) ->
      ...
      let acc = COption.fold_left (f (Id.Set.fold g ids n)) acc rtnpo in
    ...
    | CLetTuple (nal,(ona,po),b,c) ->
      ...
      f (COption.fold_right (CAst.with_val (Name.fold_right g)) ona n') (f n acc b) c
    | CIf (c,(ona,po),b1,b2) ->
      let acc = f n (f n (f n acc b1) b2) c in
      COption.fold_left
        (f (COption.fold_right (CAst.with_val (Name.fold_right g)) ona n)) acc po
    | CFix (_,l) ->
...
let map_local_binders f g e bl =
  ...
  let h (e,bl) = function
    ...
    | CLocalDef( { loc ; v = na } as cna ,c,ty) ->
      (Name.fold_right g na e, CLocalDef(cna,f e c,COption.map (f e) ty)::bl)
    | CLocalPattern { loc; v = pat,t } ->
      let ids = ids_of_pattern pat in
      (Id.Set.fold g ids e, CLocalPattern (make ?loc (pat,COption.map (f e) t))::bl) in
...
let map_constr_expr_with_binders g f e = CAst.map (function
    ...
    | CLetIn (na,a,t,b) ->
      CLetIn (na,f e a,COption.map (f e) t,f (Name.fold_right g (na.CAst.v) e) b)
    ...
    | CCases (sty,rtnpo,a,bl) ->
      ...
      let po = COption.map (f (Id.Set.fold g ids e)) rtnpo in
      CCases (sty, po, List.map (fun (tm,x,y) -> f e tm,x,y) a,bl)
    | CLetTuple (nal,(ona,po),b,c) ->
      let e' = List.fold_right (CAst.with_val (Name.fold_right g)) nal e in
      let e'' = COption.fold_right (CAst.with_val (Name.fold_right g)) ona e in
      CLetTuple (nal,(ona,COption.map (f e'') po),f e b,f e' c)
    | CIf (c,(ona,po),b1,b2) ->
      let e' = COption.fold_right (CAst.with_val (Name.fold_right g)) ona e in
      CIf (f e c,(ona,COption.map (f e') po),f e b1,f e b2)
    | CFix (id,dl) ->
...
let locs_of_notation ?loc locs ntn =
  let unloc loc = COption.cata Loc.unloc (0,0) loc in
  let (bl, el) = unloc loc        in
  let locs =  List.map unloc locs in
  let rec aux pos = function
    | [] -> if CInt.equal pos el then [] else [(pos,el)]
    | (ba,ea)::l -> if CInt.equal pos ba then aux ea l else (pos,ba)::aux ea l
