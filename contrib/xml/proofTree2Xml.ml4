...
(* Transformer un constr(?) en Xml.token Stream.t *)
let constr_to_xml obj sigma env =
  try
  let pvars = List.map (fun (id,_,_)-> id,[Names.string_of_id id])
  ...
  let seed = ref 0 in
   let annobj =
   Cic2acic.acic_of_cic_context'
       seed ids_to_terms constr_to_ids ids_to_father_ids
       ids_to_inner_sorts ids_to_inner_types
       pvars env sigma obj None in
   let xml = Acic2Xml.print_term ids_to_inner_sorts annobj in
   xml
   with _ ->( print_string "Problem during converting constr into XML";
              Stream.of_list [])
...
(Printer.prterm constr)) ;
  ...
  X.xml_nempty "Prim" of_attribute
          [< (List.fold_left (fun i n -> [< i ; (aux n old_hyps false) >])
[<>] nodes) >]
...
*)
  ...
  if forget
  then aux flat_proof old_hyps false
  else
  (
  ...
  let {Evd.evar_concl=concl;
       Evd.evar_hyps=hyps}=goal in
        ...
        let xgoal = X.xml_nempty "Goal" []
               (constr_to_xml concl sigma env) in

 let rec build_hyps hyps = match hyps with
          | [] -> xgoal
          | (id,c,tid)::hyps1 ->
       [< build_hyps hyps1;
          (X.xml_nempty "Hypothesis"
                   ["name",(Names.string_of_id id)]
     (constr_to_xml tid sigma env))
          >] in
        let old_names=List.map (fun (id,c,tid)->id) old_hyps in
        let new_hyps = List.filter
           (fun (id,c,tid)-> not (List.mem id old_names)) hyps in

 X.xml_nempty "Tactic" of_attribute
              [<(build_hyps new_hyps);
                       (aux flat_proof hyps true)>]
        )
