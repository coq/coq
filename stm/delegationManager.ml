...
type ('a,'b) corresponding = { on_worker : 'b; on_master : 'a; id : sentence_id }
...
let ids_of_mapping_m { taskmap; _ } =
  Int.Map.bindings taskmap |> List.map (fun (_, { id; _ }) -> id)

type 'a remote_mapping = {
  it : 'a marshalable_remote_mapping;
  progress_hook : unit -> unit Lwt.t
}


let marshalable_remote_mapping { it } = it
...
let new_manager : 'a remote_mapping -> link -> unit = fun remote_mapping link ->
  ...
  let rec main () =
    ...
    let resolver = Int.Map.find i remote_mapping.it.taskmap in
...
let new_worker : 'a marshalable_remote_mapping -> link -> unit = fun remote_mapping link ->
...
let lwt_remotely_wait_m (r : 'a marshalable_remote_mapping) id :  'a marshalable_remote_mapping * ('a Lwt.t * 'a Lwt.u) =
  let j = r.taskno in
  let m = r.taskmap in
  (* task = cancellable promise *)
  let master, on_master = Lwt.task () in
  let on_worker, worker = Lwt.task () in
  let m = Int.Map.add j { on_master; on_worker; id } m in
  { taskno = j+1; taskmap = m }, (master, worker)
;;
let lwt_remotely_wait r id =
  let it, x = lwt_remotely_wait_m r.it id in
  { r with it }, x
...
let fork_worker : 'a remote_mapping -> (role * 'b events) Lwt.t = fun remote_mapping ->
  ...
  if pid = 0 then
    ...
    new_worker (marshalable_remote_mapping remote_mapping) link;
    Lwt.return (Worker, [])
  else
...
let create_process_worker procname state remote_mapping job =
  ...
  | Some (worker, _) ->
      ...
      log @@ "[M] sending state";
      Lwt_io.write_value write_to () >>= fun () ->
      log @@ "[M] sending job";
