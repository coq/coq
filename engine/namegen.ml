...
let next_name_away_in_cases_pattern env_t na avoid =
  ...
  let bad id = Names.Id.List.mem id avoid || is_constructor id
...
let next_ident_away_in_goal id avoid =
  let id = if Names.Id.List.mem id avoid then restart_subscript id else id in
  let bad id = Names.Id.List.mem id avoid || (is_global id && not (is_section_variable id)) in
  next_ident_away_from id bad
...
let next_global_ident_away id avoid =
  let id = if Names.Id.List.mem id avoid then restart_subscript id else id in
  let bad id = Names.Id.List.mem id avoid || is_global id in
  next_ident_away_from id bad
...
let next_ident_away id avoid =
  if Names.Id.List.mem id avoid then
    next_ident_away_from (restart_subscript id) (fun id -> Names.Id.List.mem id avoid)
  else id
...
let next_ident_away_for_default_printing env_t id avoid =
  let visible = visible_ids env_t in
  let bad id = Names.Id.List.mem id avoid || Id.Set.mem id visible in
  next_ident_away_from id bad
