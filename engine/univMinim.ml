...
let normalize_context_set ~lbound g ctx (us:UnivFlex.t) {weak_constraints=weak;above_prop} =
  ...
  let ctx, us, smallles, g = UPairSet.fold (fun (u,v) (ctx, us, smallles, g as acc) ->
      let norm = level_subst_of (UnivFlex.normalize_univ_variable us) in
      let u = norm u and v = norm v in
      if get_set_minimization() && (Level.is_set u || Level.is_set v) then begin
        if Level.is_set u then (ctx,us,Constraints.add (u,Le,v) smallles,g)
        else (ctx,us,Constraints.add (v,Le,u) smallles,g)
      end else
        let set_to a b =
