...
let cmp_inductives cv_pb (mind,ind as spec) nargs u1 u2 cstrs =
  ...
  | Some variances ->
    ...
    else compare_cumulative_instances ~nargs:(NumArgs nargs) cv_pb (UVars.Variances.application_variances variances) u1 u2 cstrs
...
let cmp_constants cv_pb cb nargs u1 u2 cstrs =
  ...
  | Some variance ->
    ...
    compare_cumulative_instances ~nargs:(NumArgs nargs) cv_pb (UVars.Variances.application_variances variance) u1 u2 cstrs
...
let eq_universes env sigma cstrs cv_pb refargs l l' =
  if EInstance.is_empty l then (assert (EInstance.is_empty l'); true)
    ...
    | Some (ConstRef c, 1) when Environ.is_array_type env c ->
      cstrs := compare_cumulative_instances ~nargs:(NumArgs 1) cv_pb (UVars.Variances.application_variances CPrimitives.array_variances) l l' !cstrs;
      true
    | Some (ConstRef c, n) ->
