...
let mkFixOpt ((vn, i), rec_decl) = of_kind (Fix ((Array.map (fun i -> Some i) vn, i), rec_decl))
...
let compare_head_gen_proj env sigma equ eqs (eqc' : constr Constr.constr_compare_fn) nargs m n =
  let kind c = kind sigma c in
