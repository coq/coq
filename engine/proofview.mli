...
val tclZERO : ?info:(Exninfo.info * Printexc.raw_backtrace) -> exn -> 'a tactic
...
(** [tclOR t1 t2] behaves like [t1] as long as [t1] succeeds. Whenever
    ...
    backtracking point. *)
val tclOR : 'a tactic -> (exn_data -> 'a tactic) -> 'a tactic
...
val tclBREAK : (exn_data -> exn_data option) -> 'a tactic -> 'a tactic
...
sig
  ...
  val repr : 'a tactic -> ('a, state, state, exn_data) Logic_monad.BackState.t
  val make : ('a, state, state, exn_data) Logic_monad.BackState.t -> 'a tactic
end
