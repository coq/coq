...
module Delayed = struct
  ...
  let in_delayed ~file ~whatfor ch ~segment =
    let seg = get_segment ch ~segment in
    let digest = seg.hash in
    { del_what = segment; del_whatfor = whatfor;
      del_file = file; del_digest = digest; del_off = seg.pos; }

  let make ~file ~whatfor ch ~segment =
    let del = in_delayed ~file ~whatfor ch ~segment in
    ref (ToFetch del)
  ...
  let fetch_delayed ~verbose del =
    let { del_what = what; del_whatfor = whatfor;
          del_digest = digest; del_file = f; del_off = pos; } = del in
    let what = what.id in
    let () = if verbose then
        Feedback.msg_info
          Pp.(str"Fetching " ++ str what ++ str" from disk for " ++ str whatfor)
    in
    ...
    in
    Stdlib.close_in ch;
    if not (String.equal digest digest') then Error (Faulty (f,what,whatfor))
    else Ok obj
