...
let create ~name ~category ?(default=Enabled) pp =
  ...
  fun ?loc x -> let w = Hashtbl.find warnings name in
           ...
           | Enabled ->
              ...
              Feedback.feedback (Feedback.WarningMsg (loc, Pp.string_of_ppcmds msg))
...
let reset_default_warnings () =
  Hashtbl.filter_map_inplace (fun name w -> Some { w with status = w.default }) warnings

let set_all_warnings_status status =
  Hashtbl.filter_map_inplace (fun name w -> Some { w with status }) warnings
...
let parse_flags s = 
  if String.equal s "none" then begin
