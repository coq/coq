...
The syntax of the extended {\tt match} is presented in figure 
\ref{ecases-grammar}.
Remark that the annotation is mandatory when the set of equations is
empty.
...
{\pattern}  & ::= & {\ident} \\
 & $|$ & \_ \\
 & $|$ & {\ident} \nelist{\pattern}{} \\
 & $|$ & {\pattern} \texttt{as} {\ident} \\
 & $|$ & {\pattern} \texttt{\%} {\ident} \\
 & $|$ & \texttt{(} {\pattern} \texttt{)} \\
 &&\\

{\exteqn} & ::= &  \nelist{pattern}{,} \texttt{=>} {\term} \\
 && \\

{\term} & ::= &  
\texttt{match} \nelist{\caseitem}{,} \zeroone{{\tt return} {\type}}
\texttt{with}
\sequence{\exteqn}{$|$} \texttt{end} \\
\hline
\end{tabular}
\caption{extended {\tt match} syntax.}
...
use a {\tt if ... then ... else} notation. This extends the syntax of
terms as follows:

\medskip
\begin{tabular}{rcl}
{\returntype} & := & \zeroone{{\tt as} {\ident}} {\tt return} {\type}\\
{\term} & ::= & {\tt if} {\term} \zeroone{\returntype} {\tt then} {\term} {\tt else} {\term}\\
\end{tabular}
\medskip
...
This extends the syntax of terms as follows:

\medskip
\begin{tabular}{rcl}
 & $|$ & {\tt let (} \nelist{\ident}{,} {\tt )} \zeroone{\returntype} {\tt :=}  {\term} {\tt in} {\term} \\
\end{tabular}
\medskip
...
\SeeAlso Section \ref{locate} about command {\tt Locate}.
...
function, or of the type of the surrounding context of the applied
function. Hence, an implicit argument is an argument corresponding to
a dependent argument of the type of the function. Typical implicit
...
can erase its arguments), not in the body of a {\tt match}, and not
...
type and {\tt A} will always be inferable from the (normal) type {\tt
...
An implicit argument can be {\em contextual} or non. An implicit is
said {\em contextual} if it can be inferred only from the knowledge of
the type of the context of the current expression. For instance, the only argument of 
...
a type {\tt (Transitivity R)} which is reducible to {\tt forall x,y:U, R x
...
In case one wants to systematically declared implicit the arguments
...
Conversely, one way unset the mode by using {\tt Unset Implicit Arguments}.
The mode is off by default.

Auto-detection of implicit arguments is governed by options
controlling whether strict and contextual implicit arguments have to
be considered or not.

\subsubsection{Strict implicit arguments mode}
...
arguments. To relax this constraint, use command {\tt Unset Strict
Implicit}. Conversely, use command {\tt Set Strict Implicit} to
restore the strict implicit mode.

\Rem In versions of {\Coq} prior to version 8.0, the default was to consider the strict implicit arguments as implicit. 

\subsubsection{Contextual implicit arguments mode}
\comindex{Set Contextual Implicit}
\comindex{Unset Contextual Implicit}

By default, {\Coq} does not automatically set implicit the contextual
implicit argument. To tell {\Coq} to infer also contextual implicit
argument, use command  {\tt Set Contextual
Implicit}. Conversely, use command {\tt Unset Contextual Implicit} to
...
application. The syntax for this {\tt (\ident:=\term)} where {\ident}
...
variables starting with {\ident} (i.e. either {\ident} itself or
