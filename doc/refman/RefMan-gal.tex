...
Section~\ref{Binders} apply. Functions over several variables are
...
occur in the list of binders, it is expanded to a local definition
...
Note that {\term} is intended to be a type (even if {\term} and
{\type} lives in the same grammatical category).
...
{\Coq} expressions often contains redundant informations. Subterms that
can be automatically inferred by {\Coq} can be replaced by the
symbol ``\_'' and {\Coq} will guess the missing information.
...
construction, that are called {\em pattern-matching} expressions.  A
...
must denotes a constructor. There should be exactly one branch for
...
lists of a given length (see section~\ref{listn}). In this situation,
...
\begin{ErrMsgs}
\item \errindex{The {\num}$^{\mbox{\scriptize th}}$ argument of
{\ident} must be {\ident'} in {\type}}
\end{ErrMsgs}
