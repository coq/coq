...
Letouzey with a rewriting of the FSet library and a new round of
progresses in the Numbers library and to Hugo Herbelin for a slight
revision of the Sorting library and the addition of a certified
merge-sort. The reason for making FSet evolve is that the
computational and logical contents were quite intertwined in the
original implementation, leading in some cases to longer computations
than expected and this problem is solved in the new MSet
implementation. As for the Numbers library, it was only dealing with a
few arithmetic operators in the former version and its current
extension now allows to instantaneously derive the standard properties
of division from any implementation of $\mathbb{N}$ or $\mathbb{Z}$ or
$\mathbb{Z}/n\mathbb{Z}$.

The module system evolved significantly. Besides the resolution of
efficiency issues, Élie Soubiran brought a new model of name
...
{\verb!<+! for nesting functor application, what provide a light
  notation for inheriting the properties of stacks of modules.

Gb

The tactics evolved too.
