...
\subsection{Deprecated Implicit Arguments command
\comindex{Implicit Arguments}}

Before version 8.4 the command to specify the implicit arguments of a constant
was {\tt Implicit Arguments} followed by a bracketed list of identifiers.
Starting with Coq 8.4 the {\tt Argument} command must be used (see
\ref{ImplicitArguments}).
...
The first example declares the arguments of {\tt map} named {\tt A} and
{\tt B} implicit. The second one declares the argument named {\tt A} implicit
and maximally inserted.
