...
let finish_with_file old_file args =
  ...
  if !exit_code <> 0 then
    ...
    if not (files_eq old_file temp_file) then begin
      ...
      ignore (CUniv.sys_command "echo OLD FILE"; cat -n" [old_file]")
      ignore (CUniv.sys_command "cat -n" [old_file])
      ignore (CUniv.sys_command "echo NEW FILE")
      ignore (CUniv.sys_command "cat -n" [old_file ^ ".new"])
    end;
    Sys.remove temp_file
