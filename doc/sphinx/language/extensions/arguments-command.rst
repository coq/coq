...
+ `simpl never` indicates that a :term:`constant` should not be unfolded by :tacn:`cbn` or
  ...
  modifier does not apply in position of reduction of the main argument of a `match`, `fix`,
  ...
  On the other side, an expression like :g:`(Nat.sub (S x) 0) + 1`
  reduces to :g:`S (x + 1)` for :tacn:`simpl` because `Nat.sub`
  is this time in position of main argument of `+`.
