...
Sort quality variables which appear only in :ref:`relevance marks<relevancemarks>` in the replacement
...
have the type that the term being rewritten had (as described in :cite:`Rew24`).
...
Coq currently doesn't check for confluence of the rewrite rules,
even though it is required for the invariants that the typechecker use.
There are plans to add a check using the triangle criterion described in :cite:`TotR21`.

Coq doesn't check for termination of the rewrite rules either.
The reason is that allowing non-terminating rules doesn't break the invariants used by the typechecker
except for one which is very noticeable: the typechecker might not always terminate anymore.
Contrary to the previous two properties, non-termination cannot introduce crashes or anomalies.
...
A check is done to test whether this may happen,
but it is not complete (false positives)
and thus only emits a warning if failed, as with type preservation.
