...
the typechecker might not always terminate anymore.
Unlike the previous two properties, non-termination cannot introduce crashes or anomalies.
...
Rocq checks whether this may happen,
but the check is not complete (false positives).
Therefore, the check fails by only emitting a warning, similarly to the check for type preservation.
