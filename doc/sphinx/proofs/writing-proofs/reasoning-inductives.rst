...
.. tacv:: induction @term {* gen:@ident}

   This syntax is used for generalizing the induction hypothesis over
   each hypothesis prefixed with ``gen:``. For instance:
...
.. tacv:: induction @term in @goal_occurrences
   ...
   those are generalized as well in the statement to prove, as if they
   had been given with the ``gen:`` clause.

.. tacv:: induction {+, @term {? with @bindings}} {? as @disj_conj_intro_pattern} {? using @term {? with @bindings}} {* gen:@ident} {? in @goal_occurrences}
          einduction {+, @term {? with @bindings}} {? as @disj_conj_intro_pattern} {? using @term {? with @bindings}} {* gen:@ident} {? in @goal_occurrences}
 
   These are the most general forms of ``induction`` and ``einduction``. It combines the
   effects of the ``with``, ``as``, ``using``, ``in`` and ``gen:`` clauses. Note however
   that not all combinations are supported if more than one term is given.
