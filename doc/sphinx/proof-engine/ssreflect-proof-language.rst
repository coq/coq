...
.. tacn:: @ltac_expr ; last @ssrseqarg
   ...
   :tacn:`last<last (ssreflect)>` and :tacn:`first<first (ssreflect)>` are :token:`l4_tactic`\s.
...
.. tacv:: last first
          first last
   :name: last first (ssreflect); first last (ssreflect)
   :undocumented:
...
.. flag:: SsrOldRewriteGoalsOrder
   ...
   from having to add :tacn:`last first (ssreflect)` tactics that would be needed
