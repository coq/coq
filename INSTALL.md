...
To build and install Coq (and CoqIDE if desired) do:
    ...
    $ make dunestrap
    $ dune build -p coq-runtime-types,coq-core,coq-stdlib,coq,coqide-server,coqide
    $ dune install --prefix=<install_prefix> coq-runtime-types,coq-core coq-stdlib coq coqide-server coqide
