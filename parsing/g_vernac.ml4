...
GEXTEND Gram
  ...
  vernac: FIRST
    [ [ IDENT "Time"; v = located_vernac -> VernacTime v
      | IDENT "Redirect"; s = ne_string; v = located_vernac -> VernacRedirect (s, v)
...
let test_plurial_form_types = function
  | [([_],_)] ->
      Flags.if_verbose msg_warning (strbrk "Keywords Implicit Types expect more than one type")
  | _ -> ()
...
GEXTEND Gram
  ...
  record_binder_body:
    [ [ l = binders; oc = of_type_with_opt_coercion;
        t = lconstr -> fun id ->
          (oc,AssumExpr (id,mkCProdN (!@loc) l t))
      | l = binders; oc = of_type_with_opt_coercion;
  ...
  constructor_type:
    [ [ l = binders;
        t = [ coe = of_type_with_opt_coercion;
              c = lconstr ->
    ...
    [ [ (* Transparent and Opaque *)
      ...
      | IDENT "Canonical"; IDENT "Structure"; qid = global;
        d = def_body ->
          ...
          VernacDefinition ((Some Global,CanonicalStructure),((Loc.ghost,s),None),d)
      ...
      | IDENT "Context"; c = binders -> VernacContext c
      ...
      | IDENT "Implicit"; IDENT "Arguments"; qid = smart_global;
	      pos = LIST0 [ "["; l = LIST0 implicit_name; "]" ->
	        List.map (fun (id,b,f) -> (ExplByName id,b,f)) l ] ->
	          Flags.if_verbose msg_warning (strbrk "Implicit Arguments is deprecated; use Arguments instead");
...
GEXTEND Gram
  ...
  ;
  only_parsing:
    [ [ "("; IDENT "only"; IDENT "parsing"; ")" -> Some Flags.Current
      | "("; IDENT "compat"; s = STRING; ")" -> Some (Coqinit.get_compat_version s)
      | -> None
    ] ]
  ;
  obsolete_locality:
