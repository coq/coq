...
- Extraction: names which clash with OCaml builtins (like `int`, `option`,
  `bool`,...) are now renamed in extracted code (typically to, `int0`,
  `option0`, `bool0`). This fixes situations where extraction was producing
  incorrect code due to the fact that these builtins can no longer be accessed
  once shadowed (https://github.com/coq/coq/issues/7017). This change can create
  incompatibilities with code relying on the names generated by extraction for
  these types or their constructors.
