...
let register_proof_mode ({name = n} as m) =
  Hashtbl.add proof_modes n (Coq_ephemeron.create m)
...
let _ =
  Goptions.declare_string_option {Goptions.
    ...
    optread = begin fun () ->
      (Coq_ephemeron.default !default_proof_mode standard).name
    end;
...
let update_proof_mode () =
  match !pstates with
  | { mode = m } :: _ ->
      Coq_ephemeron.iter_opt !current_proof_mode (fun x -> x.reset ());
      current_proof_mode := m;
      Coq_ephemeron.iter_opt !current_proof_mode (fun x -> x.set ())
  | _ ->
      Coq_ephemeron.iter_opt !current_proof_mode (fun x -> x.reset ());
...
let activate_proof_mode mode =
  Coq_ephemeron.iter_opt (find_proof_mode mode) (fun x -> x.set ())
let disactivate_proof_mode mode =
  Coq_ephemeron.iter_opt (find_proof_mode mode) (fun x -> x.reset ())
...
let start_proof sigma id ?pl str goals terminator =
  let initial_state = {
    pid = id;
    terminator = Coq_ephemeron.create terminator;
...
let close_proof ~keep_body_ucst_separate ?feedback_id ~now fpl =
  ...
  fun pr_ending -> Coq_ephemeron.get terminator pr_ending
...
let get_terminator () = Coq_ephemeron.get ( cur_pstate() ).terminator
let set_terminator hook =
  match !pstates with
  | [] -> raise NoCurrentProof
  | p :: ps -> pstates := { p with terminator = Coq_ephemeron.create hook } :: ps
