...
let start_subproof gln p =
  ...
  | SubproofAbstract ->
    ...
    | g :: _ ->
      ...
      in
      let g', pr, _, _ = Proofview.apply ~name:p.name ~poly:p.poly (Global.env()) tac p.proofview in
...
let end_subproof p0 =
  ...
  | Some (g,g') ->
    ...
    in
    let (), pr, _, _ = Proofview.apply ~name:p.name ~poly:p.poly (Global.env()) tac p.proofview in
    let p = { p with proofview = pr } in
    p
