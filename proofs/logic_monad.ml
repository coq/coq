...
struct
  ...
  let print_debug   s = make (fun _ -> Pp.msg_info s)
  let print_info    s = make (fun _ -> Pp.msg_info s)
  let print_warning s = make (fun _ -> Pp.msg_warning s)
  let print_error   s = make (fun _ -> Pp.msg_error s)
  let print_notice  s = make (fun _ -> Pp.msg_notice s)
