...
module New : sig
  ...
  val pf_apply : (env -> evar_map -> 'a) -> evar_map -> Proofview.Goal.t -> 'a

  (** FIXME: encapsulate the level in an existential type. *)
  val of_old : (Goal.goal Evd.sigma -> 'a) -> evar_map -> Proofview.Goal.t -> 'a
  ...
  val pf_unsafe_type_of : evar_map -> Proofview.Goal.t -> constr -> types
  ...
  val pf_get_type_of : evar_map -> Proofview.Goal.t -> constr -> types

  (** This function entirely type-checks the term and computes its type
      and the implied universe constraints. *)
  val pf_type_of : evar_map -> Proofview.Goal.t -> constr -> evar_map * types
  val pf_conv_x : evar_map -> Proofview.Goal.t -> t -> t -> bool
  ...
  val pf_get_hyp : Id.t -> evar_map -> Proofview.Goal.t -> named_declaration
  val pf_get_hyp_typ : Id.t -> evar_map -> Proofview.Goal.t -> types
  val pf_last_hyp : Proofview.Goal.t -> named_declaration

  val pf_nf_concl : evar_map -> Proofview.Goal.t -> types
  val pf_reduce_to_quantified_ind : evar_map -> Proofview.Goal.t -> types -> (inductive * EInstance.t) * types

  val pf_hnf_constr : evar_map -> Proofview.Goal.t -> constr -> types
  val pf_hnf_type_of : evar_map -> Proofview.Goal.t -> constr -> types

  val pf_compute : evar_map -> Proofview.Goal.t -> constr -> constr
